...
- Operational supply and encirclement added
  → Regions now become Supplied, Strained, or Isolated based on connected land, rail-linked factory relays, port access, and naval blockade pressure.
  → Isolated areas keep fighting on local reserves for a while before harsher combat, reinforcement, and economic penalties begin.
  → A new supply overlay and region panel tooltip make logistics state readable in live matches.
- New Attack Modes layer: Blitz, Siege, and Raid add selectable offensive profiles with distinct capture speed, casualty, suppression, and disruption tradeoffs
...
- Add mission-based air warfare with Airbases, Fighter Squadrons, Bomber Squadrons, range checks, rearming, interception, UI targeting, and nation AI usage.
- Add a layered anti-air defense system with AA Batteries, Radar Stations, radar warning events, fighter/radar support bonuses, and diminishing returns on overlapping AA coverage.
