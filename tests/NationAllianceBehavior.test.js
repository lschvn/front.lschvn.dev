...
describe("AllianceBehavior.handleAllianceRequests", () => {
    beforeEach(async () => {
        ...
        const playerInfo = new PlayerInfo("player_id", PlayerType.Bot, null, "player_id");
        const requestorInfo = new PlayerInfo("requestor_id", PlayerType.Human, null, "requestor_id");
        ...
        allianceBehavior = new NationAllianceBehavior(random, game, player, new NationEmojiBehavior(random, game, player));
        while (game.inSpawnPhase()) {
            game.executeNextTick();
        }
    });
    function setupAllianceRequest({ isTraitor = false, relationDelta = 2, numTilesPlayer = 10, numTilesRequestor = 10, alliancesCount = 0, createdAtTick = game.ticks() + 1, } = {}) {
        if (isTraitor)
            requestor.markTraitor();
...
describe("AllianceBehavior.handleAllianceExtensionRequests", () => {
    ...
    beforeEach(() => {
        ...
        allianceBehavior = new NationAllianceBehavior(mockRandom, mockGame, mockPlayer, new NationEmojiBehavior(mockRandom, mockGame, mockPlayer));
    });
...
//# sourceMappingURL=NationAllianceBehavior.test.js.map