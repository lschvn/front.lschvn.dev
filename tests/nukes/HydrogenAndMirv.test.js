...
describe("Hydrogen Bomb and MIRV flows", () => {
    ...
    test("Hydrogen bomb launch fails when silo is under construction and succeeds after completion", async () => {
        ...
        gameWithConstruction.addExecution(new SpawnExecution(gameID, info, gameWithConstruction.ref(1, 1)));
        ...
        gameWithConstruction.addExecution(new ConstructionExecution(playerWithConstruction, UnitType.MissileSilo, siloTile));
        gameWithConstruction.executeNextTick();
        gameWithConstruction.executeNextTick();
        ...
        const hydrogenBombCountBefore = playerWithConstruction.units(UnitType.HydrogenBomb).length;
        const canBuildResult = playerWithConstruction.canBuild(UnitType.HydrogenBomb, targetTile);
        expect(canBuildResult).toBe(false); // Should fail because silo is under construction
        // Try to add execution - should fail
        gameWithConstruction.addExecution(new ConstructionExecution(playerWithConstruction, UnitType.HydrogenBomb, targetTile));
        gameWithConstruction.executeNextTick();
        gameWithConstruction.executeNextTick();
        // Assert launch does not succeed
        const hydrogenBombCountAfter = playerWithConstruction.units(UnitType.HydrogenBomb).length;
        ...
        const constructionDuration = gameWithConstruction.unitInfo(UnitType.MissileSilo)
        ...
        const canBuildAfterCompletion = playerWithConstruction.canBuild(UnitType.HydrogenBomb, targetTile);
        ...
        const hydrogenBombCountAfterSuccess = playerWithConstruction.units(UnitType.HydrogenBomb).length;
        expect(hydrogenBombCountAfterSuccess).toBeGreaterThan(hydrogenBombCountBefore);
    });
...
//# sourceMappingURL=HydrogenAndMirv.test.js.map