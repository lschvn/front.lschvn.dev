...
import { ColorAllocator, selectDistinctColorIndex, } from "../src/core/configuration/ColorAllocator";
import { blue, botColor, green, orange, purple, red, teal, yellow, } from "../src/core/configuration/Colors";
...
describe("ColorAllocator", () => {
    ...
    test("assignTeamPlayerColor always returns the same color for the same playerID", () => {
        ...
        const blueColor1 = allocator.assignTeamPlayerColor(ColoredTeams.Blue, playerId);
        const blueColor2 = allocator.assignTeamPlayerColor(ColoredTeams.Blue, playerId);
        expect(blueColor1.isEqual(blueColor2)).toBe(true);
        const redColor1 = allocator.assignTeamPlayerColor(ColoredTeams.Red, playerId);
        const redColor2 = allocator.assignTeamPlayerColor(ColoredTeams.Red, playerId);
    ...
    test("assignTeamPlayerColor returns a different color when the playerID is different", () => {
        ...
        const blueColorPlayerOne = allocator.assignTeamPlayerColor(ColoredTeams.Blue, playerIdOne);
        const blueColorPlayerTwo = allocator.assignTeamPlayerColor(ColoredTeams.Blue, playerIdTwo);
        expect(blueColorPlayerOne.isEqual(blueColorPlayerTwo)).toBe(false);
        const redColorPlayerOne = allocator.assignTeamPlayerColor(ColoredTeams.Red, playerIdOne);
        const redColorPlayerTwo = allocator.assignTeamPlayerColor(ColoredTeams.Red, playerIdTwo);
