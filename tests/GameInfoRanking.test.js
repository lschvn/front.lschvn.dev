import { Ranking, RankType, } from "../src/client/components/baseComponents/ranking/GameInfoRanking";
import { Difficulty, GameMapSize, GameMapType, GameMode, GameType, } from "../src/core/game/Game";
import { GOLD_INDEX_STEAL, GOLD_INDEX_TRADE, GOLD_INDEX_TRAIN_OTHER, GOLD_INDEX_TRAIN_SELF, GOLD_INDEX_WAR, } from "../src/core/StatsSchemas";
describe("Ranking class", () => {
    ...
    test("gold scores work correctly", () => {
        ...
        expect(r.score(p1, RankType.StolenGold)).toBe(Number(p1.gold[GOLD_INDEX_STEAL] ?? 0n));
        expect(r.score(p1, RankType.NavalTrade)).toBe(Number(p1.gold[GOLD_INDEX_TRADE] ?? 0n));
        const ownTrain = p1.gold[GOLD_INDEX_TRAIN_SELF] ?? 0n;
        const otherTrain = p1.gold[GOLD_INDEX_TRAIN_OTHER] ?? 0n;
        expect(r.score(p1, RankType.TrainTrade)).toBe(Number(ownTrain + otherTrain));
        expect(r.score(p1, RankType.ConqueredGold)).toBe(Number(p1.gold[GOLD_INDEX_WAR] ?? 0n));
    });
});
//# sourceMappingURL=GameInfoRanking.test.js.map