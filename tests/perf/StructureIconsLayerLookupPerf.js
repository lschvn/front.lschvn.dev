...
const rendersArray = Array.from({ length: STRUCTURE_COUNT }, (_, index) => ({
    unitId: index + 1,
    ownerId: (index % 5) + 1,
    level: (index % 4) + 1,
}));
const rendersMap = new Map();
for (const render of rendersArray) {
    rendersMap.set(render.unitId, render);
}
const activeLookupIds = Array.from({ length: LOOKUP_COUNT }, (_, index) => ((index * 97) % STRUCTURE_COUNT) + 1);
const inactiveLookupIds = Array.from({ length: LOOKUP_COUNT }, (_, index) => ((index * 193) % STRUCTURE_COUNT) + 1);
const canUpgradeIds = Array.from({ length: UPGRADE_LOOKUP_COUNT }, (_, index) => ((index * 389) % STRUCTURE_COUNT) + 1);
const myOwnerId = 3;
const results = [];
new Benchmark.Suite()
    ...
    for (const canUpgradeId of canUpgradeIds) {
        const potentialUpgrade = localRenders.find((entry) => entry.unitId === canUpgradeId && entry.ownerId === myOwnerId);
        if (potentialUpgrade) {
    ...
    .run({ async: true });
//# sourceMappingURL=StructureIconsLayerLookupPerf.js.map