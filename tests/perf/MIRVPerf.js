...
const sparseTerritoryGame = await setup("big_plains", {
    infiniteGold: true,
    instantBuild: true,
}, [new PlayerInfo("player", PlayerType.Human, "client_id1", "player_id")], dirname(fileURLToPath(import.meta.url)));
...
const denseTerritoryGame = await setup("big_plains", {
    infiniteGold: true,
    instantBuild: true,
}, [new PlayerInfo("player", PlayerType.Human, "client_id1", "player_id")], dirname(fileURLToPath(import.meta.url)));
...
const giantMapGame = await setup("giantworldmap", {
    infiniteGold: true,
    instantBuild: true,
}, [new PlayerInfo("player", PlayerType.Human, "client_id1", "player_id")], dirname(fileURLToPath(import.meta.url)));
...
new Benchmark.Suite()
    ...
    .run({ async: true });
//# sourceMappingURL=MIRVPerf.js.map