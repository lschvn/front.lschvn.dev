...
describe("Alliance acceptance immediately destroys in-flight nukes", () => {
    ...
    test("accepting alliance destroys in-flight nukes between the newly allied players", () => {
        game.addExecution(new NukeExecution(UnitType.AtomBomb, player1, game.ref(5, 5), game.ref(0, 0), -1, 5));
    ...
    test("accepting alliance destroys only nukes between allied players", () => {
        player1.buildUnit(UnitType.MissileSilo, game.ref(0, 0), {});
        game.addExecution(new NukeExecution(UnitType.AtomBomb, player1, game.ref(5, 5), null));
        game.addExecution(new NukeExecution(UnitType.AtomBomb, player1, game.ref(10, 10), null));
        game.executeNextTick(); // init
    ...
    test("accepting alliance displays a nuke-cancellation display message", () => {
        ...
        expect(player1.isFriendly(player2)).toBe(true);
        expect(game.units(UnitType.AtomBomb)).toHaveLength(0);
        const messages = updates[GameUpdateType.DisplayEvent]?.map((e) => e.message) ?? [];
        expect(messages.some((m) => m === "events_display.alliance_nukes_destroyed_outgoing" ||
            m === "events_display.alliance_nukes_destroyed_incoming")).toBe(true);
    });
});
//# sourceMappingURL=AllianceAcceptNukes.test.js.map