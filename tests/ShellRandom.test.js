...
describe("Shell Random Damage", () => {
    ...
    test("Shell damage varies randomly between 200-300 base damage", () => {
        const target = player2.buildUnit(UnitType.Warship, game.ref(coastX + 5, 10), {
        ...
        for (let i = 0; i < numShells; i++) {
            const shell = new ShellExecution(game.ref(coastX, 10), player1, player1.buildUnit(UnitType.Warship, game.ref(coastX, 10), {
    ...
    test("Warship shell attacks have random damage", () => {
        ...
        const target = player2.buildUnit(UnitType.Warship, game.ref(coastX + 2, 10), {
    ...
    test("Defense post shell attacks have random damage", () => {
        ...
        const target = player2.buildUnit(UnitType.Warship, game.ref(coastX + 1, 10), {
            patrolTile: game.ref(coastX + 1, 10),
        });
        const initialHealth = target.health();
    ...
    test("Shell damage is consistent with same random seed", () => {
        ...
        const initialHealth = target.health();
        const shell1 = new ShellExecution(game.ref(coastX, 10), player1, player1.buildUnit(UnitType.Warship, game.ref(coastX, 10), {
            patrolTile: game.ref(coastX, 10),
        }), target);
        const shell2 = new ShellExecution(game.ref(coastX, 10), player1, player1.buildUnit(UnitType.Warship, game.ref(coastX, 10), {
            patrolTile: game.ref(coastX, 10),
        }), target);
        game.executeNextTick();
...
//# sourceMappingURL=ShellRandom.test.js.map