...
import { SUPPLY_ISOLATED_RESERVE_TICKS, SUPPLY_RECALC_INTERVAL_TICKS, } from "../src/core/configuration/SupplyBalance";
import { PlayerInfo, PlayerType, SupplyState, UnitType, } from "../src/core/game/Game";
...
function grantTerritory(player, game, tiles) {
    for (const tile of tiles) {
        player.conquer(tile);
    }
}
function recomputeSupply(game, tick) {
    game.recomputeSupplyIfNeeded(tick);
}
function connectStations(game, ...units) {
...
describe("SupplySystem", () => {
    ...
    it("detects isolation, depletes reserves over time, and recovers after reconnection", () => {
        ...
        recomputeSupply(game, 120 + SUPPLY_ISOLATED_RESERVE_TICKS + SUPPLY_RECALC_INTERVAL_TICKS);
        expect(game.supplyState(game.ref(20, 1))).toBe(SupplyState.Isolated);
        expect(game.isSupplyReserveDepleted(game.ref(20, 1))).toBe(true);
        player.conquer(game.ref(10, 1));
        recomputeSupply(game, 120 + SUPPLY_ISOLATED_RESERVE_TICKS + SUPPLY_RECALC_INTERVAL_TICKS * 2);
        expect(game.supplyState(game.ref(20, 1))).toBe(SupplyState.Supplied);
        expect(game.isSupplyReserveDepleted(game.ref(20, 1))).toBe(false);
    });
    it("blockades port-based overseas supply with enemy warships and restores it when the blockade is broken", () => {
        ...
        owner.addGold(10000000n);
        raider.addGold(10000000n);
...
//# sourceMappingURL=SupplySystem.test.js.map