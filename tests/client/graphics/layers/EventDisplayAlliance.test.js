...
describe("EventsDisplay - alliance renewal cleanup (allianceID based)", () => {
    function makeRenewal(allianceID, focusID, description = "Alliance about to expire") {
        return {
            description,
    ...
    test("removes ONLY renewal events for the broken alliance", () => {
        ...
        // Break alliance A–B
        display.removeAllianceRenewalEvents(allianceAB);
        const remaining = display.events;
        // A–B renewal removed
        expect(remaining.some((e) => e.allianceID === allianceAB)).toBe(false);
        // Other alliances untouched
        expect(remaining.some((e) => e.allianceID === allianceAC)).toBe(true);
        expect(remaining.some((e) => e.allianceID === allianceBC)).toBe(true);
    });
    ...
    test("onAllianceExtensionEvent removes renewal when playerID matches myPlayer", () => {
        ...
        display.events = [makeRenewal(allianceID, mySmallID)];
        display.onAllianceExtensionEvent({
            type: GameUpdateType.AllianceExtension,
            playerID: mySmallID,
            allianceID,
        });
        const remaining = display.events;
        expect(remaining.some((e) => e.allianceID === allianceID)).toBe(false);
    });
    test("onAllianceExtensionEvent keeps renewal when playerID does not match myPlayer", () => {
        ...
        const remaining = display.events;
        expect(remaining.some((e) => e.allianceID === allianceID)).toBe(true);
    });
    test("onAllianceExtensionEvent keeps renewal when myPlayer is null", () => {
        ...
        display.requestUpdate = () => { };
        display.events = [makeRenewal(allianceID, 1)];
        display.onAllianceExtensionEvent({
            type: "AllianceExtension",
            playerID: 1,
            allianceID,
    ...
    test("does not affect non-RENEW_ALLIANCE events", () => {
        ...
        display.removeAllianceRenewalEvents(999);
        const remaining = display.events;
        expect(remaining.some((e) => e.type === MessageType.ALLIANCE_BROKEN)).toBe(true);
        expect(remaining.some((e) => e.type === MessageType.ALLIANCE_ACCEPTED)).toBe(true);
        expect(remaining.some((e) => e.type === MessageType.RENEW_ALLIANCE)).toBe(false);
    });
});
//# sourceMappingURL=EventDisplayAlliance.test.js.map