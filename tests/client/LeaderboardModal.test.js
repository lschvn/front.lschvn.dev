...
vi.mock("../../src/client/Utils", () => ({
    translateText: vi.fn((key) => {
        const translations = {
            "leaderboard_modal.win_score_tooltip": "Weighted wins based on clan participation and match difficulty",
            "leaderboard_modal.loss_score_tooltip": "Weighted losses based on clan participation and match difficulty",
...
vi.mock("../../src/client/Api", () => {
    ...
    return {
        ...
        fetchPlayerLeaderboard: vi.fn(async (page) => {
...
describe("LeaderboardModal", () => {
    let modal;
    const awaitChildUpdate = async (selector) => {
        const el = modal.querySelector(selector);
        if (el?.updateComplete) {
            await el.updateComplete;
        }
    };
    const getClanTable = () => modal.querySelector("leaderboard-clan-table");
    const getPlayerList = () => modal.querySelector("leaderboard-player-list");
    beforeEach(async () => {
        vi.stubGlobal("fetch", vi.fn());
        if (!customElements.get("leaderboard-modal")) {
            customElements.define("leaderboard-modal", LeaderboardModal);
        }
        modal = document.createElement("leaderboard-modal");
    ...
    describe("Tooltip Implementation - Issue #2508", () => {
        it("should render Win Score and Loss Score columns with title attributes", async () => {
            ...
            let winScoreHeader = null;
            let lossScoreHeader = null;
            ...
            expect(winScoreHeader.getAttribute("title")).toBe("Weighted wins based on clan participation and match difficulty");
            expect(lossScoreHeader.getAttribute("title")).toBe("Weighted losses based on clan participation and match difficulty");
        });
        it("should use translateText for tooltip internationalization", async () => {
            ...
            expect(translateText("leaderboard_modal.win_score_tooltip")).toBe("Weighted wins based on clan participation and match difficulty");
            expect(translateText("leaderboard_modal.loss_score_tooltip")).toBe("Weighted losses based on clan participation and match difficulty");
        });
    });
    describe("Player Data Mapping", () => {
        it("should map ranked leaderboard data and set current user entry", async () => {
            ...
            getUserMe.mockResolvedValueOnce({
            ...
            expect(playerData[0]).toEqual(expect.objectContaining({
                playerId: "player-1",
                username: "Alpha",
                clanTag: "[AAA]",
                elo: 1200,
            ...
            expect(playerData[1]).toEqual(expect.objectContaining({
                playerId: "player-2",
                username: "Bravo",
                clanTag: undefined,
                winRate: 0.4,
    ...
    describe("Modal Functionality", () => {
        it("should initialize with default state", () => {
            ...
            expect(modal.activeTab).toBe("players");
        });
    ...
    describe("Modal Interaction", () => {
        it("should switch to clans tab and request clan leaderboard data", async () => {
            ...
            expect(modal.activeTab).toBe("clans");
            expect(global.fetch).toHaveBeenCalledWith("http://localhost:3000/public/clans/leaderboard", { headers: { Accept: "application/json" } });
...
//# sourceMappingURL=LeaderboardModal.test.js.map