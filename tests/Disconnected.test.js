...
import { GameMode, PlayerInfo, PlayerType, UnitType, } from "../src/core/game/Game";
...
describe("Disconnected", () => {
    beforeEach(async () => {
        ...
        const player1Info = new PlayerInfo("Active Player", PlayerType.Human, null, "player1_id");
        const player2Info = new PlayerInfo("Disconnected Player", PlayerType.Human, null, "player2_id");
        player1 = game.addPlayer(player1Info);
        player2 = game.addPlayer(player2Info);
        game.addExecution(new SpawnExecution(gameID, player1Info, game.ref(1, 1)), new SpawnExecution(gameID, player2Info, game.ref(7, 7)));
    ...
    describe("Disconnected team member interactions", () => {
        const coastX = 7;
        beforeEach(async () => {
            const player1Info = new PlayerInfo("[CLAN]Player1", PlayerType.Human, null, "player_1_id");
            const player2Info = new PlayerInfo("[CLAN]Player2", PlayerType.Human, null, "player_2_id");
            game = await setup("half_land_half_ocean", {
                infiniteGold: true,
                instantBuild: true,
                gameMode: GameMode.Team,
                playerTeams: 2, // ignore player2 "kicked" console warn
            }, [player1Info, player2Info], undefined, UseRealAttackLogic);
            game.addExecution(new SpawnExecution(gameID, player1Info, game.map().ref(coastX - 2, 1)), new SpawnExecution(gameID, player2Info, game.map().ref(coastX - 2, 4)));
            while (game.inSpawnPhase()) {
                game.executeNextTick();
            }
        ...
        test("Team Warships should not attack disconnected team mate ships", () => {
            const warship = player1.buildUnit(UnitType.Warship, game.map().ref(coastX + 1, 10), {
                patrolTile: game.map().ref(coastX + 1, 10),
            });
            game.addExecution(new WarshipExecution(warship));
            const transportShip = player2.buildUnit(UnitType.TransportShip, game.map().ref(coastX + 1, 11), {
                troops: 100,
            });
            player2.markDisconnected(true);
        ...
        test("Disconnected player Warship should not attack team members' ships", () => {
            const warship = player2.buildUnit(UnitType.Warship, game.map().ref(coastX + 1, 5), {
                patrolTile: game.map().ref(coastX + 1, 10),
            });
            game.addExecution(new WarshipExecution(warship));
            const transportShip = player1.buildUnit(UnitType.TransportShip, game.map().ref(coastX + 1, 6), {
                troops: 100,
            });
            player2.markDisconnected(true);
            executeTicks(game, 10);
        ...
        test("Player can attack disconnected team mate without troop loss", () => {
            ...
            game.addExecution(new AttackExecution(startTroops, player1, player2.id(), null));
            ...
            game.executeNextTick();
            const expectedFinalTroops = Number(toInt(troopsBeforeAttack) + expectedTotalGrowth);
        ...
        test("Conqueror gets conquered disconnected team member's transport- and warships", () => {
            const warship = player2.buildUnit(UnitType.Warship, game.map().ref(coastX + 1, 1), {
                patrolTile: game.map().ref(coastX + 1, 1),
            });
            const transportShip = player2.buildUnit(UnitType.TransportShip, game.map().ref(coastX + 1, 3), {
                troops: 100,
            });
        ...
        test("Captured transport ship landing attack should be in name of new owner", () => {
            ...
            game.addExecution(new TransportShipExecution(player2, enemyShoreTile, 100));
            executeTicks(game, 1);
        ...
        test("Captured transport ship should retreat to closest owner shore tile", () => {
            ...
            expect(player2.isAlive()).toBe(false);
            expect(transportShip.owner()).toBe(player1);
            const expectedRetreatTile = player1.bestTransportShipSpawn(transportShip.tile());
        ...
        test("Retreating transport ship is deleted if new owner has no shore tiles", () => {
            ...
            game.addExecution(new TransportShipExecution(player2, enemyShoreTile, boatTroops));
            executeTicks(game, 1);
            const transportShip = player2.units(UnitType.TransportShip)[0];
            expect(player2.units(UnitType.TransportShip).length).toBe(1);
            player2.markDisconnected(true);
            ...
            const enemyInfo = new PlayerInfo("Enemy", PlayerType.Human, null, "enemy_id");
            enemy = game.addPlayer(enemyInfo);
            const shoreTiles = Array.from(player1.borderTiles()).filter((t) => game.isShore(t));
            shoreTiles.forEach((tile) => {
                enemy.conquer(tile);
            });
            expect(Array.from(player1.borderTiles()).filter((t) => game.isShore(t)).length).toBe(0);
            executeTicks(game, 1);
            const troopIncPerTick = game.config().troopIncreaseRate(player1);
            const expectedTroopGrowth = toInt(troopIncPerTick * 1);
            const expectedFinalTroops = Number(toInt(player1.troops()) + expectedTroopGrowth);
            transportShip.orderBoatRetreat();
            executeTicks(game, 1);
...
//# sourceMappingURL=Disconnected.test.js.map