...
function getMapsDirectory() {
    return join(dirname(fileURLToPath(import.meta.url)), "../../../../resources/maps");
}
/**
 ...
 */
function extractGraphBuildData() {
    const span = DebugSpan.getLastSpan();
    if (!span || span.name !== "AbstractGraphBuilder:build") {
        return null;
    }
    return {
        nodes: span.data.nodes || [],
        edges: span.data.edges || [],
        nodesCount: span.data.nodesCount || 0,
        edgesCount: span.data.edgesCount || 0,
        clustersCount: span.data.clustersCount || 0,
...
export async function loadMap(mapName) {
    ...
    DebugSpan.disable();
    const cacheEntry = { game, graphBuildData };
    // Store in cache
 ...
 */
export async function getMapMetadata(mapName) {
    ...
    if (graphBuildData) {
        ...
        allNodes = graphBuildData.nodes.map((node) => ({
            id: node.id,
            x: miniMap.x(node.tile),
            y: miniMap.y(node.tile),
        }));
        // Convert edges from DebugSpan data (AbstractEdge format)
        edges = graphBuildData.edges.map((edge) => {
            const nodeA = graphBuildData.nodes.find((n) => n.id === edge.nodeA);
            const nodeB = graphBuildData.nodes.find((n) => n.id === edge.nodeB);
            return {
                fromId: edge.nodeA,
                toId: edge.nodeB,
                from: nodeA
        ...
        console.log(`Map ${mapName}: ${allNodes.length} nodes, ${edges.length} edges (from DebugSpan, built in ${graphBuildData.buildTime.toFixed(2)}ms)`);
    }
    else if (graph) {
        // Fallback: extract directly from graph
        allNodes = graph.getAllNodes().map((node) => ({
            id: node.id,
            x: miniMap.x(node.tile),
            y: miniMap.y(node.tile),
        }));
        ...
        console.log(`Map ${mapName}: ${allNodes.length} nodes, ${edges.length} edges (fallback)`);
    }
    else {
        // No graph available
        allNodes = [];
