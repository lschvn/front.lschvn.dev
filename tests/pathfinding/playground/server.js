...
import { clearCache as clearMapCache, getMapMetadata, listMaps, } from "./api/maps.js";
 ...
 */
app.get("/api/maps/:name/thumbnail", (req, res) => {
    try {
        const { name } = req.params;
        const thumbnailPath = join(dirname(fileURLToPath(import.meta.url)), "../../../resources/maps", name, "thumbnail.webp");
...
app.post("/api/pathfind", async (req, res) => {
    try {
        ...
        // Compute paths
        const result = await computePath(map, from, to, { adapters });
        res.json(result);
    }
    catch (error) {
...
app.post("/api/spatial-query", async (req, res) => {
    try {
        ...
        const result = await computeSpatialQuery(map, ownedTiles, target);
        res.json(result);
    }
    catch (error) {
