...
async function generateScenarioForMap(mapName, options) {
    ...
    try {
        ...
        // Build ports array
        const ports = selectedPorts.map((coord, index) => ({
        ...
        if (additionalRoutesNeeded > 0) {
            ...
            for (let i = 0; i < selectedPorts.length && added < additionalRoutesNeeded; i++) {
                for (let j = ROUTES_PER_PORT + 1; j <= ROUTES_PER_PORT + 3 &&
                    i + j < selectedPorts.length &&
                    added < additionalRoutesNeeded; j++) {
                    routes.push({
                        from: `Port${String(i + 1).padStart(3, "0")}`,
                        to: `Port${String(i + j + 1).padStart(3, "0")}`,
                    });
...
function generateScenarioContent(params) {
