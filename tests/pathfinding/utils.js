...
export function getAdapter(game, name) {
    switch (name) {
        case "a.baseline": {
        ...
        case "a.full": {
            return PathFinderBuilder.create(new AStarWater(game.map())).buildWithStepper(tileStepperConfig(game));
        }
        case "hpa": {
            ...
            const clonedGame = new GameImpl(originalGame._humans, originalGame._nations, originalGame._map, originalGame.miniGameMap, originalGame._config, originalGame._stats);
            clonedGame._miniWaterHPA = new AStarWaterHierarchical(clonedGame.miniMap(), clonedGame._miniWaterGraph, { cachePaths: false });
...
export async function getScenario(scenarioName, adapterName = "hpa") {
    ...
    const routes = scenario.ROUTES.map(([fromName, toName]) => {
        const fromCoord = scenario.PORTS[fromName];
        const toCoord = scenario.PORTS[toName];
        return {
...
export function measurePathLength(adapter, route) {
...
export function measureExecutionTime(adapter, route, executions = 1) {
...
export function printRow(columns, widths) {
...
export function printSeparator(width = 80) {
    console.log("-".repeat(width));
}
export function printHeader(title, width = 80) {
    printSeparator(width);
    console.log(title);
    printSeparator(width);
    console.log("");
}
export async function setupFromPath(mapDirectory, mapName, gameConfig = {}, humans = []) {
    ...
    // Configure the game
    const config = new TestConfig(new (await import("../util/TestServerConfig")).TestServerConfig(), {
