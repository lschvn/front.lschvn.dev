...
import { Cell, GameMode, Nation, PlayerInfo, PlayerType, UnitType, } from "../src/core/game/Game";
...
describe("Nation MIRV Retaliation", () => {
    test("nation retaliates with MIRV when attacked by MIRV", async () => {
        ...
        const nationInfo = new PlayerInfo("defender_nation", PlayerType.Nation, null, "nation_id");
        ...
        attacker.addGold(1000000000n);
        nation.addGold(1000000000n);
        // Verify preconditions
        expect(attacker.units(UnitType.MissileSilo)).toHaveLength(1);
        expect(nation.units(UnitType.MissileSilo)).toHaveLength(1);
        expect(attacker.gold()).toBeGreaterThan(35000000n);
        expect(nation.gold()).toBeGreaterThan(35000000n);
    ...
    test("nation launches MIRV to prevent victory when player approaches win condition", async () => {
        ...
        // Create two players
        const dominantPlayerInfo = new PlayerInfo("dominant_player", PlayerType.Human, null, "dominant_id");
        const nationInfo = new PlayerInfo("defender_nation", PlayerType.Nation, null, "nation_id");
            ...
            for (let x = 60; x < 70; x++) {
                for (let y = 60; y < 70; y++) {
                    ...
                    if (game.map().isLand(tile) && !game.map().hasOwner(tile)) {
                        ...
                        nationTiles++;
                        if (nationTiles >= 10)
                            break; // Need at least some territory
                    }
                }
                if (nationTiles >= 10)
                    break;
        ...
        for (let x = 0; x < game.map().width() && conqueredTiles < targetTiles; x++) {
            for (let y = 0; y < game.map().height() && conqueredTiles < targetTiles; y++) {
                const tile = game.ref(x, y);
                if (game.map().isLand(tile) && !game.map().hasOwner(tile)) {
        ...
        // Give both players enough gold for MIRVs
        dominantPlayer.addGold(100000000n);
        nation.addGold(100000000n);
        // Verify preconditions
        ...
        expect(dominantPlayer.gold()).toBeGreaterThan(35000000n);
        expect(nation.gold()).toBeGreaterThan(35000000n);
        ...
        const dominantTerritoryShare = dominantPlayer.numTilesOwned() / game.map().numLandTiles();
    ...
    test("nation launches MIRV to stop steamrolling player with excessive cities", async () => {
        ...
        const steamrollerInfo = new PlayerInfo("steamroller", PlayerType.Human, null, "steamroller_id");
        const secondPlayerInfo = new PlayerInfo("second_player", PlayerType.Human, null, "second_id");
        const nationInfo = new PlayerInfo("defender_nation", PlayerType.Nation, null, "nation_id");
        ...
        steamroller.addGold(100000000n);
        secondPlayer.addGold(100000000n);
        nation.addGold(100000000n);
        // Verify preconditions
        expect(nation.units(UnitType.MissileSilo)).toHaveLength(1);
    ...
    test("nation launches MIRV to prevent team victory when team approaches victory denial threshold (targets biggest team member)", async () => {
        // Setup game
        const teamPlayer1Info = new PlayerInfo("[ALPHA]team_player_1", PlayerType.Human, null, "team1_id");
        const teamPlayer2Info = new PlayerInfo("[ALPHA]team_player_2", PlayerType.Human, null, "team2_id");
        const nationInfo = new PlayerInfo("defender_nation", PlayerType.Nation, null, "nation_id");
        const game = await setup("big_plains", {
            infiniteGold: true,
            instantBuild: true,
        ...
        let conqueredTiles = 0;
        for (let x = 0; x < game.map().width() && conqueredTiles < teamTargetTiles; x++) {
            for (let y = 0; y < game.map().height() && conqueredTiles < teamTargetTiles; y++) {
                const tile = game.ref(x, y);
                if (game.map().isLand(tile) && !game.map().hasOwner(tile)) {
                    // 3:1 bias towards teamPlayer1 to ensure largest-member targeting is well-defined
                    const teamPlayer = conqueredTiles % 4 === 0 ? teamPlayer2 : teamPlayer1;
        ...
        // Give all players enough gold for MIRVs
        teamPlayer1.addGold(100000000n);
        teamPlayer2.addGold(100000000n);
        nation.addGold(100000000n);
        // Verify preconditions
        expect(nation.units(UnitType.MissileSilo)).toHaveLength(1);
        expect(nation.units(UnitType.MIRV)).toHaveLength(0);
        expect(teamPlayer1.gold()).toBeGreaterThan(35000000n);
        expect(teamPlayer2.gold()).toBeGreaterThan(35000000n);
        expect(nation.gold()).toBeGreaterThan(35000000n);
        expect(nation.isAlive()).toBe(true);
        expect(nation.numTilesOwned()).toBeGreaterThan(0);
        // Verify team has enough territory to trigger team victory denial
        const teamTerritory = teamPlayer1.numTilesOwned() + teamPlayer2.numTilesOwned();
        ...
        for (const gameId of gameIds) {
            ...
            if (teamVictoryDenialSuccessful)
                break;
        }
        ...
        if (teamVictoryDenialTarget) {
            ...
            const biggest = teamPlayer1.numTilesOwned() >= teamPlayer2.numTilesOwned()
                ? teamPlayer1
                : teamPlayer2;
...
//# sourceMappingURL=NationMIRV.test.js.map