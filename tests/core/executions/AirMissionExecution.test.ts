...
function executeTicks(game: Game, ticks: number): void {
...
function buildOwnedStructure(
  player: Player,
  type: UnitType.Airbase | UnitType.Factory,
  tile: TileRef,
): Unit {
...
describe("AirMissionExecution", () => {
  ...
  test("fighters enter rearming and recover to ready after mission completion", async () => {
    ...
    const attackerTile = seedTerritory(
      game,
      attacker,
      25,
  ...
  test("fighter patrols intercept bombers near the defended target", async () => {
    ...
    const attackerAirbase = buildOwnedStructure(
      attacker,
      UnitType.Airbase,
      attackerTile,
    );
    const defenderAirbase = buildOwnedStructure(
      defender,
      UnitType.Airbase,
      defenderTile,
    );
    ...
    expect(bomber.isActive() === false || bomber.health() < startingHealth).toBe(
      true,
    );
  });
