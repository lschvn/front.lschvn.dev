...
describe("SAM", () => {
    beforeEach(async () => {
        ...
        const defender_info = new PlayerInfo("defender_id", PlayerType.Human, null, "defender_id");
        const middle_defender_info = new PlayerInfo("middle_defender_id", PlayerType.Human, null, "middle_defender_id");
        const far_defender_info = new PlayerInfo("far_defender_id", PlayerType.Human, null, "far_defender_id");
        const attacker_info = new PlayerInfo("attacker_id", PlayerType.Human, null, "attacker_id");
    ...
    test("SAMs should target close to launch site", async () => {
        ...
        const sam = defender.buildUnit(UnitType.SAMLauncher, game.ref(1, 1), {});
        game.addExecution(new SAMLauncherExecution(defender, null, sam));
        const nukeExecution = new NukeExecution(UnitType.AtomBomb, attacker, game.ref(targetDistance, 1), null);
        game.addExecution(nukeExecution);
        // Long distance nuke: compute the proper number of ticks
        const ticksToExecute = Math.ceil(targetDistance / game.config().defaultNukeSpeed() + 1);
    ...
    test("SAMs should target only nukes aimed at nearby targets if not close to launch site", async () => {
        ...
        const sam1 = middle_defender.buildUnit(UnitType.SAMLauncher, game.ref(50, 1), {});
        game.addExecution(new SAMLauncherExecution(defender, null, sam1));
        // Far SAM: Should intercept the nuke. Use the far_defender so the SAM can be built
        const sam2 = far_defender.buildUnit(UnitType.SAMLauncher, game.ref(targetDistance, 1), {});
    ...
    test("SAM should have increased level after upgrade", async () => {
        ...
        const upgradeStructureExecution = new UpgradeStructureExecution(defender, defender.units(UnitType.SAMLauncher)[0].id());
        game.addExecution(upgradeStructureExecution);
        executeTicks(game, 2);
        expect(defender.units(UnitType.SAMLauncher)[0].level()).toEqual(2);
    });
});
//# sourceMappingURL=SAMLauncherExecution.test.js.map