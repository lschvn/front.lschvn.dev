...
describe("NukeExecution", () => {
    beforeEach(async () => {
        ...
        game.config().nukeMagnitudes = vi.fn(() => ({
            inner: 10,
            outer: 10,
        }));
        game.config().nukeAllianceBreakThreshold = vi.fn(() => 5);
    ...
    test("nuke should destroy buildings and redraw out of range buildings", async () => {
        ...
        const defensePost = player.buildUnit(UnitType.DefensePost, game.ref(1, 27), {});
        defensePost.touch = vi.fn();
        // Add a nuke execution targeting the city
        const nukeExec = new NukeExecution(UnitType.AtomBomb, player, game.ref(1, 1), game.ref(1, 2));
    ...
    test("nuke should only be targetable near src and dst", async () => {
        ...
        const nukeExec = new NukeExecution(UnitType.AtomBomb, player, game.ref(199, 199), game.ref(1, 1));
        game.addExecution(nukeExec);
    ...
    test("nuke should break alliances on launch", async () => {
        ...
        game.addExecution(new NukeExecution(UnitType.AtomBomb, player, game.ref(85, 85), null));
    ...
    test("nuke should break alliance when destroying ally's building even with few tiles", async () => {
        ...
        game.addExecution(new NukeExecution(UnitType.AtomBomb, player, game.ref(50, 50), null));
        game.executeNextTick(); // init
        game.executeNextTick(); // exec
...
//# sourceMappingURL=NukeExecution.test.js.map