...
describe("Spawn execution", () => {
    ...
    test.each([
        ...
        ["ocean_and_land", 1],
        ["plains", 9],
    ])("Spawn location is found for all players in %s map with %i players", async (mapName, maxPlayers) => {
        const players = [];
        const spawnExecutions = [];
        for (let i = 0; i < maxPlayers; i++) {
            const playerInfo = new PlayerInfo(`player${i}`, PlayerType.Human, `client_id${i}`, `player_id${i}`);
        ...
        for (let i = 0; i < game.allPlayers().length; i++) {
            for (let j = i + 1; j < game.allPlayers().length; j++) {
                const distance = game.manhattanDist(game.allPlayers()[i].spawnTile(), game.allPlayers()[j].spawnTile());
                expect(distance).toBeGreaterThanOrEqual(game.config().minDistanceBetweenPlayers());
            }
        }
    });
    test("Handles spawn failure when map is too crowded", async () => {
        ...
        expect(game.allPlayers().filter((player) => player.spawnTile() !== undefined)
            .length).toBe(1);
    });
    test("Spawn on specific tile", async () => {
        const playerInfo = new PlayerInfo(`player`, PlayerType.Human, `client_id`, `player_id`);
...
//# sourceMappingURL=SpawnExecution.test.js.map