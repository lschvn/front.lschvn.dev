...
describe("PathFinding.Water", () => {
    let game;
    let worldGame;
    function createPathFinder(g = game) {
    ...
    describe("World map routes", () => {
        it("Spain to France (Mediterranean)", () => {
            ...
            const path = pathFinder.findPath(worldGame.ref(926, 283), worldGame.ref(950, 257));
            expect(path).not.toBeNull();
        });
        it("Miami to Rio (Atlantic)", () => {
            const pathFinder = createPathFinder(worldGame);
            const path = pathFinder.findPath(worldGame.ref(488, 355), worldGame.ref(680, 658));
            expect(path).not.toBeNull();
        });
        it("France to Poland (around Europe)", () => {
            const pathFinder = createPathFinder(worldGame);
            const path = pathFinder.findPath(worldGame.ref(950, 257), worldGame.ref(1033, 175));
            expect(path).not.toBeNull();
        });
        it("Miami to Spain (transatlantic)", () => {
            const pathFinder = createPathFinder(worldGame);
            const path = pathFinder.findPath(worldGame.ref(488, 355), worldGame.ref(926, 283));
            expect(path).not.toBeNull();
        });
        it("Rio to Poland (South Atlantic to Baltic)", () => {
            const pathFinder = createPathFinder(worldGame);
            const path = pathFinder.findPath(worldGame.ref(680, 658), worldGame.ref(1033, 175));
            expect(path).not.toBeNull();
        });
    });
    describe("Error handling", () => {
        it("returns NOT_FOUND for null source", () => {
            ...
            const consoleSpy = vi
                .spyOn(console, "error")
                .mockImplementation(() => { });
            const result = pathFinder.next(null, game.ref(8, 0));
        ...
        it("returns NOT_FOUND for null destination", () => {
            ...
            const consoleSpy = vi
                .spyOn(console, "error")
                .mockImplementation(() => { });
            const result = pathFinder.next(game.ref(8, 0), null);
            expect(result.status).toBe(PathStatus.NOT_FOUND);
            consoleSpy.mockRestore();
        });
    });
    describe("Known bugs", () => {
        it("path can cross 1-tile land barrier", () => {
            ...
            const path = pathFinder.findPath(syntheticGame.ref(0, 0), syntheticGame.ref(9, 0));
            expect(path).not.toBeNull();
        });
        it("path can cross diagonal land barrier", () => {
            ...
            const pathFinder = createPathFinder(syntheticGame);
            const path = pathFinder.findPath(syntheticGame.ref(0, 0), syntheticGame.ref(1, 1));
            expect(path).not.toBeNull();
        });
    });
});
//# sourceMappingURL=PathFinding.Water.test.js.map