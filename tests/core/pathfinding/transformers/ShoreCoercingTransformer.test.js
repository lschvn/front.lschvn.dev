...
describe("ShoreCoercingTransformer", () => {
    ...
    function createMockPathFinder() {
        const mock = {
            ...
            findPath(from, to) {
                ...
                if (mock.returnPath !== undefined)
                    return mock.returnPath;
    ...
    describe("findPath", () => {
        ...
        it("handles multiple sources, filters invalid ones", () => {
            ...
            const result = transformer.findPath([waterSrc, shoreSrc, landSrc], waterDest);
            expect(result).not.toBeNull();
            expect(inner.calls).toHaveLength(1);
            const fromArg = inner.calls[0].from;
            expect(Array.isArray(fromArg)).toBe(true);
            expect(fromArg.length).toBe(2);
        });
...
//# sourceMappingURL=ShoreCoercingTransformer.test.js.map