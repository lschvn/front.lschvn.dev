...
import { ConnectedComponents, LAND_MARKER, } from "../../../src/core/pathfinding/algorithms/ConnectedComponents";
    ...
    describe("direct terrain access optimization", () => {
        it("produces same results with accessTerrainDirectly=false", () => {
            ...
            for (let y = 0; y < 5; y++) {
                for (let x = 0; x < 7; x++) {
                    const tile = map.ref(x, y);
                    expect(wcDirect.getComponentId(tile)).toBe(wcIndirect.getComponentId(tile));
                }
            }
        });
    });
});
//# sourceMappingURL=WaterComponents.test.js.map