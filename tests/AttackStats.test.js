...
describe("AttackStats", () => {
    beforeEach(async () => {
        ...
        game.addExecution(new SpawnExecution(gameID, player1.info(), game.ref(50, 50)));
        game.addExecution(new SpawnExecution(gameID, player2.info(), game.ref(50, 55)));
    ...
    test("should increase war gold stat when a player is eliminated", () => {
        ...
        game.addExecution(new AttackExecution(1, player2, game.terraNullius().id()));
        game.executeNextTick();
...
function expectWarGoldStatIsIncreasedAfterKill(game, attacker, defender) {
    ...
    expect(attackerStats?.gold?.[GOLD_INDEX_WAR]).toBe(defenderStats?.gold?.reduce((acc, g) => acc + g, 0n));
}
function performAttack(game, attacker, defender) {
    // Execute the attack
    game.addExecution(new AttackExecution(attacker.troops(), attacker, defender.id()));
    // Wait for the attack to complete
    do {
        game.executeNextTick();
    } while (attacker.outgoingAttacks().length > 0);
}
//# sourceMappingURL=AttackStats.test.js.map