...
let defenderSpawn;
let attackerSpawn;
function sendBoat(target, troops) {
...
describe("Attack", () => {
    beforeEach(async () => {
        ...
        const attackerInfo = new PlayerInfo("attacker dude", PlayerType.Human, null, "attacker_id");
        game.addPlayer(attackerInfo);
        const defenderInfo = new PlayerInfo("defender dude", PlayerType.Human, null, "defender_id");
        game.addPlayer(defenderInfo);
        defenderSpawn = game.ref(0, 15);
        attackerSpawn = game.ref(0, 10);
        game.addExecution(new SpawnExecution(gameID, game.player(attackerInfo.id).info(), attackerSpawn), new SpawnExecution(gameID, game.player(defenderInfo.id).info(), defenderSpawn));
        ...
        game.config().setDefaultNukeSpeed(50);
    });
...
let playerA;
let playerB;
function addPlayerToGame(playerInfo, game, tile) {
...
describe("Attack race condition with alliance requests", () => {
    beforeEach(async () => {
        ...
        const playerAInfo = new PlayerInfo("playerA", PlayerType.Human, null, "playerA_id");
        playerA = addPlayerToGame(playerAInfo, game, game.ref(0, 10));
        const playerBInfo = new PlayerInfo("playerB", PlayerType.Human, null, "playerB_id");
    ...
    it("Should not mark attacker as traitor when alliance is formed after attack starts", async () => {
        ...
        const attackExecution = new AttackExecution(null, playerA, playerB.id(), null);
        game.addExecution(attackExecution);
        // Player B counter-attacks Player A
        const counterAttackExecution = new AttackExecution(null, playerB, playerA.id(), null);
    ...
    test("Should cancel alliance requests if the recipient attacks", async () => {
        ...
        const attackExecution = new AttackExecution(null, playerB, playerA.id(), null);
        game.addExecution(attackExecution);
        // Execute a few ticks to process the attacks
        for (let i = 0; i < 5; i++) {
            game.executeNextTick();
    ...
    test("Should cancel the proper alliance request among many", async () => {
        ...
        const playerCInfo = new PlayerInfo("playerB", PlayerType.Human, null, "playerB_id");
...
describe("Attack immunity", () => {
    beforeEach(async () => {
        ...
        game.config().setSpawnImmunityDuration(immunityPhaseTicks);
        const playerAInfo = new PlayerInfo("playerA", PlayerType.Human, null, "playerA_id");
    ...
    test("Should not be able to attack nations during nation immunity phase", async () => {
        game.config().setNationSpawnImmunityDuration(immunityPhaseTicks);
