 ...
 */
const IGNORED_UNUSED_KEY_PATTERNS = [
...
function flattenTranslations(obj, file, parentKey = "", result = {}, errors = []) {
    for (const [key, value] of Object.entries(obj)) {
        ...
        if (value && typeof value === "object" && !Array.isArray(value)) {
            flattenTranslations(value, file, fullKey, result, errors);
            continue;
        }
        errors.push(`${file}:${fullKey} has invalid type ${Array.isArray(value) ? "array" : typeof value}`);
    }
    return result;
}
function listLanguageJsonFiles() {
    return fs
...
function loadTranslationFiles() {
    ...
    for (const file of listLanguageJsonFiles()) {
        ...
        files.push({
            file,
            flatMessages: flattenTranslations(parsed, file, "", {}, errors),
        });
    }
    return { files, errors };
}
...
seen = new Set()) {
    ...
    if (seen.has(realDir))
        return []; // cycle via directory symlink
    seen.add(realDir);
    const results = [];
...
function isTranslateTextCall(node) {
...
function getStaticStringFromPlus(expr) {
    const parts = getPlusOperands(expr);
    let value = "";
    for (const part of parts) {
        if (!isStringLiteralLike(part))
            return null;
        value += part.text;
    }
    return value;
}
function getDynamicPrefixFromPlus(expr) {
    const parts = getPlusOperands(expr);
    if (parts.length === 0)
        return null;
    const first = parts[0];
    if (!isStringLiteralLike(first))
        return null;
    if (parts.every((part) => isStringLiteralLike(part)))
        return null;
    return first.text || null;
}
function extractDataI18nKeys(content) {
    const keys = new Set();
    const attrRegex = /data-i18n(?:-title|-alt|-aria-label|-placeholder)?\s*=\s*["']([^"']+)["']/g;
    let match;
...
function extractTranslationKeyLikeAttrs(content) {
    const keys = new Set();
    const keyLikeAttrRegex = /\b(?:translationKey|labelKey|defaultLabelKey|disabledKey|titleKey|ariaLabelKey|placeholderKey)\s*=\s*["']([^"']+)["']/g;
    let match;
...
function isPotentialTranslationKey(key, rootKeys, enKeySet, allowBareRoot = false) {
    if (enKeySet.has(key))
        return true;
    if (allowBareRoot && rootKeys.has(key))
        return true;
    if (!key.includes("."))
        return false;
...
function isKeyNamedProperty(name) {
...
function collectFromExpression(expression, result, rootKeys, enKeySet, allowBareRoot = false) {
    if (isStringLiteralLike(expression)) {
        if (isPotentialTranslationKey(expression.text, rootKeys, enKeySet, allowBareRoot)) {
    ...
    if (ts.isParenthesizedExpression(expression)) {
        collectFromExpression(expression.expression, result, rootKeys, enKeySet, allowBareRoot);
        return;
    }
    if (ts.isConditionalExpression(expression)) {
        collectFromExpression(expression.whenTrue, result, rootKeys, enKeySet, allowBareRoot);
        collectFromExpression(expression.whenFalse, result, rootKeys, enKeySet, allowBareRoot);
        return;
    }
}
function scanTsFile(filePath, rootKeys, enKeySet) {
...
describe("Translation System", () => {
    test("metadata languages point to existing lang json and flag files", () => {
        if (!fs.existsSync(METADATA_FILE)) {
            console.log("No resources/lang/metadata.json file found. Skipping check.");
            return;
        }
        const metadata = JSON.parse(fs.readFileSync(METADATA_FILE, "utf-8"));
        if (!Array.isArray(metadata) || metadata.length === 0) {
            console.log("No language entries found in metadata.json. Skipping check.");
            return;
        }
        ...
        for (const entry of metadata) {
            ...
            if (typeof code !== "string" || code.length === 0) {
                errors.push(`metadata entry missing valid code: ${JSON.stringify(entry)}`);
                continue;
            }
            if (typeof svg !== "string" || svg.length === 0) {
                errors.push(`[${code}]: metadata svg is missing or not a non-empty string`);
                continue;
            }
        ...
        if (errors.length > 0) {
            console.error("Metadata lang or SVG file check failed:\n" + errors.join("\n"));
        }
    ...
    test("en.json keys stay in sync with source usage", () => {
        ...
        for (const key of Array.from(referencedStaticKeys).sort()) {
            if (enKeySet.has(key))
                continue;
            if (!isPotentialTranslationKey(key, rootKeys, enKeySet))
                continue;
...
//# sourceMappingURL=TranslationSystem.test.js.map