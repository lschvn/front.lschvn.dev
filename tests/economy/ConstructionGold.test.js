...
describe("Construction economy", () => {
    ...
    let player;
    let other;
    const builderInfo = new PlayerInfo("builder", PlayerType.Human, null, "builder_id");
    ...
    test("City charges gold once and no refund thereafter (allow passive income)", () => {
        ...
        expect(player.units(UnitType.City)[0].isUnderConstruction?.() ?? false).toBe(false);
    });
    test("MIRV gets more expensive with each launch", () => {
        expect(game.config().unitInfo(UnitType.MIRV).cost(game, other)).toBe(25000000n);
        ...
        game.addExecution(new NukeExecution(UnitType.MIRV, player, game.ref(10, 10)));
        ...
        expect(game.config().unitInfo(UnitType.MIRV).cost(game, other)).toBe(40000000n);
    });
});
//# sourceMappingURL=ConstructionGold.test.js.map