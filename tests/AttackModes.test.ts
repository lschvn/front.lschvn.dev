import {
  ATTACK_MODE_BALANCE,
  DEFAULT_ATTACK_MODE,
} from "../src/core/configuration/AttackModeBalance";
...
describe("Attack modes", () => {
  ...
  beforeEach(async () => {
    ...
    attacker.addTroops(20_000);
    defender.addTroops(20_000);
  });
  ...
  test("keeps simultaneous attacks with different modes separate", () => {
    ...
    expect(attacker.outgoingAttacks().map((attack) => attack.mode())).toEqual(
      expect.arrayContaining([AttackMode.Blitz, AttackMode.Siege]),
    );
  });
  ...
  test("siege suppression expires automatically after the configured duration", () => {
    ...
    executeTicks(
      game,
      ATTACK_MODE_BALANCE[AttackMode.Siege].frontlineSuppressionDuration + 2,
    );
  ...
  test("raid damages nearby economic targets and removes defender gold", () => {
    ...
    executeTicks(
      game,
      ATTACK_MODE_BALANCE[AttackMode.Raid].buildingDamageIntervalTicks * 5,
    );
