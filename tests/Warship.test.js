...
describe("Warship", () => {
    ...
    test("Warship heals only if player has port", async () => {
        ...
        const warship = player1.buildUnit(UnitType.Warship, game.ref(coastX + 1, 10), {
    ...
    test("Warship captures trade if player has port", async () => {
        ...
        game.addExecution(new WarshipExecution(player1.buildUnit(UnitType.Warship, portTile, {
            patrolTile: portTile,
        })));
        const tradeShip = player2.buildUnit(UnitType.TradeShip, game.ref(coastX + 1, 7), {
    ...
    test("Warship do not capture trade if player has no port", async () => {
        game.addExecution(new WarshipExecution(player1.buildUnit(UnitType.Warship, game.ref(coastX + 1, 11), {
            patrolTile: game.ref(coastX + 1, 11),
        })));
        const tradeShip = player2.buildUnit(UnitType.TradeShip, game.ref(coastX + 1, 11), {
    ...
    test("Warship does not target trade ships that are safe from pirates", async () => {
        ...
        game.addExecution(new WarshipExecution(warship));
        const tradeShip = player2.buildUnit(UnitType.TradeShip, game.ref(coastX + 1, 10), {
            targetUnit: player2.buildUnit(UnitType.Port, game.ref(coastX, 10), {}),
        });
    ...
    test("Warship moves to new patrol tile", async () => {
        ...
        game.addExecution(new WarshipExecution(warship));
        game.addExecution(new MoveWarshipExecution(player1, warship.id(), game.ref(coastX + 5, 15)));
        executeTicks(game, 10);
    ...
    test("Warship does not not target trade ships outside of patrol range", async () => {
        ...
        game.addExecution(new WarshipExecution(warship));
        const tradeShip = player2.buildUnit(UnitType.TradeShip, game.ref(coastX + 1, 15), {
            targetUnit: player2.buildUnit(UnitType.Port, game.ref(coastX, 10), {}),
        });
        executeTicks(game, 10);
    ...
    test("MoveWarshipExecution fails if player is not the owner", async () => {
        ...
        const warship = player1.buildUnit(UnitType.Warship, game.ref(coastX + 1, 5), {
            patrolTile: originalPatrolTile,
        });
        new MoveWarshipExecution(player2, warship.id(), game.ref(coastX + 5, 15)).init(game, 0);
    ...
    test("MoveWarshipExecution fails if warship is not active", async () => {
        ...
        warship.delete();
        new MoveWarshipExecution(player1, warship.id(), game.ref(coastX + 5, 15)).init(game, 0);
        expect(warship.patrolTile()).toBe(originalPatrolTile);
    });
    test("MoveWarshipExecution fails gracefully if warship not found", async () => {
        const exec = new MoveWarshipExecution(player1, 123, game.ref(coastX + 5, 15));
        // Verify that no error is thrown.
        exec.init(game, 0);
        expect(exec.isActive()).toBe(false);
    });
});
//# sourceMappingURL=Warship.test.js.map