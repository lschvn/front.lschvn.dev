...
let game;
let attacker;
function attackerBuildsNuke(source, target, initialize = true) {
    game.addExecution(new NukeExecution(UnitType.AtomBomb, attacker, target, source));
    if (initialize) {
...
describe("MissileSilo", () => {
    beforeEach(async () => {
        ...
        game.addExecution(new SpawnExecution(gameID, game.player(attacker_info.id).info(), game.ref(1, 1)));
    ...
    test("missilesilo should launch nuke", async () => {
        ...
        expect(attacker.units(UnitType.AtomBomb)[0].tile()).not.toBe(game.map().ref(7, 7));
        for (let i = 0; i < 5; i++) {
            game.executeNextTick();
        }
    ...
    test("missilesilo should have increased level after upgrade", async () => {
        ...
        const upgradeStructureExecution = new UpgradeStructureExecution(attacker, attacker.units(UnitType.MissileSilo)[0].id());
        game.addExecution(upgradeStructureExecution);
        executeTicks(game, 2);
        expect(attacker.units(UnitType.MissileSilo)[0].level()).toEqual(2);
    });
});
//# sourceMappingURL=MissileSilo.test.js.map