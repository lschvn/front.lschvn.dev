...
async function checkRange(mapName, unitPosX, rangeCheck, range) {
    const game = await setup(mapName, { infiniteGold: true, instantBuild: true });
    const grid = new UnitGrid(game.map());
    const player = game.addPlayer(new PlayerInfo("test_player", PlayerType.Human, null, "test_id"));
    ...
    return grid.hasUnitNearby(tileToCheck, range, UnitType.DefensePost, "test_id");
}
async function nearbyUnits(mapName, unitPosX, rangeCheck, range, unitTypes) {
    const game = await setup(mapName, { infiniteGold: true, instantBuild: true });
    const grid = new UnitGrid(game.map());
    const player = game.addPlayer(new PlayerInfo("test_player", PlayerType.Human, null, "test_id"));
    const unitTile = game.map().ref(unitPosX, 0);
...
describe("Unit Grid range tests", () => {
    ...
    describe("Is unit in range", () => {
        test.each(hasUnitCases)("on %p map, look if unit at position %p with a range of %p is in range of %p position, returns %p", async (mapName, unitPosX, range, rangeCheck, expectedResult) => {
    ...
    describe("Retrieve all units in range", () => {
        test.each(unitsInRangeCases)("on %p map, look if unit at position %p with a range of %p is in range of %p position, returns %p", async (mapName, unitPosX, range, rangeCheck, units, expectedResult) => {
            const result = await nearbyUnits(mapName, unitPosX, rangeCheck, range, units);
        ...
        test("Wrong unit type in range", async () => {
            ...
            expect(grid.nearbyUnits(tileToCheck, 10, [UnitType.Port])).toHaveLength(0);
        });
...
//# sourceMappingURL=UnitGrid.test.js.map