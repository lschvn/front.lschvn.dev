...
// ── Factory helpers ──────────────────────────────────────────────────────────
function makeUnit(tile) {
    return { tile: () => tile };
}
function makeStation(unit, cluster = null) {
    return { unit, getCluster: () => cluster };
}
function makeGame(stations = []) {
    return {
        config: () => ({
            trainGold: (rel) => TRAIN_GOLD[rel] ?? 0n,
        }),
        railNetwork: () => ({
...
function makePlayer(ownUnits, neighborList, opts = {}) {
    return {
        units: vi.fn(() => ownUnits),
        neighbors: vi.fn(() => neighborList),
        canTrade: vi.fn((n) => opts.canTrade?.(n) ?? true),
        isOnSameTeam: vi.fn((n) => opts.isOnSameTeam?.(n) ?? false),
        isAlliedWith: vi.fn((n) => opts.isAlliedWith?.(n) ?? false),
    };
}
...
function makeBehavior(game, player, random = new PseudoRandom(0)) {
...
describe("NationStructureBehavior.shouldUseConnectivityScore", () => {
    ...
    function behaviorWithNextInt(returnValue) {
    ...
    it("always returns false for Easy (randomChance = 0)", () => {
        for (const v of [0, 50, 99]) {
            ...
            expect(behavior.shouldUseConnectivityScore(Difficulty.Easy)).toBe(false);
        }
    });
    it("returns true for Medium when nextInt < 60", () => {
        const { behavior } = behaviorWithNextInt(59);
        expect(behavior.shouldUseConnectivityScore(Difficulty.Medium)).toBe(true);
    });
    it("returns false for Medium when nextInt === 60 (boundary)", () => {
        const { behavior } = behaviorWithNextInt(60);
        expect(behavior.shouldUseConnectivityScore(Difficulty.Medium)).toBe(false);
    });
    it("returns true for Hard when nextInt < 75", () => {
        const { behavior } = behaviorWithNextInt(74);
        expect(behavior.shouldUseConnectivityScore(Difficulty.Hard)).toBe(true);
    });
    it("returns false for Hard when nextInt === 75 (boundary)", () => {
        const { behavior } = behaviorWithNextInt(75);
        expect(behavior.shouldUseConnectivityScore(Difficulty.Hard)).toBe(false);
    });
    it("always returns true for Impossible (randomChance = 100)", () => {
        for (const v of [0, 50, 99]) {
            const { behavior, random } = behaviorWithNextInt(v);
            vi.spyOn(random, "nextInt").mockReturnValue(v);
            expect(behavior.shouldUseConnectivityScore(Difficulty.Impossible)).toBe(true);
        }
    });
});
...
describe("NationStructureBehavior.buildReachableStations", () => {
    ...
    it("includes own registered units with self weight and correct cluster", () => {
        ...
        const result = behavior.buildReachableStations();
    ...
    it("collects own and neighbor units together", () => {
        ...
        const behavior = makeBehavior(makeGame([ownStation, neighborStation]), player);
        const result = behavior.buildReachableStations();
...
describe("NationStructureBehavior.getOrBuildReachableStations", () => {
    ...
    beforeEach(() => {
        ...
        buildSpy = vi.spyOn(behavior, "buildReachableStations");
    });
    afterEach(() => {
        vi.restoreAllMocks();
    });
    it("calls buildReachableStations exactly once on first access", () => {
        behavior.getOrBuildReachableStations();
        expect(buildSpy).toHaveBeenCalledTimes(1);
    });
    it("returns the same array instance on repeated calls", () => {
        const first = behavior.getOrBuildReachableStations();
        const second = behavior.getOrBuildReachableStations();
    ...
    it("rebuilds after the cache is reset to null", () => {
        behavior.getOrBuildReachableStations();
        behavior.reachableStationsCache = null;
        behavior.getOrBuildReachableStations();
        expect(buildSpy).toHaveBeenCalledTimes(2);
    });
});
//# sourceMappingURL=NationStructureBehavior.test.js.map