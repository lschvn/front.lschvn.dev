...
describe("Ai Attack Behavior", () => {
    ...
    async function setupTestEnvironment() {
        ...
        const behavior = new AiAttackBehavior(new PseudoRandom(42), testGame, testBot, 0.5, 0.5, 0.2);
        return { testGame, testBot, testHuman, behavior };
    }
    // Helper functions for tile assignment
    function assignAlternatingLandTiles(game, players, totalTiles) {
        let assigned = 0;
        game.map().forEachTile((tile) => {
            if (assigned >= totalTiles)
                return;
    ...
    test("nation cannot attack allied player", () => {
        // Create nation
        const nationInfo = new PlayerInfo("nation_test", PlayerType.Nation, null, "nation_test");
        ...
        const nationBehavior = new AiAttackBehavior(new PseudoRandom(42), game, nation, 0.5, 0.5, 0.2);
...
//# sourceMappingURL=AiAttackBehavior.test.js.map