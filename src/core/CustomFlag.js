...
export function renderPlayerFlag(flag, target, cosmetics = undefined) {
    ...
    for (const { layerKey, colorKey } of layers) {
        ...
        const isSpecial = !colorValue.startsWith("#") &&
        ...
        if (isSpecial) {
            ...
            if (!duration)
                console.warn(`No animation duration for: ${colorValue}`);
...
//# sourceMappingURL=CustomFlag.js.map