...
export class AiAttackBehavior {
  ...
  private attackWithRandomBoat(borderingEnemies: Player[] = []) {
    ...
    const mode = this.chooseAttackMode(
      targetOwner.isPlayer() ? targetOwner : this.game.terraNullius(),
      true,
    );
  ...
  private findSupplyCutTarget(borderingEnemies: Player[]): Player | null {
    const candidates = borderingEnemies
      ...
      .filter(
        ({ enemy, score }) =>
          score > 0 && enemy.troops() < this.player.troops() * 1.4,
      )
      .sort((a, b) => {
        if (a.score !== b.score) {
  ...
  sendAttack(target: Player | TerraNullius, force = false) {
    ...
    const mode = this.chooseAttackMode(
      target,
      !this.player.sharesBorderWith(target) && target.isPlayer(),
    );
  ...
  private sendLandAttack(target: Player | TerraNullius, mode: AttackMode) {
    ...
    troops = Math.min(
      this.player.troops(),
      troops * ATTACK_MODE_BALANCE[mode].troopCommitmentMultiplier,
    );
    if (frontReadiness === 0) {
      troops *= 0.8;
  ...
  private sendBoatAttack(target: Player, mode: AttackMode) {
    if (this.isLogisticallyOverextended()) {
      return;
    }
    ...
    this.game.addExecution(
      new TransportShipExecution(this.player, closest.y, troops, mode),
    );
  }

  private chooseAttackMode(
    target: Player | TerraNullius,
    isBoatAttack: boolean,
  ): AttackMode {
    if (!target.isPlayer()) {
  ...
  private localNumericalAdvantage(target: Player): number {
    const myFront = Array.from(this.player.borderTiles()).filter((tile) =>
      this.game
        .neighbors(tile)
        .some((neighbor) => this.game.owner(neighbor) === target),
    ).length;
    const enemyFront = Array.from(target.borderTiles()).filter((tile) =>
      this.game
        .neighbors(tile)
        .some((neighbor) => this.game.owner(neighbor) === this.player),
    ).length;
    const myLocalTroops =
      (this.player.troops() / Math.max(1, this.player.numTilesOwned())) *
      Math.max(1, myFront);
    const enemyLocalTroops =
      (target.troops() / Math.max(1, target.numTilesOwned())) *
      Math.max(1, enemyFront);
    return myLocalTroops / Math.max(1, enemyLocalTroops);
  }

  private isFortifiedFront(target: Player): boolean {
    const contestedTiles = Array.from(target.borderTiles()).filter((tile) =>
      this.game
        .neighbors(tile)
        .some((neighbor) => this.game.owner(neighbor) === this.player),
    );
    ...
    const defendedFront = contestedTiles.some((tile) =>
    ...
    const roughTerrain = contestedTiles.filter((tile) => {
      ...
      return (
        terrain === TerrainType.Mountain || terrain === TerrainType.Highland
      );
    });
    return roughTerrain.length >= Math.ceil(contestedTiles.length / 2);
  }

  private hasExposedRaidTargets(
    target: Player,
    isBoatAttack: boolean,
  ): boolean {
    return target.units(...RAID_PRIORITY_UNIT_TYPES).some((unit) => {
      ...
      return Array.from(this.player.borderTiles()).some(
        (tile) => this.game.manhattanDist(tile, unit.tile()) <= 12,
      );
    });
  }
  ...
  private isLogisticallyOverextended(): boolean {
    const summary = this.player.supplySummary();
    const total =
      summary.suppliedTiles + summary.strainedTiles + summary.isolatedTiles;
    if (total <= 0) {
      return false;
    }
  ...
  private frontSupplyReadiness(target: Player | TerraNullius): number {
  ...
  private supplyPressureScore(target: Player): number {
    let score = 0;
