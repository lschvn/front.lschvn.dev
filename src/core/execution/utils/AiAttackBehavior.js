import { ATTACK_MODE_BALANCE, DEFAULT_ATTACK_MODE, RAID_PRIORITY_UNIT_TYPES, } from "../../configuration/AttackModeBalance";
import { AttackMode, Difficulty, GameMode, HumansVsNations, PlayerType, Relation, Structures, SupplyState, TerrainType, UnitType, } from "../../game/Game";
import { canBuildTransportShip } from "../../game/TransportShipUtils";
import { assertNever, boundingBoxCenter, calculateBoundingBoxCenter, } from "../../Util";
import { AttackExecution } from "../AttackExecution";
import { DonateTroopsExecution } from "../DonateTroopExecution";
import { EMOJI_ASSIST_ACCEPT, EMOJI_ASSIST_RELATION_TOO_LOW, EMOJI_ASSIST_TARGET_ALLY, EMOJI_ASSIST_TARGET_ME, } from "../nation/NationEmojiBehavior";
import { TransportShipExecution } from "../TransportShipExecution";
import { closestTwoTiles } from "../Util";
export class AiAttackBehavior {
    constructor(random, game, player, triggerRatio, reserveRatio, expandRatio, allianceBehavior, emojiBehavior) {
        this.random = random;
        this.game = game;
        this.player = player;
    ...
    maybeAttack() {
        ...
        const borderingFriends = borderingPlayers.filter((o) => this.player?.isFriendly(o) === true);
        const borderingEnemies = borderingPlayers.filter((o) => this.player?.isFriendly(o) === false);
        // Attack TerraNullius but not nuked territory
        const hasNonNukedTerraNullius = border.some((t) => !this.game.hasOwner(t) && !this.game.hasFallout(t));
    ...
    attackWithRandomBoat(borderingEnemies = []) {
        if (this.player === null)
            throw new Error("not initialized");
        if (this.isLogisticallyOverextended()) {
            return;
        }
        // Check if we've already sent out the maximum number of transport ships
        if (this.player.unitCount(UnitType.TransportShip) >=
        ...
        const targetOwner = this.game.owner(dst);
        const mode = this.chooseAttackMode(targetOwner.isPlayer() ? targetOwner : this.game.terraNullius(), true);
        this.game.addExecution(new TransportShipExecution(this.player, dst, this.player.troops() / 5, mode));
        return;
    }
    findRandomBoatTarget(tile, borderingEnemies, highInterestOnly = false) {
        if (this.player === null)
            throw new Error("not initialized");
    ...
    attackBestTarget(borderingFriends, borderingEnemies) {
        ...
        const strategies = this.getAttackStrategies(borderingFriends, borderingEnemies);
    ...
    getAttackStrategies(borderingFriends, borderingEnemies) {
        ...
        const bots = () => this.attackBots();
        const assist = () => this.assistAllies();
        const traitor = () => {
        ...
        const afk = () => {
            ...
            const afk = borderingEnemies.find((enemy) => enemy.isDisconnected() && enemy.troops() < this.player.troops() * 3);
            if (afk) {
                this.sendAttack(afk);
                return true;
            }
        ...
        const betray = () => this.maybeBetrayAndAttack(borderingFriends, borderingEnemies);
        const nuked = () => {
        ...
        const supplyCut = () => {
            const target = this.findSupplyCutTarget(borderingEnemies);
            if (target) {
        ...
        const hated = () => {
            for (const relation of this.player.allRelationsSorted()) {
                if (relation.relation !== Relation.Hostile)
                    continue;
                const other = relation.player;
                if (this.player.isFriendly(other))
                    continue;
                if (other.troops() > this.player.troops() * 3)
                    continue;
        ...
        const donate = () => this.donateTroops();
        ...
        switch (difficulty) {
            ...
            case Difficulty.Medium:
                // prettier-ignore
                return [bots, nuked, retaliate, assist, betray, hated, afk, traitor, supplyCut, weakest, island, donate];
            case Difficulty.Hard:
                // prettier-ignore
                return [bots, retaliate, assist, betray, nuked, traitor, afk, supplyCut, hated, veryWeak, victim, weakest, island, donate];
            case Difficulty.Impossible:
                // prettier-ignore
                return [retaliate, bots, veryWeak, assist, traitor, afk, supplyCut, betray, victim, nuked, hated, weakest, island, donate];
            default:
    ...
    findIncomingAttackPlayer() {
        ...
        if (this.player.type() !== PlayerType.Bot) {
            incomingAttacks = incomingAttacks.filter((attack) => attack.attacker().type() !== PlayerType.Bot);
        }
        let largestAttack = 0;
        let largestAttacker;
    ...
    attackBots() {
        ...
        const ownsStructures = (p) => p.units().some((u) => Structures.has(u.type()));
    ...
    assistAllies() {
        if (this.emojiBehavior === undefined)
            throw new Error("not initialized");
    ...
    findTraitor(borderingEnemies) {
        ...
        return (borderingEnemies.find((enemy) => enemy.isTraitor() && enemy.troops() < this.player.troops() * 1.2) ?? null);
    }
    maybeBetrayAndAttack(borderingFriends, borderingEnemies) {
        if (this.allianceBehavior === undefined)
            throw new Error("not initialized");
        if (borderingFriends.length > 0) {
            for (const friend of borderingFriends) {
                if (this.allianceBehavior.maybeBetray(friend, borderingFriends.length + borderingEnemies.length)) {
    ...
    findVictim(borderingEnemies) {
        ...
        return (borderingEnemies.find((enemy) => {
            if (enemy.troops() > this.player.troops() * 1.2)
                return false;
    ...
    findSupplyCutTarget(borderingEnemies) {
        const candidates = borderingEnemies
            .map((enemy) => ({
            enemy,
            score: this.supplyPressureScore(enemy),
        }))
            .filter(({ enemy, score }) => score > 0 && enemy.troops() < this.player.troops() * 1.4)
            .sort((a, b) => {
            if (a.score !== b.score) {
    ...
    findVeryWeakEnemy(borderingEnemies) {
        const veryWeakEnemies = borderingEnemies.filter((enemy) => {
            ...
            return (enemy.troops() < enemyMaxTroops * 0.15 &&
    ...
    findNearestIslandEnemy() {
        ...
        const sortedPlayers = filteredPlayers
            ...
            const filteredPlayerCenterTile = this.game.ref(filteredPlayerCenter.x, filteredPlayerCenter.y);
            const distance = this.game.manhattanDist(playerCenterTile, filteredPlayerCenterTile);
        ...
        for (const entry of sortedPlayers) {
            const closest = closestTwoTiles(this.game, Array.from(this.player.borderTiles()).filter((t) => this.game.isOceanShore(t)), Array.from(entry.player.borderTiles()).filter((t) => this.game.isOceanShore(t)));
            if (closest === null)
                continue;
    ...
    attackRandomTarget() {
        ...
        for (const neighbor of this.random.shuffleArray(neighbors)) {
            ...
            if (this.player.isFriendly(neighbor))
                continue;
            if (neighbor.type() === PlayerType.Nation ||
    ...
    getNeighborTraitorToAttack() {
        const traitors = this.player
            .neighbors()
            .filter((n) => n.isPlayer() && this.player.isFriendly(n) === false && n.isTraitor());
        return traitors.length > 0 ? this.random.randElement(traitors) : null;
    }
    forceSendAttack(target) {
        const mode = this.chooseAttackMode(target, false);
        this.game.addExecution(new AttackExecution(this.player.troops() / 2, this.player, target.isPlayer() ? target.id() : this.game.terraNullius().id(), null, true, mode));
    }
    sendAttack(target, force = false) {
        if (!force && !this.shouldAttack(target))
            return;
        const mode = this.chooseAttackMode(target, !this.player.sharesBorderWith(target) && target.isPlayer());
        if (this.player.sharesBorderWith(target)) {
            this.sendLandAttack(target, mode);
        }
        else if (target.isPlayer()) {
            this.sendBoatAttack(target, mode);
        }
    }
    shouldAttack(other) {
        if (
    ...
    sendLandAttack(target, mode) {
        const frontReadiness = this.frontSupplyReadiness(target);
        if (frontReadiness < 0) {
            return;
        }
        const maxTroops = this.game.config().maxTroops(this.player);
        const botWithStructures = target.isPlayer() &&
        ...
        if (target.isPlayer() &&
            target.type() === PlayerType.Bot &&
            this.player.type() !== PlayerType.Bot) {
            troops = this.calculateBotAttackTroops(target, this.player.troops() - targetTroops - this.botAttackTroopsSent);
        }
        else {
        ...
        troops = Math.min(this.player.troops(), troops * ATTACK_MODE_BALANCE[mode].troopCommitmentMultiplier);
        if (frontReadiness === 0) {
            troops *= 0.8;
        }
        else if (frontReadiness === 1) {
            troops *= 0.92;
        }
        if (troops < 1) {
            return;
        ...
        this.game.addExecution(new AttackExecution(troops, this.player, target.isPlayer() ? target.id() : this.game.terraNullius().id(), null, true, mode));
    }
    sendBoatAttack(target, mode) {
        if (this.isLogisticallyOverextended()) {
            return;
        }
        const closest = closestTwoTiles(this.game, Array.from(this.player.borderTiles()).filter((t) => this.game.isOceanShore(t)), Array.from(target.borderTiles()).filter((t) => this.game.isOceanShore(t)));
        if (closest === null) {
            return;
        }
        if (!canBuildTransportShip(this.game, this.player, closest.y)) {
        ...
        this.game.addExecution(new TransportShipExecution(this.player, closest.y, troops, mode));
    }
    chooseAttackMode(target, isBoatAttack) {
        if (!target.isPlayer()) {
            return DEFAULT_ATTACK_MODE;
        }
        if (this.hasExposedRaidTargets(target, isBoatAttack)) {
            return AttackMode.Raid;
        }
        if (this.isFortifiedFront(target)) {
            return AttackMode.Siege;
        }
        if (this.localNumericalAdvantage(target) >= 1.4) {
            return AttackMode.Blitz;
        }
        return DEFAULT_ATTACK_MODE;
    }
    localNumericalAdvantage(target) {
        const myFront = Array.from(this.player.borderTiles()).filter((tile) => this.game
            .neighbors(tile)
            .some((neighbor) => this.game.owner(neighbor) === target)).length;
        const enemyFront = Array.from(target.borderTiles()).filter((tile) => this.game
            .neighbors(tile)
            .some((neighbor) => this.game.owner(neighbor) === this.player)).length;
        const myLocalTroops = (this.player.troops() / Math.max(1, this.player.numTilesOwned())) *
            Math.max(1, myFront);
        const enemyLocalTroops = (target.troops() / Math.max(1, target.numTilesOwned())) *
            Math.max(1, enemyFront);
        return myLocalTroops / Math.max(1, enemyLocalTroops);
    }
    isFortifiedFront(target) {
        const contestedTiles = Array.from(target.borderTiles()).filter((tile) => this.game
            .neighbors(tile)
            .some((neighbor) => this.game.owner(neighbor) === this.player));
        if (contestedTiles.length === 0) {
            return false;
        }
        const defendedFront = contestedTiles.some((tile) => this.game.hasUnitNearby(tile, this.game.config().defensePostRange(), UnitType.DefensePost, target.id()));
        if (defendedFront) {
            return true;
        }
        const roughTerrain = contestedTiles.filter((tile) => {
            const terrain = this.game.terrainType(tile);
            return (terrain === TerrainType.Mountain || terrain === TerrainType.Highland);
        });
        return roughTerrain.length >= Math.ceil(contestedTiles.length / 2);
    }
    hasExposedRaidTargets(target, isBoatAttack) {
        return target.units(...RAID_PRIORITY_UNIT_TYPES).some((unit) => {
            if (unit.type() === UnitType.Port ||
                unit.type() === UnitType.TradeShip ||
                unit.type() === UnitType.TransportShip) {
                return true;
            }
            if (isBoatAttack) {
                return this.game.isOceanShore(unit.tile());
            }
            return Array.from(this.player.borderTiles()).some((tile) => this.game.manhattanDist(tile, unit.tile()) <= 12);
        });
    }
    ...
    isLogisticallyOverextended() {
        const summary = this.player.supplySummary();
        const total = summary.suppliedTiles + summary.strainedTiles + summary.isolatedTiles;
        if (total <= 0) {
            return false;
        }
        if (summary.depletedIsolatedTiles > 0) {
            return true;
        }
        return summary.isolatedTiles / total >= 0.18;
    }
    frontSupplyReadiness(target) {
        let best = Number.NEGATIVE_INFINITY;
        for (const tile of this.player.borderTiles()) {
            if (!this.game
                .neighbors(tile)
                .some((neighbor) => this.game.owner(neighbor) === target)) {
                continue;
            }
            const state = this.game.supplyState(tile);
            let score = 2;
            if (state === SupplyState.Strained) {
                score = 1;
            }
            else if (state === SupplyState.Isolated) {
                score = this.game.isSupplyReserveDepleted(tile) ? -1 : 0;
            }
            best = Math.max(best, score);
        }
        return best === Number.NEGATIVE_INFINITY ? 2 : best;
    }
    supplyPressureScore(target) {
        let score = 0;
        for (const tile of target.borderTiles()) {
            ...
            if (state === SupplyState.Strained) {
                ...
                score += this.game.isSupplyReserveDepleted(tile) ? 3 : 2;
            }
        }
        return score;
    }
    donateTroops() {
        ...
        const teammates = this.game
            ...
            .filter((p) => p.incomingAttacks().length > 0 || p.outgoingAttacks().length > 0);
        ...
        this.game.addExecution(new DonateTroopsExecution(this.player, selectedTeammate.id(), availableTroops));
        return true;
    }
}
//# sourceMappingURL=AiAttackBehavior.js.map