...
export class WarshipExecution {
    ...
    init(mg, ticks) {
        ...
        if (isUnit(this.input)) {
            ...
            const spawn = this.input.owner.canBuild(UnitType.Warship, this.input.patrolTile);
            if (spawn === false) {
                console.warn(`Failed to spawn warship for ${this.input.owner.name()} at ${this.input.patrolTile}`);
                return;
            }
            this.warship = this.input.owner.buildUnit(UnitType.Warship, spawn, this.input);
        }
    }
    tick(ticks) {
    ...
    findTargetUnit() {
        ...
        const ships = mg.nearbyUnits(this.warship.tile(), config.warshipTargettingRange(), [
        ...
        for (const { unit, distSquared } of ships) {
            ...
            if (type === UnitType.Submarine && !unit.targetable()) {
                continue;
            }
            if (type === UnitType.TradeShip) {
                ...
                if (mg.euclideanDistSquared(patrolTile, unit.tile()) > patrolRangeSquared) {
            ...
            const typePriority = type === UnitType.TransportShip
                ? 0
    ...
    shootTarget() {
        ...
        if (this.mg.ticks() - this.lastShellAttack > shellAttackRate) {
            ...
            this.mg.addExecution(new ShellExecution(this.warship.tile(), this.warship.owner(), this.warship, this.warship.targetUnit()));
    ...
    huntDownTradeShip() {
        for (let i = 0; i < 2; i++) {
            ...
            const result = this.pathfinder.next(this.warship.tile(), this.warship.targetUnit().tile(), 5);
    ...
    patrol() {
        ...
        const result = this.pathfinder.next(this.warship.tile(), this.warship.targetTile());
        switch (result.status) {
            case PathStatus.COMPLETE:
...
//# sourceMappingURL=WarshipExecution.js.map