import { AA_BATTERY_BOMBER_DAMAGE, AA_BATTERY_FIGHTER_DAMAGE, AA_OVERLAP_PENALTY_PER_EXTRA_BATTERY, AIR_DEFENSE_HIGH_VALUE_TARGETS, AIR_DEFENSE_PRIORITY_STRUCTURE_RADIUS, AIR_DEFENSE_STRIKE_MISSIONS, RADAR_STATION_AA_DAMAGE_MULTIPLIER, RADAR_STATION_LOCKON_REDUCTION, } from "../configuration/AntiAirBalance";
import { AirMissionPhase, AirMissionType, AirUnits, UnitType, } from "../game/Game";
export class AABatteryExecution {
    constructor(battery) {
    ...
    tick(ticks) {
        ...
        const radarSupported = this.mg.hasRadarCoverage(target.tile(), this.battery.owner());
        const lockTicks = Math.max(1, AA_BATTERY_LOCKON_TICKS -
    ...
    fireAtTarget(target, radarSupported) {
        this.battery.launch();
        const baseDamage = target.type() === UnitType.BomberSquadron
            ? AA_BATTERY_BOMBER_DAMAGE
            : AA_BATTERY_FIGHTER_DAMAGE;
        const overlappingCoverage = this.coverageCount(target);
        const overlapDivisor = 1 +
        ...
        const radarMultiplier = radarSupported
            ...
            : 1;
        const damage = Math.max(1, Math.round((baseDamage * radarMultiplier) / overlapDivisor));
    ...
    selectTarget() {
        const candidates = this.mg.nearbyUnits(this.battery.tile(), this.mg.config().aaBatteryRange(), AirUnits.types, ({ unit }) => unit.owner() !== this.battery.owner() &&
    ...
    coverageCount(target) {
        return this.mg.nearbyUnits(target.tile(), this.mg.config().aaBatteryRange(), UnitType.AABattery, ({ unit }) => unit.owner() === this.battery.owner() &&
            unit.isActive() &&
            !unit.isUnderConstruction() &&
            !unit.isMarkedForDeletion(), true).length;
    }
    targetPriority(unit) {
    ...
    threatensHighValueStructure(unit) {
        return this.mg.anyUnitNearby(unit.tile(), AIR_DEFENSE_PRIORITY_STRUCTURE_RADIUS, AIR_DEFENSE_HIGH_VALUE_TARGETS, (structure) => structure.owner() === this.battery.owner() &&
            structure.isActive() &&
...
//# sourceMappingURL=AABatteryExecution.js.map