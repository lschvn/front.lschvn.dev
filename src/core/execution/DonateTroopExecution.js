...
import { EMOJI_DONATION_TOO_SMALL, EMOJI_LOVE, } from "./nation/NationEmojiBehavior";
export class DonateTroopsExecution {
    ...
    init(mg, ticks) {
        ...
        if (!mg.hasPlayer(this.recipientID)) {
            console.warn(`DonateTroopExecution recipient ${this.recipientID} not found`);
            this.active = false;
            return;
        }
    ...
    tick(ticks) {
        ...
        if (this.sender.canDonateTroops(this.recipient) &&
            ...
            if (this.recipient.type() === PlayerType.Nation &&
                ...
                this.mg.addExecution(new EmojiExecution(this.recipient, this.sender.id(), this.random.randElement(this.troops >= minTroops ? EMOJI_LOVE : EMOJI_DONATION_TOO_SMALL)));
            }
        }
        else {
    ...
    getMinTroopsForRelationUpdate() {
        const { difficulty } = this.mg.config().gameConfig();
        const recipientMaxTroops = this.mg.config().maxTroops(this.recipient);
        switch (difficulty) {
            // ~7.7k - ~9.1k troops (for 100k troops)
            case Difficulty.Easy:
                return this.random.nextInt(recipientMaxTroops / 13, recipientMaxTroops / 11);
            // ~9.1k - ~11.1k troops (for 100k troops)
            case Difficulty.Medium:
                return this.random.nextInt(recipientMaxTroops / 11, recipientMaxTroops / 9);
            // ~11.1k - ~14.3k troops (for 100k troops)
            case Difficulty.Hard:
                return this.random.nextInt(recipientMaxTroops / 9, recipientMaxTroops / 7);
            // ~14.3k - ~20k troops (for 100k troops)
            case Difficulty.Impossible:
                return this.random.nextInt(recipientMaxTroops / 7, recipientMaxTroops / 5);
            default:
...
//# sourceMappingURL=DonateTroopExecution.js.map