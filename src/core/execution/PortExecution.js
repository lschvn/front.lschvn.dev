import { SUPPLY_PORT_DEPLETED_TRADE_MULTIPLIER, SUPPLY_PORT_ISOLATED_TRADE_MULTIPLIER, SUPPLY_PORT_STRAINED_TRADE_MULTIPLIER, } from "../configuration/SupplyBalance";
import { SupplyState, UnitType } from "../game/Game";
import { PseudoRandom } from "../PseudoRandom";
import { TradeShipExecution } from "./TradeShipExecution";
import { TrainStationExecution } from "./TrainStationExecution";
export class PortExecution {
    constructor(port) {
        this.active = true;
    ...
    createStation() {
        const nearbyFactory = this.mg.hasUnitNearby(this.port.tile(), this.mg.config().trainStationMaxRange(), UnitType.Factory);
        if (nearbyFactory) {
            this.mg.addExecution(new TrainStationExecution(this.port));
        }
    }
    tradeSpawnMultiplier() {
        const tile = this.port.tile();
        const state = this.mg.supplyState(tile);
        if (state === SupplyState.Strained) {
            return SUPPLY_PORT_STRAINED_TRADE_MULTIPLIER;
        }
        if (state !== SupplyState.Isolated) {
            return 1;
        }
        return this.mg.isSupplyReserveDepleted(tile)
            ? SUPPLY_PORT_DEPLETED_TRADE_MULTIPLIER
            : SUPPLY_PORT_ISOLATED_TRADE_MULTIPLIER;
    }
    ...
    tradingPorts() {
        const ports = this.mg
            ...
            .sort((p1, p2) => {
            return (this.mg.manhattanDist(this.port.tile(), p1.tile()) -
        ...
        for (const [i, otherPort] of ports.entries()) {
            ...
            const tooClose = this.mg.manhattanDist(this.port.tile(), otherPort.tile()) <
...
//# sourceMappingURL=PortExecution.js.map