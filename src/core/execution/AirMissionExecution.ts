import {
  BOMBER_MISSION_DURATION,
  BOMBER_REARM_TICKS,
  BOMBER_TICKS_PER_MOVE,
  CLOSE_AIR_SUPPORT_RADIUS,
  FIGHTER_ESCORT_DAMAGE_REDUCTION,
  FIGHTER_INTERCEPTION_DAMAGE,
  FIGHTER_INTERCEPTION_RANGE,
  FIGHTER_MISSION_DURATION,
  FIGHTER_REARM_TICKS,
  FIGHTER_TICKS_PER_MOVE,
  PORT_STRIKE_RADIUS,
  PORT_STRIKE_SHIP_DAMAGE,
  PORT_STRIKE_STRUCTURE_DAMAGE,
  PORT_STRIKE_TARGETS,
  STRATEGIC_BOMBING_STRUCTURE_DAMAGE,
  STRATEGIC_BOMBING_TARGET_RADIUS,
  STRATEGIC_BOMBING_TARGETS,
} from "../configuration/AirBalance";
import {
  RADAR_STATION_FIGHTER_DAMAGE_BONUS,
  RADAR_STATION_FIGHTER_RANGE_BONUS,
...
export class AirMissionExecution implements Execution {
  private active = true;
  private mg!: Game;

  constructor(private readonly squadron: Unit) {}
  ...
  tick(ticks: number): void {
    ...
    if (
      this.mg.euclideanDistSquared(airbase.tile(), targetTile) >
  ...
  private handleOnStation(ticks: number) {
  ...
  private resolveMissionArrival() {
    ...
    switch (missionType) {
      ...
      case AirMissionType.CloseAirSupport:
        this.mg.addAirSupportZone(
          this.squadron.owner(),
          targetTile,
          CLOSE_AIR_SUPPORT_RADIUS,
          BOMBER_MISSION_DURATION[AirMissionType.CloseAirSupport],
        );
        return;
  ...
  private resolveStrategicBombing(targetTile: TileRef) {
    const targets = this.mg.nearbyUnits(
      targetTile,
      STRATEGIC_BOMBING_TARGET_RADIUS,
      STRATEGIC_BOMBING_TARGETS,
      ({ unit }) =>
        unit.owner() !== this.squadron.owner() &&
        this.squadron.owner().canAttackPlayer(unit.owner(), true),
      true,
    );
    if (targets.length === 0) {
      return;
    ...
    const targetPriority = new Map(
      STRATEGIC_BOMBING_TARGETS.map((type, index) => [type, index]),
    );
    targets.sort((a, b) => {
    ...
    this.damageStructureTarget(
      targets[0].unit,
      STRATEGIC_BOMBING_STRUCTURE_DAMAGE,
    );
  }

  private resolvePortStrike(targetTile: TileRef) {
    const targets = this.mg.nearbyUnits(
      targetTile,
      PORT_STRIKE_RADIUS,
      PORT_STRIKE_TARGETS,
  ...
  private damageStructureTarget(target: Unit, damage: number) {
  ...
  private maybeInterceptBomber() {
    if (this.squadron.type() !== UnitType.BomberSquadron || !this.squadron.isActive()) {
      return;
    }

    const interceptors = this.mg.nearbyUnits(
      this.squadron.tile(),
      FIGHTER_INTERCEPTION_RANGE + RADAR_STATION_FIGHTER_RANGE_BONUS,
    ...
    const escortingFriendlyFighters = this.mg.nearbyUnits(
      this.squadron.tile(),
      FIGHTER_INTERCEPTION_RANGE,
      UnitType.FighterSquadron,
      ({ unit }) =>
    ...
    let damage =
      FIGHTER_INTERCEPTION_DAMAGE * Math.min(2, interceptors.length);
  ...
  private beginReturn(airbase: Unit, ticks: number) {
  ...
  private missionDuration(): number {
    ...
    if (this.squadron.type() === UnitType.FighterSquadron) {
      return FIGHTER_MISSION_DURATION[
        missionType as
    ...
    return BOMBER_MISSION_DURATION[
      missionType as
  ...
  private moveTowards(target: TileRef): boolean {
    if (this.squadron.tile() === target) {
      return true;
    }

    const path = new AirPathFinder(this.mg).findPath(
      this.squadron.tile(),
      target,
    );
    ...
    const speed =
      this.squadron.type() === UnitType.FighterSquadron
        ? FIGHTER_TICKS_PER_MOVE
        : BOMBER_TICKS_PER_MOVE;
    if (speed > 1) {
      return false;
  ...
  private findAirbase(): Unit | undefined {
