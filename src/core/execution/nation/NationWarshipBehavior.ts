...
export class NationWarshipBehavior {
  ...
  maybeSpawnWarship(): boolean {
    ...
    if (
      ...
      const targetTile =
        this.selectPatrolTile() ?? this.warshipSpawnTile(port.tile());
      if (targetTile === null) {
        return false;
      }
  ...
  private selectPatrolTile(): TileRef | null {
    const threatenedOwnPorts = this.player
      .units(UnitType.Port)
      .filter(
        (port) => this.game.supplyState(port.tile()) !== SupplyState.Supplied,
      )
      .sort(
        (a, b) =>
          this.portPriorityScore(b.tile()) - this.portPriorityScore(a.tile()),
      );
  ...
  private portPatrolTile(portTile: TileRef): TileRef | null {
  ...
  private portPriorityScore(portTile: TileRef): number {
