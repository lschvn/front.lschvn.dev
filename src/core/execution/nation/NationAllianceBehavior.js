import { Difficulty, GameMode, PlayerType, Relation, } from "../../game/Game";
import { assertNever } from "../../Util";
import { AllianceExtensionExecution } from "../alliance/AllianceExtensionExecution";
import { AllianceRequestExecution } from "../alliance/AllianceRequestExecution";
import { EMOJI_CONFUSED, EMOJI_HANDSHAKE, EMOJI_LOVE, EMOJI_SCARED_OF_THREAT, } from "./NationEmojiBehavior";
export class NationAllianceBehavior {
    ...
    handleAllianceExtensionRequests() {
        for (const alliance of this.player.alliances()) {
            ...
            if (!this.getAllianceDecision(human, true))
                continue;
    ...
    maybeSendAllianceRequests(borderingEnemies) {
        ...
        const isAcceptablePlayerType = (p) => (p.type() === PlayerType.Bot &&
        ...
        for (const enemy of borderingEnemies) {
            if (this.random.chance(30) &&
                ...
                this.game.addExecution(new AllianceRequestExecution(this.player, enemy.id()));
            }
        }
    }
    ...
    isEarlygame() {
        ...
        switch (difficulty) {
            case Difficulty.Easy:
                ...
                return (this.game.ticks() < 3000 + spawnTicks &&
                    this.random.nextInt(0, 100) >= 10);
            case Difficulty.Medium:
                // On medium, accept 70% in the first 3 minutes
                return (this.game.ticks() < 1800 + spawnTicks &&
            ...
            case Difficulty.Impossible:
                ...
                return (this.game.ticks() < 600 + spawnTicks &&
    ...
    isAlliancePartnerThreat(otherPlayer) {
        ...
        switch (difficulty) {
            ...
            case Difficulty.Impossible: {
                ...
                const otherHasMoreTroops = otherPlayer.troops() > this.player.troops() * 1.5;
                const otherHasMoreMaxTroops = otherPlayer.troops() > this.player.troops() &&
                    this.game.config().maxTroops(otherPlayer) >
                        this.game.config().maxTroops(this.player) * 1.5;
                const otherHasMoreTiles = otherPlayer.troops() > this.player.troops() &&
    ...
    maybeBetray(otherPlayer, borderingPlayerCount) {
        if (!this.player.isAlliedWith(otherPlayer))
            return false;
        const { difficulty } = this.game.config().gameConfig();
        ...
        if (difficulty !== Difficulty.Easy && difficulty !== Difficulty.Medium) {
            ...
            const otherPlayerOutgoingTroops = otherPlayer
                .outgoingAttacks()
                .reduce((sum, attack) => sum + attack.troops(), 0);
            if (otherPlayer.troops() + otherPlayerOutgoingTroops <
...
//# sourceMappingURL=NationAllianceBehavior.js.map