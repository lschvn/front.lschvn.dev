import { Difficulty, PlayerType, Relation, Structures, SupplyState, UnitType, } from "../../game/Game";
...
function getStructureRatios(difficulty) {
    return {
        ...
        [UnitType.Airbase]: {
            ratioPerCity: 0.2,
            perceivedCostIncreasePerOwned: 1.25,
        },
...
export class NationStructureBehavior {
    ...
    handleStructures() {
        ...
        const nukesEnabled = !config.isUnitDisabled(UnitType.AtomBomb) ||
    ...
    hasCoastalTiles() {
        for (const tile of this.player.borderTiles()) {
            if (this.game.isOceanShore(tile))
                return true;
        }
        return false;
    }
    ...
    shouldBuildStructure(type, cityCount, hasCoastalTiles) {
        ...
        let targetCount = Math.floor(cityCount * ratio);
        const supplySummary = this.player.supplySummary();
        if ((type === UnitType.Port || type === UnitType.Factory) &&
            (supplySummary.strainedTiles > 0 || supplySummary.isolatedTiles > 0)) {
            targetCount++;
        }
        if (type === UnitType.Port && supplySummary.isolatedTiles > 0) {
            targetCount++;
        }
     ...
     */
    maybeUpgradeStructure(structures) {
        if (this.getTotalStructureDensity() <= UPGRADE_DENSITY_THRESHOLD) {
            return false;
        }
        if (structures.length === 0) {
        ...
        if (structureToUpgrade !== null) {
            ...
            this.game.addExecution(new UpgradeStructureExecution(this.player, structureToUpgrade.id()));
            return true;
        }
        return false;
    }
     ...
     */
    findBestStructureToUpgrade(structures) {
        const game = this.game;
        if (structures.length === 0) {
            return null;
        }
        ...
        for (const structure of upgradable) {
            ...
            for (const sam of samLaunchers) {
                ...
                const distSquared = game.euclideanDistSquared(structure.tile(), sam.tile());
        ...
        if (scored.length >= 2 && this.random.chance(2)) {
            const pickIndex = scored.length >= 3
    ...
    randCoastalTileArray(numTiles) {
        const tiles = Array.from(this.player.borderTiles()).filter((t) => this.game.isOceanShore(t));
    ...
    *arraySampler(a, sampleSize) {
    ...
    portValue() {
        ...
        return (tile) => {
            ...
            w += this.localSupplyBuildBonus(tile, 10);
            return w;
        };
    }
    /**
    ...
    factoryValue() {
        ...
        const cityTiles = new Set(player.units(UnitType.City).map((u) => u.tile()));
        return (tile) => {
            let w = 0;
            // Prefer higher elevations
            w += game.magnitude(tile);
            ...
            if (!useConnectionScore) {
                return w + this.localSupplyBuildBonus(tile, structureSpacing);
            }
            w +=
                this.computeConnectivityScore(tile, reachableStations, minRangeSquared, stationRangeSquared) * structureSpacing;
            return w + this.localSupplyBuildBonus(tile, structureSpacing);
        };
    }
    /**
    ...
    getOrBuildReachableStations() {
        this.reachableStationsCache ?? (this.reachableStationsCache = this.buildReachableStations());
    ...
    buildReachableStations() {
        ...
        for (const unit of player.units(UnitType.City, UnitType.Port, UnitType.Factory)) {
        ...
        for (const neighbor of player.neighbors()) {
            if (!neighbor.isPlayer())
                continue;
            if (neighbor.type() === PlayerType.Bot)
                continue;
            if (!player.canTrade(neighbor))
                continue;
            ...
            for (const unit of neighbor.units(UnitType.City, UnitType.Port, UnitType.Factory)) {
                if (unitToCluster.has(unit)) {
                    result.push({
                        tile: unit.tile(),
                        cluster: unitToCluster.get(unit),
     ...
     */
    computeConnectivityScore(tile, reachableStations, minRangeSquared, stationRangeSquared) {
        ...
        for (const { tile: stationTile, cluster, weight } of reachableStations) {
            ...
            if (dist < minRangeSquared || dist > stationRangeSquared)
                continue;
            if (cluster !== null) {
                clustersInRange.set(cluster, Math.max(clustersInRange.get(cluster) ?? 0, weight));
            }
            else {
        ...
        for (const cw of clustersInRange.values())
            score += cw;
        return score;
    }
    /**
    ...
    cityValue() {
        ...
        const factoryTiles = new Set(player.units(UnitType.Factory).map((u) => u.tile()));
        return (tile) => {
            let w = 0;
            w += game.magnitude(tile);
            const [, closestBorderDist] = closestTile(game, borderTiles, tile);
            ...
            if (!useConnectionScore) {
                return w + this.localSupplyBuildBonus(tile, structureSpacing * 0.7);
            }
            w +=
                this.computeConnectivityScore(tile, reachableStations, minRangeSquared, stationRangeSquared) * structureSpacing;
            return w + this.localSupplyBuildBonus(tile, structureSpacing * 0.7);
    ...
    localSupplyBuildBonus(tile, weight) {
    ...
    defensePostValue() {
        ...
        const hasHostileNeighbor = player
            .neighbors()
            .filter((n) => n.isPlayer() &&
            player.isFriendly(n) === false &&
            n.type() !== PlayerType.Bot &&
            n.troops() > player.troops()).length > 0;
        ...
        return (tile) => {
            ...
            if (closest !== null) {
                ...
                for (const neighborTile of game.neighbors(closest)) {
                    ...
                    if (neighbor.type() === PlayerType.Bot)
                        continue;
                    if (neighbor.troops() <= player.troops())
                        continue;
    ...
    samLauncherValue() {
        ...
        const { borderSpacing, structureSpacing } = this.spacingConstants();
        const { difficulty } = game.config().gameConfig();
        const weightByLevel = difficulty === Difficulty.Hard || difficulty === Difficulty.Impossible;
        ...
        const useCoverageWeighting = difficulty !== Difficulty.Easy && this.random.nextInt(0, 100) < 25;
...
//# sourceMappingURL=NationStructureBehavior.js.map