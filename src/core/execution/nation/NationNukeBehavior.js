import { Difficulty, GameMode, PlayerType, Relation, UnitType, } from "../../game/Game";
...
import { EMOJI_NUKE } from "./NationEmojiBehavior";
...
export class NationNukeBehavior {
    ...
    maybeSendNuke() {
        ...
        let nukeType;
        if (!this.game.config().isUnitDisabled(UnitType.HydrogenBomb) &&
            this.player.gold() >= hydroCost) {
            nukeType = UnitType.HydrogenBomb;
        }
        else if (!this.game.config().isUnitDisabled(UnitType.AtomBomb) &&
        ...
        const structures = nukeTarget.units(UnitType.City, UnitType.DefensePost, UnitType.MissileSilo, UnitType.Port, UnitType.SAMLauncher, UnitType.Factory);
        ...
        const randomTiles = randTerritoryTileArray(this.random, this.game, nukeTarget, numRandomTiles);
    ...
    findBestNukeTarget() {
        ...
        for (const relation of this.player.allRelationsSorted()) {
            ...
            if (myMaxTroops >= otherMaxTroops * 2)
                continue;
            return other;
        }
    ...
    findStrongestTeamTarget() {
        ...
        const sortedTeams = Array.from(teamTiles.entries()).sort((a, b) => b[1] - a[1]);
        ...
        const validTargets = targetTeamPlayers.filter((p) => !this.player.isFriendly(p));
        if (validTargets.length === 0) {
            return null;
        }
        if (this.random.chance(2)) {
            // Strongest player
            return validTargets.reduce((prev, current) => this.game.config().maxTroops(prev) >
    ...
    removeOldNukeEvents() {
        ...
        while (this.recentlySentNukes.length > 0 &&
    ...
    isTeammateAlreadyNukingThisSpot(tile, nukeType) {
        ...
        const activeNukes = this.game.units(UnitType.AtomBomb, UnitType.HydrogenBomb);
    ...
    isTrajectoryInterceptableBySam(spawnTile, targetTile, excludedSamIds) {
        ...
        const targetRangeSquared = this.game.config().defaultNukeTargetableRange() ** 2;
        let untargetableStart = -1;
        let untargetableEnd = -1;
        for (let i = 0; i < trajectory.length; i++) {
            const tile = trajectory[i];
            if (untargetableStart === -1) {
                if (this.game.euclideanDistSquared(tile, spawnTile) > targetRangeSquared) {
                    if (this.game.euclideanDistSquared(tile, targetTile) <
                        targetRangeSquared) {
            ...
            else if (this.game.euclideanDistSquared(tile, targetTile) < targetRangeSquared) {
                untargetableEnd = i;
                break;
            }
        }
        for (let i = 0; i < trajectory.length; i++) {
            ...
            const tile = trajectory[i];
            const nearbySams = this.game.nearbyUnits(tile, this.game.config().maxSamRange(), UnitType.SAMLauncher);
    ...
    nukeTileScore(tile, silos, targets, nukeType) {
        ...
        if (difficulty === Difficulty.Medium) {
            ...
            const hasSam = targets.some((unit) => unit.type() === UnitType.SAMLauncher &&
        ...
        if (difficulty === Difficulty.Impossible &&
            ...
            const nearbySams = this.game.nearbyUnits(tile, hydroMagnitude.outer, UnitType.SAMLauncher);
            for (const sam of nearbySams) {
                const samLevel = sam.unit.level();
                if (samLevel >= 5)
                    continue; // Can't outrange level 5+ SAMs
                const samRange = this.game.config().samRange(samLevel);
                const distToSam = Math.sqrt(this.game.euclideanDistSquared(tile, sam.unit.tile()));
    ...
    sendNuke(tile, nukeType, targetPlayer, waitTicks = 0) {
        const tick = this.game.ticks();
        ...
        this.game.addExecution(new NukeExecution(nukeType, this.player, tile, null, -1, waitTicks));
     ...
     */
    maybeDestroyEnemySam(nukeTarget) {
        ...
        for (const targetSam of sortedSams) {
            ...
            const totalInterceptions = coveringSams.reduce((sum, sam) => sum + sam.level(), 0);
            ...
            for (const silo of ourSilos) {
                ...
                const interceptable = this.isTrajectoryInterceptableBySam(silo.tile(), targetTile, coveringSamIds);
            ...
            allAvailableSilos.sort((a, b) => this.game.manhattanDist(a.silo.tile(), targetTile) -
            ...
            const sortedByFlight = [...unblockedBombs].sort((a, b) => a.flightTicks - b.flightTicks);
            let bestWindowStart = 0;
            let bestWindowCount = 0;
            for (let start = 0; start < sortedByFlight.length; start++) {
                let end = start;
                while (end < sortedByFlight.length &&
            ...
            const windowBombs = sortedByFlight.slice(bestWindowStart, bestWindowStart + bestWindowCount);
            ...
            const staggerInterval = Math.max(1, Math.floor(maxTotalArrivalSpread / totalBombs));
            let selectedIdx = 0;
            const waitTicksPerBomb = [];
            for (let i = 0; i < bombsToFire; i++) {
                if (selectedSet.has(i)) {
                    const targetArrival = selectedFlightMin + selectedIdx * staggerInterval;
                    waitTicksPerBomb.push(Math.max(0, targetArrival - launchSequence[i].flightTicks));
                    selectedIdx++;
                }
                else {
            ...
            for (let i = 0; i < bombsToFire; i++) {
                this.sendNuke(targetTile, UnitType.AtomBomb, nukeTarget, waitTicksPerBomb[i]);
            }
            return;
        }
    ...
    maybeUpgradeBestProtectedSilo() {
        ...
        for (const silo of silos) {
            if (!this.player.canUpgradeUnit(silo))
                continue;
            let protection = 0;
            for (const sam of ourSams) {
                const range = this.game.config().samRange(sam.level());
                const distSquared = this.game.euclideanDistSquared(silo.tile(), sam.tile());
        ...
        if (bestSilo !== null) {
            this.game.addExecution(new UpgradeStructureExecution(this.player, bestSilo.id()));
        }
    }
    cost(type) {
...
//# sourceMappingURL=NationNukeBehavior.js.map