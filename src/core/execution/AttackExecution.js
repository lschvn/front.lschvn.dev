...
import { CLOSE_AIR_SUPPORT_CAPTURE_MULTIPLIER, CLOSE_AIR_SUPPORT_CASUALTY_MULTIPLIER, } from "../configuration/AirBalance";
import { ATTACK_MODE_BALANCE, DEFAULT_ATTACK_MODE, RAID_PRIORITY_UNIT_TYPES, RAID_TARGET_PRIORITY, SIEGE_DAMAGEABLE_UNIT_TYPES, } from "../configuration/AttackModeBalance";
import { DEPLETED_ISOLATION_BALANCE, SUPPLY_STATE_BALANCE, } from "../configuration/SupplyBalance";
import { AttackMode, Difficulty, MessageType, PlayerType, SupplyState, TerrainType, UnitType, } from "../game/Game";
import { PseudoRandom } from "../PseudoRandom";
import { assertNever, simpleHash } from "../Util";
import { FlatBinaryHeap } from "./utils/FlatBinaryHeap"; // adjust path if needed
const malusForRetreat = 25;
export class AttackExecution {
    constructor(startTroops = null, _owner, _targetID, sourceTile = null, removeTroops = true, mode = DEFAULT_ATTACK_MODE) {
    ...
    init(mg, ticks) {
        ...
        this.startTroops ?? (this.startTroops = Math.floor(this.mg.config().attackAmount(this._owner, this.target) *
            ATTACK_MODE_BALANCE[this.mode].troopCommitmentMultiplier));
        if (this.removeTroops) {
            this.startTroops = Math.min(this._owner.troops(), this.startTroops);
            this._owner.removeTroops(this.startTroops);
        }
        this.attack = this._owner.createAttack(this.target, this.startTroops, this.sourceTile, new Set(), this.mode);
    ...
    retreat(malusPercent = 0) {
        ...
        if (deaths) {
            this.mg.displayMessage("events_display.attack_cancelled_retreat", MessageType.ATTACK_CANCELLED, this._owner.id(), undefined, { troops: renderTroops(deaths) });
        }
    ...
    tick(ticks) {
        ...
        while (numTilesPerTick > 0) {
            ...
            let effectiveTilesPerTickUsed = tilesPerTickUsed /
            ...
            let effectiveAttackerTroopLoss = attackerTroopLoss *
                ATTACK_MODE_BALANCE[this.mode].attackerCasualtyMultiplier *
                attackerSupply.attackCasualtyMultiplier;
            const effectiveDefenderTroopLoss = defenderTroopLoss * defenderSupply.defenderCasualtyMultiplier;
    ...
    attackPriorityBias(tile) {
        let bias = this.supplyScore(tile);
        if (!this.target.isPlayer() || this.mode !== AttackMode.Raid) {
            return bias;
        }
        const nearbyTargets = this.mg.nearbyUnits(tile, ATTACK_MODE_BALANCE[this.mode].targetPriorityRadius, RAID_PRIORITY_UNIT_TYPES, ({ unit }) => unit.owner() === this.target, true);
        if (nearbyTargets.length === 0) {
            return bias;
        }
        const best = nearbyTargets.reduce((currentBest, candidate) => {
            if (currentBest === null) {
                return candidate;
            }
    ...
    resolveAttackingSupplyTile(tileToConquer) {
        if (this.sourceTile !== null) {
    ...
    isFortifiedTile(tile, defender) {
        ...
        return this.mg.hasUnitNearby(tile, this.mg.config().defensePostRange(), UnitType.DefensePost, defender.id());
    }
    applyModeEffects(tile, ticks, targetPlayer) {
        ...
        switch (this.mode) {
            ...
            case AttackMode.Siege:
                ...
                this.maybePulseBuildingDamage(tile, ticks, targetPlayer, SIEGE_DAMAGEABLE_UNIT_TYPES);
                return;
            case AttackMode.Raid:
                this.maybePulseBuildingDamage(tile, ticks, targetPlayer, RAID_PRIORITY_UNIT_TYPES);
                return;
            default:
                assertNever(this.mode);
        }
    }
    applySiegeSuppression(tile) {
        const { frontlineSuppressionDuration, frontlineSuppressionRadius } = ATTACK_MODE_BALANCE[AttackMode.Siege];
        this.mg.suppressTile(tile, frontlineSuppressionDuration);
        for (const nearbyTile of this.mg.circleSearch(tile, frontlineSuppressionRadius)) {
    ...
    maybePulseBuildingDamage(tile, ticks, targetPlayer, types) {
    ...
    selectNearbyPriorityTarget(tile, targetPlayer, types) {
        const targets = this.mg.nearbyUnits(tile, ATTACK_MODE_BALANCE[this.mode].targetPriorityRadius, types, ({ unit }) => unit.owner() === targetPlayer, true);
        if (targets.length === 0) {
            return null;
        }
        targets.sort((a, b) => {
    ...
    damageModeTarget(targetUnit, targetPlayer) {
        const pulseDamage = ATTACK_MODE_BALANCE[this.mode].buildingDamagePerPulse;
        switch (targetUnit.type()) {
            case UnitType.TransportShip: {
                const remainingTroops = Math.max(0, targetUnit.troops() - pulseDamage * 15);
    ...
    applyEconomicDisruption(targetPlayer) {
        const { economicDisruptionRatio, economicDisruptionMin, economicDisruptionMax, } = ATTACK_MODE_BALANCE[AttackMode.Raid];
        if (economicDisruptionRatio <= 0) {
            return;
        }
        const availableGold = targetPlayer.gold();
        const boundedGold = availableGold > 9000000000000000n
            ? 9000000000000000n
            : availableGold;
        const proportionalLoss = BigInt(Math.floor(Number(boundedGold) * economicDisruptionRatio));
        const disruption = proportionalLoss < economicDisruptionMin
...
//# sourceMappingURL=AttackExecution.js.map