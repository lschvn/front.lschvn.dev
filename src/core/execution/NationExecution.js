import { Difficulty, Relation, TerrainType, } from "../game/Game";
...
export class NationExecution {
    constructor(gameID, nation) {
        ...
        this.random = new PseudoRandom(simpleHash(nation.playerInfo.id) + simpleHash(gameID));
    ...
    tick(ticks) {
        ...
        if (this.mg.inSpawnPhase()) {
            ...
            if (this.nation.spawnCell === undefined) {
                this.mg.addExecution(new SpawnExecution(this.gameID, this.nation.playerInfo));
                return;
            }
            ...
            this.mg.addExecution(new SpawnExecution(this.gameID, this.nation.playerInfo, rl));
            return;
        }
        ...
        if (ticks % this.attackRate !== this.attackTick) {
            ...
            if (this.player.isAlive()) {
                ...
                const twoThirds = (this.attackTick + Math.floor((this.attackRate * 2) / 3)) %
                    this.attackRate;
        ...
        this.structureBehavior.handleStructures();
        this.airBehavior.maybeManageAirPower();
    ...
    initializeBehaviors() {
        ...
        this.emojiBehavior = new NationEmojiBehavior(this.random, this.mg, this.player);
        this.mirvBehavior = new NationMIRVBehavior(this.random, this.mg, this.player, this.emojiBehavior);
        this.allianceBehavior = new NationAllianceBehavior(this.random, this.mg, this.player, this.emojiBehavior);
        this.airBehavior = new NationAirBehavior(this.random, this.mg, this.player);
        this.warshipBehavior = new NationWarshipBehavior(this.random, this.mg, this.player, this.emojiBehavior);
        this.attackBehavior = new AiAttackBehavior(this.random, this.mg, this.player, this.triggerRatio, this.reserveRatio, this.expandRatio, this.allianceBehavior, this.emojiBehavior);
        this.nukeBehavior = new NationNukeBehavior(this.random, this.mg, this.player, this.attackBehavior, this.emojiBehavior);
        this.structureBehavior = new NationStructureBehavior(this.random, this.mg, this.player);
    ...
    randomSpawnLand() {
        ...
        while (tries < 50) {
            ...
            if (this.mg.isLand(tile) && !this.mg.hasOwner(tile)) {
                if (this.mg.terrainType(tile) === TerrainType.Mountain &&
    ...
    handleEmbargoesToHostileNations() {
        ...
        others.forEach((other) => {
            ...
            if (player.relation(other) <= Relation.Hostile &&
                !player.hasEmbargoAgainst(other) &&
                !player.isOnSameTeam(other)) {
                player.addEmbargo(other, false);
            }
            else if (player.relation(other) >= Relation.Neutral &&
                player.hasEmbargoAgainst(other) &&
            ...
            else if (player.relation(other) >= Relation.Friendly &&
                player.hasEmbargoAgainst(other) &&
                this.mg.config().gameConfig().difficulty !== Difficulty.Impossible) {
                player.stopEmbargo(other);
            }
...
//# sourceMappingURL=NationExecution.js.map