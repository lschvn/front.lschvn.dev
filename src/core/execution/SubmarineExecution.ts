...
import {
  ...
  SUBMARINE_ATTACK_RATE,
  SUBMARINE_ATTACK_REVEAL_TICKS,
  SUBMARINE_TRANSPORT_DAMAGE,
  SUBMARINE_WARSHIP_DAMAGE,
} from "../configuration/NavalStealthBalance";
import { isSubmarineVisibleToPlayer } from "../game/NavalStealth";
...
export class SubmarineExecution implements Execution {
  ...
  init(mg: Game): void {
    ...
    this.submarine.setRevealedUntilTick(undefined);
  }
  ...
  private updateRevealState(ticks: number): void {
    const revealed =
      (this.submarine.revealedUntilTick() ?? -1) >= ticks ||
      this.hasEnemySonarCoverage();
    this.submarine.setTargetable(revealed);
  }

  private hasEnemySonarCoverage(): boolean {
    ...
    for (const player of this.mg.players()) {
      ...
      for (const sonar of player.units(UnitType.SonarStation)) {
        ...
        if (
          this.mg.euclideanDistSquared(sonar.tile(), this.submarine.tile()) <=
          radiusSquared
        ) {
          return true;
        }
  ...
  private findTargetUnit(): Unit | undefined {
    ...
    const ships = this.mg.nearbyUnits(
      ...
      [UnitType.TransportShip, UnitType.TradeShip, UnitType.Warship],
    );
    ...
    for (const { unit, distSquared } of ships) {
      ...
      if (
        unit.type() === UnitType.Warship &&
        isSubmarineVisibleToPlayer(this.mg, this.submarine, unit.owner())
      ) {
        continue;
      }
  ...
  private huntAndStrikeTarget(ticks: number): void {
    ...
    const inRange =
      this.mg.euclideanDistSquared(this.submarine.tile(), target.tile()) <=
    ...
    this.submarine.setRevealedUntilTick(
      ticks + SUBMARINE_ATTACK_REVEAL_TICKS,
    );
    this.updateRevealState(ticks);
    ...
    switch (target.type()) {
      case UnitType.TransportShip: {
        const remainingTroops = Math.max(
          0,
          target.troops() - SUBMARINE_TRANSPORT_DAMAGE,
        );
