...
let tickUpdateSink = null;
function gameUpdate(gu) {
    tickUpdateSink?.(gu);
}
function sendGameUpdateBatch(gameUpdates) {
...
ctx.addEventListener("message", async (e) => {
    ...
    switch (message.type) {
        case "init":
            try {
                gameRunner = createGameRunner(message.gameStartInfo, message.clientID, mapLoader, gameUpdate).then((gr) => {
                    sendMessage({
                        type: "initialized",
                        id: message.id,
                    });
        ...
        case "player_actions":
            ...
            try {
                const actions = (await gameRunner).playerActions(message.playerID, message.x, message.y, message.units);
                sendMessage({
        ...
        case "player_buildables":
            ...
            try {
                const buildables = (await gameRunner).playerBuildables(message.playerID, message.x, message.y, message.units);
                sendMessage({
