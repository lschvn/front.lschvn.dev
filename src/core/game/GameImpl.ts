...
export class GameImpl implements Game {
  ...
  private supplyUpdatePairs: number[] = [];
  ...
  private supplySystem: SupplySystem = new SupplySystem(this);
  ...
  private suppressedTilesUntil = new Map<TileRef, Tick>();
  private suppressionExpiryIndex = new Map<Tick, Set<TileRef>>();
  private airSupportZones: AirSupportZone[] = [];
  ...
  suppressTile(tile: TileRef, duration: Tick): void {
    ...
    const expiryTiles =
      this.suppressionExpiryIndex.get(expiresAt) ?? new Set<TileRef>();
  ...
  hasRadarCoverage(tile: TileRef, owner: Player): boolean {
    return this.anyUnitNearby(
      tile,
      this.config().radarStationRange(),
  ...
  private expireAirSupportZones() {
    if (this.airSupportZones.length === 0) {
      return;
    }
    this.airSupportZones = this.airSupportZones.filter(
      (zone) => zone.expiresAt > this._ticks,
    );
  }

  private touchUnitsOnTile(tile: TileRef) {
  ...
  recordSupplyUpdate(tile: TileRef): void {
    this.supplyUpdatePairs.push(
      tile,
      this.supplySystem.supplyState(tile) |
  ...
  conquer(owner: PlayerImpl, tile: TileRef): void {
    ...
    this.recordTileUpdate(tile);
    this.supplySystem.markTileOwnershipChanged(
      tile,
      previousOwnerSmallId,
      owner.smallID(),
    );
  }
  ...
  hasSuppression(ref: TileRef): boolean {
    return this._map.hasSuppression(ref);
  }
  setSuppression(ref: TileRef, value: boolean): void {
  ...
  recomputeSupplyIfNeeded(ticks: number): void {
