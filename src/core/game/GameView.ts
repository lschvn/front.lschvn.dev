...
import { isSubmarineVisibleToPlayer } from "./NavalStealth";
...
function unpackSupplyTileState(packed: number): {
  state: SupplyState;
  reserveDepleted: boolean;
} {
  return {
    state: (packed & 0b11) as SupplyState,
...
export class UnitView {
  ...
  revealedUntilTick(): Tick | undefined {
    return this.data.revealedUntilTick;
  }
...
export class GameView implements GameMap {
  ...
  private updatedSupplyTiles: TileRef[] = [];
  private supplyStates: Uint8Array;
  private supplyReserveDepleted: Uint8Array;
  ...
  constructor(
    ...
    this.supplyReserveDepleted = new Uint8Array(
  ...
  public update(gu: GameUpdateViewData) {
    ...
    if (packedSupply) {
      for (let i = 0; i + 1 < packedSupply.length; i += 2) {
        const tile = packedSupply[i] as TileRef;
  ...
  recentlyUpdatedSupplyTiles(): TileRef[] {
  ...
  nearbyUnits(
    ...
    return this.unitGrid
      ...
      .filter(({ unit }) => this.canPlayerSeeUnit(viewer, unit)) as Array<{
      unit: UnitView;
      distSquared: number;
    }>;
  }
  ...
  hasUnitNearby(
    ...
    return this.nearbyUnits(tile, searchRange, type).some(({ unit }) => {
  ...
  anyUnitNearby(
    ...
    return this.nearbyUnits(tile, searchRange, types).some(({ unit }) => {
      if (playerId !== undefined && unit.owner().id() !== playerId) {
        return false;
      }
      if (!includeUnderConstruction && unit.isUnderConstruction()) {
  ...
  canPlayerSeeUnit(
    ...
    return isSubmarineVisibleToPlayer(this, unit, player);
  }
  ...
  units(...types: UnitType[]): UnitView[] {
    ...
    if (types.length === 0) {
      return Array.from(this._units.values()).filter(
        (u) => u.isActive() && this.canPlayerSeeUnit(viewer, u),
      );
    }
    return Array.from(this._units.values()).filter(
      (u) =>
