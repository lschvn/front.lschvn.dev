...
export class UserSettings {
    emitChange(key, value) {
        try {
            const maybeDispatch = globalThis?.dispatchEvent;
            if (typeof maybeDispatch !== "function")
                return;
            globalThis.dispatchEvent(new CustomEvent("user-settings-changed", {
                detail: { key, value },
            }));
        }
        catch {
    ...
    supplyOverlay() {
        return this.get("settings.supplyOverlay", false);
    }
    cursorCostLabel() {
    ...
    toggleSupplyOverlay() {
        this.set("settings.supplyOverlay", !this.supplyOverlay());
    }
    toggleDarkMode() {
    ...
    getSelectedPatternName(cosmetics) {
        if (cosmetics === null)
            return null;
    ...
    attackRatioIncrement() {
        const increment = Math.round(this.getFloat("settings.attackRatioIncrement", 10));
