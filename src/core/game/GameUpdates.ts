...
export interface GameUpdateViewData {
  ...
  /**
   * Packed supply updates as `[tileRef, packedSupplyState]` uint32 pairs.
   *
   * `packedSupplyState` uses the low 2 bits for `SupplyState` and bit 2 for
   * the depleted-isolation reserve flag.
   */
  packedSupplyUpdates?: Uint32Array;
  /**
...
export interface UnitUpdate {
  ...
  airbaseId?: number; // Only for aircraft squadrons
  airMissionType?: AirMissionType; // Only for aircraft squadrons
  airMissionPhase?: AirMissionPhase; // Only for aircraft squadrons
  airMissionTicksRemaining?: Tick; // Only for aircraft squadrons
  rearmCompleteTick?: Tick; // Only for aircraft squadrons
}
