import { Cell, GameMapSize, GameMode, GameType, HumansVsNations, Nation, PlayerInfo, PlayerType, } from "./Game";
/**
 ...
 */
export function createNationsForGame(gameStart, manifestNations, numHumans, random) {
    const toNation = (n) => new Nation(new Cell(n.coordinates[0], n.coordinates[1]), new PlayerInfo(n.name, PlayerType.Nation, null, random.nextID()));
    const isCompactMap = gameStart.config.gameMapSize === GameMapSize.Compact;
    const isHumansVsNations = gameStart.config.gameMode === GameMode.Team &&
    ...
    if (typeof configNations === "number") {
        return createRandomNations(configNations, manifestNations, toNation, random);
    }
 ...
 */
function createRandomNations(targetCount, manifestNations, toNation, random) {
    const shuffled = random.shuffleArray(manifestNations);
    ...
    const nations = shuffled.map(toNation);
    ...
    for (let i = 0; i < additionalCount; i++) {
        ...
        usedNames.add(name);
        nations.push(new Nation(undefined, new PlayerInfo(name, PlayerType.Nation, null, random.nextID())));
    }
    return nations;
}
// For compact maps, only 25% of nations are used (minimum 1).
export function getCompactMapNationCount(manifestNationCount, isCompactMap) {
...
function generateUniqueNationName(random, usedNames) {
...
function generateNationName(random) {
...
//# sourceMappingURL=NationCreation.js.map