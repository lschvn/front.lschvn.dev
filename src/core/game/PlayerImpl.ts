...
export class PlayerImpl implements Player {
  ...
  private _supplySummary: SupplySummary = EMPTY_SUPPLY_SUMMARY;
  ...
  setSupplySummary(summary: SupplySummary): void {
  ...
  navalPatrolSpawn(tile: TileRef): TileRef | false {
  ...
  airSquadronSpawn(unitType: UnitType, tile: TileRef): TileRef | false {
    const airbase = this.units(UnitType.Airbase).find(
      (unit) =>
        unit.tile() === tile &&
    ...
    const stationedSquadrons = this.units(
      UnitType.FighterSquadron,
      UnitType.BomberSquadron,
    ).filter(
      (unit) => unit.airbaseId() === airbase.id() && unit.isActive(),
    ).length;
  ...
  coastalStructureSpawn(
    tile: TileRef,
    validTiles: TileRef[] | null = null,
  ): TileRef | false {
    const tiles = (validTiles ?? this.validStructureSpawnTiles(tile)).filter((t) =>
