...
export class MiniMapTransformer {
    ...
    findPath(from, to) {
        ...
        const miniFromArray = fromArray.map((f) => this.miniMap.ref(Math.floor(this.map.x(f) / 2), Math.floor(this.map.y(f) / 2)));
        const miniFrom = miniFromArray.length === 1 ? miniFromArray[0] : miniFromArray;
        const miniTo = this.miniMap.ref(Math.floor(this.map.x(to) / 2), Math.floor(this.map.y(to) / 2));
        // Search on minimap
        ...
        const cellPath = path.map((ref) => new Cell(this.miniMap.x(ref), this.miniMap.y(ref)));
    ...
    upscalePath(path, scaleFactor = 2) {
    ...
    fixExtremes(upscaled, cellDst, cellSrc) {
...
//# sourceMappingURL=MiniMapTransformer.js.map