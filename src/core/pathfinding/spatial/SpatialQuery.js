...
export class SpatialQuery {
    constructor(game) {
        this.game = game;
        this.boundedAStar = null;
    }
    getBoundedAStar() {
        this.boundedAStar ?? (this.boundedAStar = new AStarWaterBounded(this.game.map(), REFINE_MAX_SEARCH_AREA));
     ...
     */
    bfsNearest(from, maxDist, predicate) {
        const map = this.game.map();
        const candidates = [];
        for (const tile of map.bfs(from, (_, t) => map.manhattanDist(from, t) <= maxDist)) {
            if (predicate(tile)) {
        ...
        candidates.sort((a, b) => map.manhattanDist(from, a) - map.manhattanDist(from, b));
        return candidates[0];
    }
    /**
     ...
     */
    closestShore(owner, tile, maxDist = 50) {
        const gm = this.game;
        const ownerId = owner.smallID();
        const isValidTile = (t) => {
            if (!gm.isShore(t) || !gm.isLand(t))
                return false;
     ...
     */
    closestShoreByWater(owner, target) {
        return DebugSpan.wrap("SpatialQuery.closestShoreByWater", () => {
            ...
            if (!gm.isWater(target) && !gm.isShore(target))
                return null;
            ...
            if (!path || path.length === 0)
                return null;
            return DebugSpan.wrap("SpatialQuery.refineStartTile", () => this.refineStartTile(path, shores, gm));
        });
    }
    refineStartTile(path, shores, gm) {
        ...
        const map = gm.map();
        const candidates = shores.filter((s) => map.manhattanDist(s, bestTile) <= CANDIDATE_RADIUS);
        ...
        if (boundsArea > REFINE_MAX_SEARCH_AREA)
            return bestTile;
        const refinedPath = this.getBoundedAStar().searchBounded(candidates, waypoint, bounds);
