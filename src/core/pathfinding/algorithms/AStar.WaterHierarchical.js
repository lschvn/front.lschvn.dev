...
export class AStarWaterHierarchical {
    constructor(map, graph, options = {}) {
        this.map = map;
        this.graph = graph;
        ...
        this.localAStarMultiCluster = new AStarWaterBounded(map, maxMultiClusterNodes);
    ...
    findPath(from, to) {
        return DebugSpan.wrap("AStar.WaterHierarchical:findPath", () => {
            ...
            if (Array.isArray(from)) {
                return this.findPathMultiSource(from, to);
            }
            return this.findPathSingle(from, to);
        });
    }
    findPathMultiSource(sources, target) {
    ...
    tryShortPathMultiSource(sources, target) {
        const SHORT_PATH_THRESHOLD = 120;
        const PADDING = 10;
        const candidates = sources.filter((s) => this.map.manhattanDist(s, target) <= SHORT_PATH_THRESHOLD);
        if (candidates.length === 0)
            return null;
        const toX = this.map.x(target);
        const toY = this.map.y(target);
        let minX = toX, maxX = toX, minY = toY, maxY = toY;
    ...
    findPathSingle(from, to) {
        ...
        DebugSpan.set("nodePath", () => nodePath
            .map((nodeId) => {
        ...
        const startSegment = this.findLocalPath(from, firstNodeTile, startClusterX, startClusterY);
        if (!startSegment) {
            return null;
        }
        ...
        for (let i = 0; i < nodePath.length - 1; i++) {
            ...
            if (this.options.cachePaths) {
                ...
                if (cachedPath && cachedPath.length > 0) {
                    ...
                    DebugSpan.set("$cachedSegmentsUsed", (prev) => (prev ?? 0) + 1);
                    continue;
                }
            }
            const segmentPath = this.findLocalPath(fromTile, toTile, edge.clusterX, edge.clusterY);
            if (!segmentPath) {
                return null;
            }
        ...
        const endSegment = this.findLocalPath(lastNodeTile, to, endClusterX, endClusterY);
        if (!endSegment) {
            return null;
        }
    ...
    findNearestNode(tile) {
        ...
        return this.tileBFS.search(this.map.width(), this.map.height(), tile, maxDistance, (t) => this.graph.getComponentId(t) !== LAND_MARKER, (t, _dist) => {
    ...
    findAbstractPath(fromNodeId, toNodeId) {
        return this.abstractAStar.findPath(fromNodeId, toNodeId);
    }
    findLocalPath(from, to, clusterX, clusterY, multiCluster = false) {
...
//# sourceMappingURL=AStar.WaterHierarchical.js.map