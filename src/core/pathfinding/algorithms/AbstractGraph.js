...
export class AbstractGraph {
    constructor(clusterSize, clustersX, clustersY) {
        ...
        this._edges = [];
        this._nodeEdgeIds = []; // nodeId → edge IDs
     ...
     */
    getCachedPath(edgeId, fromNodeId) {
        const edge = this._edges[edgeId];
        if (!edge)
            return null;
     ...
     */
    setCachedPath(edgeId, fromNodeId, path) {
        const edge = this._edges[edgeId];
        if (!edge)
            return;
        // Direction: 0 if traversing A→B, 1 if traversing B→A
    ...
    _setCluster(key, cluster) {
        this._clusters[key] = cluster;
    }
    _addNodeToCluster(clusterKey, nodeId) {
...
export class AbstractGraphBuilder {
    constructor(map, clusterSize = AbstractGraphBuilder.CLUSTER_SIZE) {
        this.map = map;
        this.clusterSize = clusterSize;
        this.tileToNode = new Map();
        this.nextNodeId = 0;
        this.nextEdgeId = 0;
    ...
    build() {
        ...
        this.graph = new AbstractGraph(this.clusterSize, this.clustersX, this.clustersY);
    ...
    addNodeToCluster(clusterX, clusterY, node) {
    ...
    findNodesOnVerticalEdge(x, baseY) {
        const nodes = [];
        ...
        for (let y = baseY; y < maxY; y++) {
            ...
            const isEntrance = this.map.isWater(tile) && nextTile !== -1 && this.map.isWater(nextTile);
            if (isEntrance) {
                if (spanStart === -1) {
                    spanStart = y;
                }
    ...
    findNodesOnHorizontalEdge(y, baseX) {
        const nodes = [];
    ...
    buildClusterConnections(cx, cy) {
        ...
        const clusterMaxX = Math.min(this.width - 1, clusterMinX + this.clusterSize - 1);
        const clusterMaxY = Math.min(this.height - 1, clusterMinY + this.clusterSize - 1);
        for (let i = 0; i < nodes.length; i++) {
            ...
            const reachable = this.findAllReachableNodesInBounds(fromNode.tile, targetNodes, clusterMinX, clusterMaxX, clusterMinY, clusterMaxY);
     ...
     */
    addOrUpdateEdge(nodeIdA, nodeIdB, cost, clusterX, clusterY) {
    ...
    findAllReachableNodesInBounds(from, targetNodes, minX, maxX, minY, maxY) {
        ...
        let foundCount = 0;
        this.tileBFS.search(this.map.width(), this.map.height(), from, maxDistance, (tile) => this.map.isWater(tile), (tile, dist) => {
...
AbstractGraphBuilder.CLUSTER_SIZE = 32;
