import { BucketQueue, MinHeap } from "./PriorityQueue";
export class AbstractGraphAStar {
    ...
    findPathSingle(startId, goalId) {
        ...
        cameFrom[startId] = -1;
        const startH = weight * (Math.abs(startNode.x - goalX) + Math.abs(startNode.y - goalY));
    ...
    findPathMultiSource(startIds, goalId) {
        if (startIds.length === 0)
            return null;
        if (startIds.length === 1)
            return this.findPathSingle(startIds[0], goalId);
        this.stamp++;
        if (this.stamp > 0xffffffff) {
            this.closedStamp.fill(0);
            this.gScoreStamp.fill(0);
...
//# sourceMappingURL=AStar.AbstractGraph.js.map