...
import { ClientMessageSchema, PartialGameRecordSchema, } from "../core/Schemas";
...
export async function startWorker() {
    ...
    const privilegeRefresher = new PrivilegeRefresher(config.jwtIssuer() + "/cosmetics.json", config.jwtIssuer() + "/profane_words_game_server", config.apiKey(), log);
    ...
    app.use("/maps", express.static(path.join(__dirname, "../../static/maps"), {
        maxAge: "1y",
    ...
    app.post("/api/create_game/:id", async (req, res) => {
        ...
        else if (!req.headers[config.adminHeader()] // Public games use admin token instead
        ) {
            return res
                .status(400)
        ...
        if (gc?.gameType === GameType.Public &&
            ...
            log.warn(`cannot create public game ${id}, ip ${ipAnonymize(clientIP)} incorrect admin token`);
            return res.status(401).send("Unauthorized");
        }
        // Double-check this worker should host this game
        const expectedWorkerId = config.workerIndex(id);
        if (expectedWorkerId !== workerId) {
            log.warn(`This game ${id} should be on worker ${expectedWorkerId}, but this is worker ${workerId}`);
        ...
        log.info(`Worker ${workerId}: IP ${ipAnonymize(clientIP)} creating ${game.isPublic() ? GameType.Public : GameType.Private}${gc?.gameMode ? ` ${gc.gameMode}` : ""} game with id ${id}${creatorPersistentID ? `, creator: ${creatorPersistentID.substring(0, 8)}...` : ""}`);
    ...
    app.post("/api/start_game/:id", async (req, res) => {
        ...
        if (game.isPublic()) {
            // eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
            const clientIP = req.ip || req.socket.remoteAddress || "unknown";
            log.info(`cannot start public game ${game.id}, game is public, ip: ${ipAnonymize(clientIP)}`);
            return;
        }
        game.start();
    ...
    app.post("/api/archive_singleplayer_game", async (req, res) => {
        try {
            ...
            if (gameRecord.info.config.gameType !== GameType.Singleplayer) {
                log.warn(`cannot archive singleplayer with game type ${gameRecord.info.config.gameType}`, {
    ...
    wss.on("connection", (ws, req) => {
        ws.on("message", async (message) => {
            ...
            try {
                ...
                const parsed = ClientMessageSchema.safeParse(JSON.parse(message.toString()));
                if (!parsed.success) {
                ...
                if (clientMsg.type === "ping") {
                    ...
                    log.warn(`Invalid message before join: ${JSON.stringify(clientMsg, replacer)}`);
                    return;
                }
                // Verify this worker should handle this game
                const expectedWorkerId = config.workerIndex(clientMsg.gameID);
                if (expectedWorkerId !== workerId) {
                    log.warn(`Worker mismatch: Game ${clientMsg.gameID} should be on worker ${expectedWorkerId}, but this is worker ${workerId}`);
                    return;
                }
                ...
                if (clientMsg.type === "rejoin") {
                    ...
                    const wasFound = gm.rejoinClient(ws, persistentId, clientMsg.gameID, clientMsg.lastTurn);
                    if (!wasFound) {
                ...
                if (gm.rejoinClient(ws, persistentId, clientMsg.gameID, 0, censoredUsername)) {
                    return;
                }
                let roles;
                let flares;
                ...
                if (claims === null) {
                    ...
                    if (allowedFlares !== undefined) {
                        const allowed = allowedFlares.length === 0 ||
                            allowedFlares.some((f) => flares?.includes(f));
                        if (!allowed) {
                            log.warn("Forbidden: player without an allowed flare attempted to join game");
                ...
                if (config.env() !== GameEnv.Dev) {
                    const turnstileResult = await verifyTurnstileToken(ip, clientMsg.turnstileToken, config.turnstileSecretKey());
                ...
                const client = new Client(generateID(), persistentId, claims, roles, flares, ip, censoredUsername, clientMsg.username, ws, cosmeticResult.cosmetics);
                ...
                log.warn(`error handling websocket message for ${ipAnonymize(ip)}: ${error}`.substring(0, 250));
            }
        });
        ws.on("error", (error) => {
...
async function startMatchmakingPolling(gm) {
    startPolling(async () => {
        try {
            ...
            if (data.assignment) {
                gm.createGame(gameId, playlist.get1v1Config(), undefined, Date.now() + 7000);
            }
        }
        catch (error) {
...
function generateGameIdForWorker() {
    let attempts = 1000;
    while (attempts > 0) {
