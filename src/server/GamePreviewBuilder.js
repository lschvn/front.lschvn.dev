...
function parseWinner(winnerArray, players) {
    if (!winnerArray || winnerArray.length < 2)
        return undefined;
    const idToName = new Map((players ?? []).map((p) => [p.clientID, p.username]));
...
function countActivePlayers(players) {
    return (players ?? []).filter((p) => {
        if (!p || p.stats === null || p.stats === undefined)
            return false;
...
export function buildPreview(gameID, origin, workerPath, lobby, publicInfo) {
    ...
    const numericTeamCount = typeof playerTeams === "number" && playerTeams > 0
        ? playerTeams
        : undefined;
    // For finished games, show "x teams of y". For lobbies, just show "x teams"
    const teamBreakdownLabel = numericTeamCount
        ? isFinished
            ? `${numericTeamCount} teams of ${Math.max(1, Math.ceil(activePlayers / numericTeamCount))}`
    ...
    if (isFinished) {
        ...
        const matchTimestamp = publicInfo?.info?.start ??
            publicInfo?.info?.end ??
        ...
        if (duration !== undefined)
            detailParts.push(`${formatDuration(duration)}`);
        ...
        if (isPrivate) {
            ...
            if (gc?.infiniteGold)
                gameOptions.push("Infinite Gold");
            if (gc?.infiniteTroops)
                gameOptions.push("Infinite Troops");
            if (gc?.instantBuild)
                gameOptions.push("Instant Build");
            if (gc?.randomSpawn)
                gameOptions.push("Random Spawn");
            if (gc?.nations === "disabled")
                gameOptions.push("Nations Disabled");
            if (gc?.donateTroops)
                gameOptions.push("Troop Donations Enabled");
            if (gameOptions.length > 0) {
                sections.push(`Game Options: ${gameOptions.join(" | ")}`);
            }
            if (Array.isArray(gc?.disabledUnits) && gc.disabledUnits.length > 0) {
                sections.push(`Disabled Units: ${gc.disabledUnits.map(String).join(" | ")}`);
            }
...
//# sourceMappingURL=GamePreviewBuilder.js.map