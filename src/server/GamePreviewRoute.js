...
import { buildPreview, escapeHtml, ExternalGameInfoSchema, } from "./GamePreviewBuilder";
import { renderHtmlContent, setHtmlNoCacheHeaders } from "./RenderHtml";
const requestOrigin = (req, config) => {
...
export function registerGamePreviewRoute(opts) {
    ...
    const fetchPublicGameInfo = async (gameID) => {
    ...
    app.get("/game/:id", async (req, res) => {
        ...
        const lobby = game ? game.gameInfo() : null;
        try {
            ...
            const meta = buildPreview(gameID, origin, config.workerPath(gameID), lobby, publicInfo);
...
//# sourceMappingURL=GamePreviewRoute.js.map