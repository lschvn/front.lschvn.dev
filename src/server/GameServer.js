...
import { ClientMessageSchema, GameStartInfoSchema, ServerDesyncSchema, ServerPrestartMessageSchema, } from "../core/Schemas";
...
export class GameServer {
    constructor(id, log_, createdAt, config, gameConfig, creatorPersistentID, startsAt, publicGameType) {
        this.id = id;
        this.log_ = log_;
        this.createdAt = createdAt;
        this.config = config;
        this.gameConfig = gameConfig;
        this.creatorPersistentID = creatorPersistentID;
        this.startsAt = startsAt;
        this.publicGameType = publicGameType;
        this.sentDesyncMessageClients = new Set();
        this.maxGameDuration = 3 * 60 * 60 * 1000; // 3 hours
        this.disconnectedTimeout = 1 * 30 * 1000; // 30 seconds
        this.turns = [];
        this.intents = [];
        this.activeClients = [];
        this.allClients = new Map();
        // Map persistentID to clientID for reconnection lookup
        this.persistentIdToClientId = new Map();
        this.clientsDisconnectedStatus = new Map();
        this._hasStarted = false;
        this._startTime = null;
        this.lastPingUpdate = 0;
        this.winner = null;
        this._hasPrestarted = false;
        this.kickedPersistentIds = new Set();
        this.outOfSyncClients = new Set();
        this.isPaused = false;
    ...
    isKicked(clientID) {
        ...
        return (persistentID !== undefined && this.kickedPersistentIds.has(persistentID));
    }
    // Get existing clientID for this persistentID, or null if new player
    getClientIdForPersistentId(persistentID) {
    ...
    joinClient(client) {
        ...
        if (this.gameConfig.gameType === GameType.Public &&
            this.activeClients.filter((c) => c.ip === client.ip && c.clientID !== client.clientID).length >= 3) {
        ...
        if (this.config.env() === GameEnv.Prod) {
            ...
            const conflicting = this.activeClients.find((c) => c.persistentID === client.persistentID &&
    ...
    rejoinClient(ws, persistentID, lastTurn = 0, newUsername) {
        ...
        client.ws.on("message", async (message) => {
            try {
                ...
                switch (clientMsg.type) {
                    ...
                    case "intent": {
                        ...
                        switch (stampedIntent.type) {
                            case "mark_disconnected": {
                                this.log.warn(`Should not receive mark_disconnected intent from client`);
                                return;
                            }
                    ...
                    default: {
                        this.log.warn(`Unknown message type: ${clientMsg.type}`, {
                            clientID: client.clientID,
                        });
                        break;
                    }
                ...
                this.log.info(`error handline websocket request in game server: ${error}`, {
                    clientID: client.clientID,
                });
            }
        });
    ...
    prestart() {
        ...
        if (!prestartMsg.success) {
            console.error(`error creating prestart message for game ${this.id}, ${prestartMsg.error}`.substring(0, 250));
            return;
        }
    ...
    start() {
        ...
        this.gameStartInfo = result.data;
    ...
    sendStartGameMsg(ws, lastTurn) {
        ...
        try {
            ...
            throw new Error(`error sending start message for game ${this.id}, ${error}`.substring(0, 250));
        }
    }
    endTurn() {
    ...
    phase() {
        ...
        const notEnoughPlayers = this.gameConfig.gameType === GameType.Public &&
    ...
    kickClient(clientID, reasonKey = KICK_REASON_DUPLICATE_SESSION) {
        ...
        if (client) {
            ...
            this.activeClients = this.activeClients.filter((c) => c.clientID !== clientID);
        }
        else {
            this.log.warn(`cannot kick client, not found in game`, {
                clientID,
    ...
    isClientDisconnected(clientID) {
        return this.clientsDisconnectedStatus.get(clientID) ?? true;
    }
    markClientDisconnected(clientID, isDisconnected) {
    ...
    archiveGame() {
        ...
        const playerRecords = this.gameStartInfo.players.map((player) => {
            ...
            return {
                ...
                persistentID: this.allClients.get(player.clientID)?.persistentID ?? "",
                stats,
                cosmetics: player.cosmetics,
                clanTag: getClanTag(player.username) ?? undefined,
            };
        });
        archive(finalizeGameRecord(createPartialGameRecord(this.id, this.gameStartInfo.config, playerRecords, this.turns, this._startTime ?? 0, Date.now(), this.winner?.winner, this.createdAt)));
    }
    handleSynchronization() {
        ...
        const { mostCommonHash, outOfSyncClients } = this.findOutOfSyncClients(lastHashTurn);
        ...
        const serverDesync = ServerDesyncSchema.safeParse({
            ...
            clientsWithCorrectHash: this.activeClients.length - outOfSyncClients.length,
    ...
    findOutOfSyncClients(turnNumber) {
    ...
    handleWinner(client, clientMsg) {
        if (this.outOfSyncClients.has(client.clientID) ||
        ...
        this.log.info(`received winner vote ${clientMsg.winner}, ${ratio} votes for this winner`, {
            clientID: client.clientID,
        });
        ...
        this.log.info(`Winner determined by ${potentialWinner.ips.size}/${activeUniqueIPs.size} active IPs`, {
            winnerKey: winnerKey,
        });
        this.archiveGame();
    }
