...
export class ChatIntegration {
    constructor(game, eventBus) {
        ...
        if (!this.ctModal) {
            throw new Error("Chat modal element not found. Ensure chat-modal element exists in DOM before initializing ChatIntegration");
        }
    }
    setupChatModal(sender, recipient) {
        this.ctModal.setSender(sender);
        this.ctModal.setRecipient(recipient);
    }
    createQuickChatMenu(recipient) {
        if (!this.ctModal) {
        ...
        return this.ctModal.categories.map((category) => {
            ...
            const categoryColor = COLORS.chat[category.id] ||
                COLORS.chat.default;
                ...
                return {
                    ...
                    action: (params) => {
                        if (phrase.requiresPlayer) {
                            this.ctModal.openWithSelection(category.id, phrase.key, myPlayer, recipient);
                        }
                        else {
                            this.eventBus.emit(new SendQuickChatEvent(recipient, `${category.id}.${phrase.key}`, undefined));
                        }
                    },
                };
            });
...
//# sourceMappingURL=ChatIntegration.js.map