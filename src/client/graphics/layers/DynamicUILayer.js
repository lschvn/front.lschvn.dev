...
export class DynamicUILayer {
    ...
    renderUIElements(context, delta) {
    ...
    createdThisTick(unit) {
        return unit.createdAt() === this.game.ticks();
    }
    addNumber(num, x, y, duration, riseDistance) {
        if (BigInt(num) === 0n)
        ...
        const sign = num >= 0 ? "+" : "-";
        this.uiElements.push(new TextIndicator(this.transformHandler, `${sign} ${shortened}`, x, y, duration, riseDistance));
    }
}
//# sourceMappingURL=DynamicUILayer.js.map