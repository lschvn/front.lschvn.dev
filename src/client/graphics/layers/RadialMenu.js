...
function resolveColor(item, params) {
    if (typeof item.color === "function") {
        return params ? item.color(params) : undefined;
    }
    return item.color;
}
export class CloseRadialMenuEvent {
    constructor() { }
}
export class RadialMenu {
    constructor(eventBus, rootMenu, centerButtonElement, config = {}) {
        this.eventBus = eventBus;
        this.rootMenu = rootMenu;
        this.centerButtonElement = centerButtonElement;
        this.tooltipElement = null;
        this.isVisible = false;
        this.currentLevel = 0; // Current menu level (0 = main menu, 1 = submenu, etc.)
        this.menuStack = []; // Stack to track menu navigation history
        this.currentMenuItems = []; // Current active menu items (changes based on level)
        ...
        this.defaultCenterButtonColor = "#2c3e50";
        this.params = null;
        this.handleResize = () => {
            if (this.isVisible)
    ...
    createMenuElement() {
        ...
        const totalSize = this.config.menuSize *
    ...
    getInnerRadiusForLevel(level) {
        return level === 0 ? 40 : 50 + 25;
    }
    getOuterRadiusForLevel(level) {
    ...
    renderPaths(arcs, arc, level) {
        arcs
            ...
            .attr("stroke", "#ffffff")
            .attr("stroke-width", "2")
            .style("cursor", (d) => this.params === null || d.data.disabled(this.params)
            ? "not-allowed"
            : "pointer")
            .style("opacity", (d) => this.params === null || d.data.disabled(this.params) ? 0.5 : 1)
            .style("transition", `filter ${this.config.menuTransitionDuration / 2}ms, stroke-width ${this.config.menuTransitionDuration / 2}ms, fill ${this.config.menuTransitionDuration / 2}ms`)
        ...
        arcs.each((d) => {
            if (d.data.timerFraction && this.params) {
                ...
                const baseColor = disabled
                    ...
                    : (resolveColor(d.data, this.params) ?? "#333333");
                const opacity = disabled ? 0.5 : 0.7;
                const normalColor = d3.color(baseColor)?.copy({ opacity: opacity })?.toString() ??
                    baseColor;
                const interpolated = d3.color(d3.interpolateRgb(baseColor, "white")(0.4));
                const fadedColor = interpolated?.copy({ opacity })?.toString() ?? normalColor;
        ...
        arcs.each((d) => {
            ...
            this.menuPaths.set(pathId, path);
            if (pathId === this.selectedItemId &&
                level === 0 &&
                this.currentLevel > 0) {
                path.attr("filter", "url(#glow)");
                path.attr("stroke-width", "3");
                const color = this.params === null || d.data.disabled(this.params)
                    ? this.config.disabledColor
                    : (resolveColor(d.data, this.params) ?? "#333333");
    ...
    setupEventHandlers(arcs, level) {
        const onHover = (d, path) => {
            const disabled = this.params === null || d.data.disabled(this.params);
        ...
        function handleMouseMove(event) {
            const tooltipEl = document.querySelector(".radial-tooltip");
    ...
    renderIconsAndText(arcs, arc) {
        arcs
            ...
            if (d.data.renderType && this.params) {
                const stateKey = this.getStateKeyByType(d.data.renderType, disabled, this.params);
                if (stateKey) {
                    content.attr("data-prev-state", stateKey);
                }
                if (d.data.renderType === "allyExtend") {
                    this.renderAllyExtendIcon(content.node(), arc.centroid(d)[0], arc.centroid(d)[1], this.config.iconSize, disabled, this.params, d.data.icon);
                }
            }
            else if (d.data.text) {
                content
            ...
            this.menuIcons.set(contentId, content);
        });
    }
    setupAnimations(menuGroup) {
        menuGroup
            .transition()
    ...
    updateMenuVisibility(direction = "backward") {
    ...
    animatePreviousMenu() {
        const container = this.menuElement.select(".menu-container");
        const currentMenu = container.select(`.menu-level-${this.currentLevel - 1}`);
        currentMenu
            .transition()
            .duration(this.config.menuTransitionDuration * 0.8)
            .style("transform", `scale(${this.currentLevel === 1 ? "0.65" : "0.5"})`)
    ...
    animateMenuTransitions() {
        const container = this.menuElement.select(".menu-container");
        const currentSubmenu = container.select(`.menu-level-${this.currentLevel + 1}`);
    ...
    animateExistingMenu(previousMenu) {
        previousMenu
            .transition()
            .duration(this.config.menuTransitionDuration * 0.8)
            .style("transform", "scale(1)")
    ...
    updateCenterButtonState(state) {
        ...
        const buttonColor = state === "back" ? this.defaultCenterButtonColor : this.centerButtonColor;
        centerButton
            .select(".center-button-visible")
    ...
    onCenterButtonHover(isHovering) {
    ...
    getDefaultCenterIconSize() {
        return this.config.centerIconSize;
    }
    setCenterButtonAppearance(icon, color, iconSize) {
        ...
        this.menuElement
            .select(".center-button-visible")
            .attr("fill", this.isCenterButtonEnabled() ? this.centerButtonColor : "#999999");
    ...
    refreshAllyExtendIcon(item, disabled, icon) {
        if (item.renderType !== "allyExtend" || !this.params) {
            return;
        }
        const stateKey = this.getStateKeyByType(item.renderType, disabled, this.params);
        ...
        if (stateKey && stateKey === prevState) {
            ...
            this.renderAllyExtendIcon(icon.node(), cx, cy, this.config.iconSize, disabled, this.params, item.icon, true);
        }
    }
    maybeUpdateTimerGradient(item, color, opacity) {
        ...
        if (!gradient.empty()) {
            const offset = 1 - fraction;
            const normalColor = d3.color(color)?.copy({ opacity: opacity })?.toString() ?? color;
    ...
    getStateKeyByType(type, disabled, params) {
        switch (type) {
            case "allyExtend":
    ...
    getAllyExtendStateKey(disabled, params) {
    ...
    renderAllyExtendIcon(content, cx, cy, iconSize, disabled, params, icon, update) {
        if (update) {
    ...
    clampAndSetMenuPositionForLevel(level) {
        const outerRadius = this.getOuterRadiusForLevel(level);
        ...
        const clampedX = 2 * margin > vw
            ? vw / 2
            : Math.min(Math.max(this.anchorX, margin), vw - margin);
        const clampedY = 2 * margin > vh
            ? vh / 2
