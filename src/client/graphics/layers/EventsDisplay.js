...
import { AllPlayers, getMessageCategory, MessageCategory, MessageType, } from "../../../core/game/Game";
import { GameUpdateType, } from "../../../core/game/GameUpdates";
import { SendAllianceExtensionIntentEvent, SendAllianceRejectIntentEvent, SendAllianceRequestIntentEvent, } from "../../Transport";
...
let EventsDisplay = class EventsDisplay extends LitElement {
    updated(changed) {
    ...
    renderToggleButton(src, category) {
    ...
    constructor() {
        ...
        this.alliancesCheckedAt = new Map();
        this._hidden = false;
    ...
    tick() {
        ...
        let remainingEvents = this.events.filter((event) => {
            const shouldKeep = this.game.ticks() - event.createdAt < (event.duration ?? 600) &&
            ...
            this.addEvent({
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        className: "btn",
                        action: () => this.eventBus.emit(new SendAllianceExtensionIntentEvent(other)),
                    },
                    {
    ...
    removeAllianceRenewalEvents(allianceID) {
        this.events = this.events.filter((event) => !(event.type === MessageType.RENEW_ALLIANCE &&
    ...
    onDisplayChatEvent(event) {
        ...
        if (event.target) {
            try {
                ...
                console.warn(`Failed to resolve player for target ID '${event.target}'`, e);
                return;
            }
        }
        ...
        this.addEvent({
            ...
            buttons: [
                ...
                },
                {
                    ...
                    className: "btn",
                    action: () => this.eventBus.emit(new SendAllianceRequestIntentEvent(recipient, requestor)),
                },
                {
                    text: translateText("events_display.reject_alliance"),
                    className: "btn-info",
                    action: () => this.eventBus.emit(new SendAllianceRejectIntentEvent(requestor)),
                },
            ],
            highlight: true,
    ...
    onAllianceRequestReplyEvent(update) {
        const myPlayer = this.game.myPlayer();
        if (!myPlayer) {
            return;
        }
        ...
        if (update.request.recipientID === myPlayer.smallID()) {
            ...
            this.events = this.events.filter((event) => !(event.type === MessageType.ALLIANCE_REQUEST &&
    ...
    onBrokeAllianceEvent(update) {
        ...
        if (betrayed.isDisconnected())
            return; // Do not send the message if betraying a disconnected player
        if (!betrayed.isTraitor() && traitor === myPlayer) {
            const malusPercent = Math.round((1 - this.game.config().traitorDefenseDebuff()) * 100);
            const traitorDuration = Math.floor(this.game.config().traitorDuration() * 0.1);
            const durationText = traitorDuration === 1
    ...
    onAllianceExpiredEvent(update) {
        const myPlayer = this.game.myPlayer();
        if (!myPlayer)
            return;
        const otherID = update.player1ID === myPlayer.smallID()
            ? update.player2ID
        ...
        if (!other || !myPlayer.isAlive() || !other.isAlive())
            return;
        this.addEvent({
    ...
    onAllianceExtensionEvent(update) {
        const myPlayer = this.game.myPlayer();
        if (!myPlayer || myPlayer.smallID() !== update.playerID)
            return;
        this.removeAllianceRenewalEvents(update.allianceID);
        this.requestUpdate();
    }
    onTargetPlayerEvent(event) {
        const other = this.game.playerBySmallID(event.playerID);
        const myPlayer = this.game.myPlayer();
        if (!myPlayer || !myPlayer.isFriendly(other))
            return;
    ...
    emitGoToUnitEvent(unit) {
        this.eventBus.emit(new GoToUnitEvent(unit));
    }
    onEmojiMessageEvent(update) {
        const myPlayer = this.game.myPlayer();
        if (!myPlayer)
            return;
        const recipient = update.emoji.recipientID === AllPlayers
            ? AllPlayers
      ...
      ${this._hidden
            ...
            <div class="relative w-fit z-50">
              ${this.renderButton({
                ...
                className: "text-white cursor-pointer pointer-events-auto w-fit p-2 lg:p-3 min-[1200px]:rounded-lg sm:rounded-tl-lg bg-gray-800/70 backdrop-blur-xs",
            })}
            </div>
          `
            : html `
              ...
              >
                <div class="flex justify-between items-center gap-3">
                  <div class="flex gap-4">
                    ...
                    ${this.renderToggleButton(donateGoldIcon, MessageCategory.TRADE)}
                  ...
                  >
                    <tbody>
                      ${filteredEvents.map((event, index) => html `
                          <tr>
                            <td
                              class="lg:px-2 lg:py-1 p-1 text-left ${getMessageTypeClasses(event.type)}"
                            >
                              ${event.focusID
                ? this.renderButton({
...
__decorate([
    state()
], EventsDisplay.prototype, "_hidden", void 0);
__decorate([
    state()
], EventsDisplay.prototype, "_isVisible", void 0);
__decorate([
    state()
], EventsDisplay.prototype, "newEvents", void 0);
__decorate([
    state()
], EventsDisplay.prototype, "latestGoldAmount", void 0);
__decorate([
    state()
], EventsDisplay.prototype, "goldAmountAnimating", void 0);
__decorate([
    state()
], EventsDisplay.prototype, "eventsFilters", void 0);
__decorate([
    query(".events-container")
], EventsDisplay.prototype, "_eventsContainer", void 0);
EventsDisplay = __decorate([
    customElement("events-display")
], EventsDisplay);
export { EventsDisplay };
//# sourceMappingURL=EventsDisplay.js.map