...
import { computeAllianceClipPath, createAllianceProgressIcon, getFirstPlacePlayer, getPlayerIcons, } from "../PlayerIcons";
import shieldIcon from "/images/ShieldIconBlack.svg?url";
class RenderInfo {
    constructor(player, lastRenderCalc, location, fontSize, fontColor, element) {
        this.player = player;
        this.lastRenderCalc = lastRenderCalc;
...
export class NameLayer {
    constructor(game, transformHandler, eventBus) {
        this.game = game;
    ...
    updateElementVisibility(render) {
    ...
    renderLayer(mainContex) {
        ...
        mainContex.drawImage(this.canvas, 0, 0, mainContex.canvas.width, mainContex.canvas.height);
    }
    ...
    renderPlayerInfo(render) {
        ...
        // Update text sizes
        const nameDiv = render.element.querySelector(".player-name");
        const flagDiv = render.element.querySelector(".player-flag");
        const troopsDiv = render.element.querySelector(".player-troops");
        ...
        const density = renderNumber(render.player.troops() / render.player.numTilesOwned());
        const shieldDiv = render.element.querySelector(".player-shield");
        ...
        // Handle icons
        const iconsDiv = render.element.querySelector(".player-icons");
        ...
        for (const icon of icons) {
            if (icon.kind === "emoji" && icon.text) {
                let emojiDiv = render.icons.get(icon.id);
                if (!emojiDiv) {
                ...
                if (icon.id === "alliance") {
                    let allianceWrapper = render.icons.get(icon.id);
                    ...
                    if (!allianceWrapper) {
                        allianceWrapper = createAllianceProgressIcon(iconSize, fraction, hasExtensionRequest, this.userSettings.darkMode());
                        ...
                        const overlay = allianceWrapper.querySelector(".alliance-progress-overlay");
                        if (overlay) {
                            overlay.style.clipPath = computeAllianceClipPath(fraction);
                        }
                        const questionMark = allianceWrapper.querySelector(".alliance-question-mark");
                        if (questionMark) {
                ...
                let imgElement = render.icons.get(icon.id);
                if (!imgElement) {
    ...
    createIconElement(src, size, center = false) {
...
//# sourceMappingURL=NameLayer.js.map