...
import { AlternateViewEvent, ContextMenuEvent, MouseUpEvent, TouchEvent, UnitSelectionEvent, } from "../../InputHandler";
import { MoveWarshipIntentEvent } from "../../Transport";
import { GameUpdateType } from "../../../core/game/GameUpdates";
import { getColoredSprite, isSpriteReady, loadAllSprites, } from "../SpriteLoader";
var Relationship;
(function (Relationship) {
    Relationship[Relationship["Self"] = 0] = "Self";
    Relationship[Relationship["Ally"] = 1] = "Ally";
    Relationship[Relationship["Enemy"] = 2] = "Enemy";
...
export class UnitLayer {
    constructor(game, eventBus, transformHandler) {
        ...
        this.NAVAL_SELECTION_RADIUS = 10; // Radius in game cells for selectable naval patrol units
    ...
    tick() {
        const updatedUnitIds = this.game
            .updatesSinceLastTick()?.[GameUpdateType.Unit]?.map((unit) => unit.id) ?? [];
        ...
        const unitIds = new Set(updatedUnitIds);
    ...
    /**
     * Find player-owned naval patrol units near the given cell within a configurable radius
     * @param clickRef The tile to check
     * @returns Array of player's naval units in range, sorted by distance (closest first)
     */
    findNavalUnitsNearCell(clickRef) {
        return this.game
    ...
    onMouseUp(event, clickRef, nearbyNavalUnits) {
        ...
        if (this.selectedUnit) {
            this.eventBus.emit(new MoveWarshipIntentEvent(this.selectedUnit.id(), clickRef));
            // Deselect
            this.eventBus.emit(new UnitSelectionEvent(this.selectedUnit, false));
            return;
        }
        nearbyNavalUnits ?? (nearbyNavalUnits = this.findNavalUnitsNearCell(clickRef));
    ...
    onTouch(event) {
        ...
        if (nearbyNavalUnits.length > 0) {
            this.onMouseUp(new MouseUpEvent(event.x, event.y), clickRef, nearbyNavalUnits);
        }
        else {
            this.eventBus.emit(new ContextMenuEvent(event.x, event.y));
        }
    ...
    redraw() {
        ...
        this.unitToTrail.forEach((trail, unit) => {
            for (const t of trail) {
                this.paintCell(this.game.x(t), this.game.y(t), this.relationship(unit), unit.owner().territoryColor(), 150, this.unitTrailContext);
            }
        });
    }
    updateUnitsSprites(unitIds) {
        const unitsToUpdate = unitIds
            ?.map((id) => this.game.unit(id))
            .filter((unit) => unit !== undefined);
        if (unitsToUpdate) {
            this.clearUnitsCells(unitsToUpdate);
            this.drawUnitsCells(unitsToUpdate.filter((unit) => this.game.canPlayerSeeUnit(this.game.myPlayer(), unit)));
        }
    }
    clearUnitsCells(unitViews) {
        unitViews
            ...
            const currX = this.game.x(unitView.tile());
            const currY = this.game.y(unitView.tile());
            this.context.clearRect(lastX - clearsize / 2, lastY - clearsize / 2, clearsize, clearsize);
            this.context.clearRect(currX - clearsize / 2, currY - clearsize / 2, clearsize, clearsize);
        });
    }
    drawUnitsCells(unitViews) {
    ...
    handleSubmarineEvent(unit) {
        if (unit.targetable()) {
            this.drawSprite(unit, colord("rgb(80,180,255)"));
            return;
        }
        this.drawSprite(unit, colord("rgb(34,72,112)"));
    }
    handleShellEvent(unit) {
        ...
        this.paintCell(this.game.x(unit.tile()), this.game.y(unit.tile()), rel, unit.owner().borderColor(), 255);
        this.paintCell(this.game.x(unit.lastTile()), this.game.y(unit.lastTile()), rel, unit.owner().borderColor(), 255);
    }
    // interception missile from SAM
    handleMissileEvent(unit) {
        this.drawSprite(unit);
    }
    drawTrail(trail, color, rel) {
        // Paint new trail
        for (const t of trail) {
            this.paintCell(this.game.x(t), this.game.y(t), rel, color, 150, this.unitTrailContext);
        }
    }
    clearTrail(unit) {
        ...
        for (const [other, trail] of this.unitToTrail) {
            for (const t of trail) {
                if (trailSet.has(t)) {
                    this.paintCell(this.game.x(t), this.game.y(t), rel, other.owner().territoryColor(), 150, this.unitTrailContext);
                }
            }
        }
    }
    handleNuke(unit) {
        ...
        this.drawTrail(trail.slice(-newTrailSize), unit.owner().territoryColor(), rel);
        this.drawSprite(unit);
        if (!unit.isActive()) {
            this.clearTrail(unit);
        }
    }
    handleMIRVWarhead(unit) {
        const rel = this.relationship(unit);
        this.clearCell(this.game.x(unit.lastTile()), this.game.y(unit.lastTile()));
        if (unit.isActive()) {
            // Paint area
    ...
    handleTradeShipEvent(unit) {
        this.drawSprite(unit);
    }
    handleTrainEvent(unit) {
        this.drawSprite(unit);
    }
    handleBoatEvent(unit) {
        const rel = this.relationship(unit);
        if (!this.unitToTrail.has(unit)) {
            this.unitToTrail.set(unit, []);
        }
    ...
    paintCell(x, y, relationship, color, alpha, context = this.context) {
    ...
    clearCell(x, y, context = this.context) {
        context.clearRect(x, y, 1, 1);
    }
    drawSprite(unit, customTerritoryColor) {
        ...
        const sprite = getColoredSprite(unit, this.theme, alternateViewColor ?? customTerritoryColor, alternateViewColor ?? undefined);
        if (unit.isActive()) {
            ...
            this.context.drawImage(sprite, Math.round(x - sprite.width / 2), Math.round(y - sprite.height / 2), sprite.width, sprite.width);
            if (!targetable) {
...
//# sourceMappingURL=UnitLayer.js.map