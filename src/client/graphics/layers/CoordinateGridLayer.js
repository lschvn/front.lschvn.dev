...
export class CoordinateGridLayer {
    ...
    renderLayer(context) {
        if (!this.isVisible && !this.alternateView)
            return;
        ...
        const cacheKey = this.buildCacheKey(width, height, canvasWidth, canvasHeight);
        const cacheContext = this.ensureCacheContext(canvasWidth, canvasHeight);
        if (cacheContext === null || this.cachedGridCanvas === null)
            return;
    ...
    ensureCacheContext(canvasWidth, canvasHeight) {
        this.cachedGridCanvas ?? (this.cachedGridCanvas = document.createElement("canvas"));
        if (this.cachedGridCanvas.width !== canvasWidth ||
        ...
        this.cachedGridContext ?? (this.cachedGridContext = this.cachedGridCanvas.getContext("2d"));
        return this.cachedGridContext;
    }
    buildCacheKey(width, height, canvasWidth, canvasHeight) {
        const topLeft = this.transformHandler.worldToScreenCoordinates(new Cell(0, 0));
        const bottomRight = this.transformHandler.worldToScreenCoordinates(new Cell(width, height));
        return [
            width,
            height,
            canvasWidth,
    ...
    drawGrid(context, width, height) {
        const { cellSize, rows, cols, fullCols, fullRows, lastColWidth, lastRowHeight, hasExtraCol, hasExtraRow, gridWidth, gridHeight, } = computeGrid(width, height);
        const cellWidth = cellSize;
        const cellHeight = cellSize;
        const canvasWidth = context.canvas.width;
        const canvasHeight = context.canvas.height;
        const mapTopScreenRaw = this.transformHandler.worldToScreenCoordinates(new Cell(0, 0)).y;
        const mapBottomScreenRaw = this.transformHandler.worldToScreenCoordinates(new Cell(0, height)).y;
        const mapLeftScreenRaw = this.transformHandler.worldToScreenCoordinates(new Cell(0, 0)).x;
        const mapRightScreenRaw = this.transformHandler.worldToScreenCoordinates(new Cell(width, 0)).x;
        ...
        for (let col = 0; col <= fullCols; col++) {
            ...
            const screenX = this.transformHandler.worldToScreenCoordinates(new Cell(worldX, mapTopWorld)).x;
            if (screenX < -1 || screenX > canvasWidth + 1)
                continue;
            const screenBottom = this.transformHandler.worldToScreenCoordinates(new Cell(worldX, gridHeight)).y;
        ...
        if (!hasExtraCol) {
            const mapRightLine = this.transformHandler.worldToScreenCoordinates(new Cell(gridWidth, mapTopWorld)).x;
            context.moveTo(mapRightLine, mapTopScreen);
            context.lineTo(mapRightLine, this.transformHandler.worldToScreenCoordinates(new Cell(gridWidth, gridHeight)).y);
        }
        for (let row = 0; row <= fullRows; row++) {
            const worldY = row * cellHeight + mapTopWorld;
            const screenY = this.transformHandler.worldToScreenCoordinates(new Cell(mapLeftWorld, worldY)).y;
            if (screenY < -1 || screenY > canvasHeight + 1)
                continue;
            const screenRight = this.transformHandler.worldToScreenCoordinates(new Cell(gridWidth, worldY)).x;
        ...
        if (!hasExtraRow) {
            const mapBottomLine = this.transformHandler.worldToScreenCoordinates(new Cell(mapLeftWorld, gridHeight)).y;
            context.moveTo(mapLeftScreen, mapBottomLine);
            context.lineTo(this.transformHandler.worldToScreenCoordinates(new Cell(gridWidth, gridHeight)).x, mapBottomLine);
        }
        context.stroke();
        ...
        const fontSize = Math.min(16, Math.max(9, 10 + (this.transformHandler.scale - 1) * 1.2));
        ...
        for (let row = 0; row < rows; row++) {
            ...
            const screenY = this.transformHandler.worldToScreenCoordinates(new Cell(0, centerY)).y;
            ...
            for (let col = 0; col < cols; col++) {
                ...
                const screenX = this.transformHandler.worldToScreenCoordinates(new Cell(centerX, centerY)).x;
                if (screenX < -LABEL_PADDING || screenX > canvasWidth + LABEL_PADDING)
                    continue;
                // Position at cell top-left in screen space
                const cellTopLeft = this.transformHandler.worldToScreenCoordinates(new Cell(startX, startY));
                drawLabel(`${rowLabel}${col + 1}`, cellTopLeft.x + LABEL_PADDING, cellTopLeft.y + LABEL_PADDING);
            }
        }
        context.restore();
    }
}
//# sourceMappingURL=CoordinateGridLayer.js.map