...
import { CloseViewEvent, MouseMoveEvent, MouseUpEvent, UnitSelectionEvent, } from "../../InputHandler";
import { LaunchAirMissionIntentEvent } from "../../Transport";
export class AirMissionLayer {
    ...
    renderLayer(context) {
        ...
        context.beginPath();
        context.arc(centerX, centerY, this.game.config().airbaseOperationalRange(), 0, Math.PI * 2);
        context.fill();
        context.stroke();
        context.setLineDash([]);
    ...
    handleMouseUp(event) {
        ...
        if (this.uiState.pendingAirMission !== null && this.selectedAirbase) {
            this.eventBus.emit(new LaunchAirMissionIntentEvent(this.uiState.pendingAirMission.airbaseId, this.uiState.pendingAirMission.squadronType, this.uiState.pendingAirMission.missionType, tile));
            this.uiState.pendingAirMission = null;
            return;
        }
        ...
        const airbase = this.game
            ...
            .sort((a, b) => this.game.manhattanDist(a.tile(), tile) -
            this.game.manhattanDist(b.tile(), tile) || a.id() - b.id())[0];
        if (!airbase) {
            if (this.selectedAirbase) {
                this.eventBus.emit(new UnitSelectionEvent(this.selectedAirbase, false));
            }
    ...
    hoveredTile() {
        ...
        const cell = this.transformHandler.screenToWorldCoordinates(this.mouseScreen.x - rect.left, this.mouseScreen.y - rect.top);
        if (!this.game.isValidCoord(cell.x, cell.y)) {
            return null;
        }
        return this.game.ref(cell.x, cell.y);
...
//# sourceMappingURL=AirMissionLayer.js.map