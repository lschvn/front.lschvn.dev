...
import {
  attackModeTroopCommitment,
  DEFAULT_ATTACK_MODE,
} from "../../../core/configuration/AttackModeBalance";
import { EventBus } from "../../../core/EventBus";
import { AttackMode, Gold } from "../../../core/game/Game";
...
const ATTACK_MODE_UI: Array<{
...
}> = [
  {
    ...
    icon: blitzIcon,
    description:
      "Fast commitment and capture. Higher attacker losses and weaker into fortified lines.",
  },
  {
    ...
    icon: siegeIcon,
    description:
      "Slow advance with lower losses. Suppresses frontline defenses and grinds nearby buildings.",
  },
  {
    ...
    icon: raidIcon,
    description:
      "Low capture priority. Disrupts ports, factories, cities, trade ships, and transports when exposed.",
  },
];
...
export class ControlPanel extends LitElement implements Layer {
  ...
  @state()
  private attackMode: AttackMode = DEFAULT_ATTACK_MODE;
  ...
  init() {
    ...
    if (
      savedMode &&
      Object.values(AttackMode).includes(savedMode as AttackMode)
    ) {
      this.attackMode = savedMode as AttackMode;
    }
  ...
  private cycleAttackMode() {
    const currentIndex = ATTACK_MODE_UI.findIndex(
      ({ mode }) => mode === this.attackMode,
    );
    const next =
      ATTACK_MODE_UI[(currentIndex + 1 + ATTACK_MODE_UI.length) % ATTACK_MODE_UI.length];
  ...
  private committedTroopsPreview(): number {
    return attackModeTroopCommitment(
  ...
  private renderAttackModes() {
    return html`
      <div class="grid grid-cols-3 gap-1.5">
        ${ATTACK_MODE_UI.map(
          ({ mode, label, icon, description }) => html`
            <button
