...
import { Cell, ColoredTeams, PlayerType, UnitType, } from "../../../core/game/Game";
import { euclDistFN } from "../../../core/game/GameMap";
import { GameUpdateType } from "../../../core/game/GameUpdates";
import { PlayerView } from "../../../core/game/GameView";
import { PseudoRandom } from "../../../core/PseudoRandom";
import { AlternateViewEvent, DragEvent, MouseOverEvent, } from "../../InputHandler";
import { FrameProfiler } from "../FrameProfiler";
export class TerritoryLayer {
    constructor(game, eventBus, transformHandler, userSettings) {
        this.game = game;
        this.eventBus = eventBus;
        this.transformHandler = transformHandler;
        this.borderAnimTime = 0;
        this.tileToRenderQueue = new PriorityQueue((a, b) => {
            return a.lastUpdate - b.lastUpdate;
        });
        this.random = new PseudoRandom(123);
        this.suppressionColor = new Colord("#f59e0b");
        this.highlightedTerritory = null;
        this.alternativeView = false;
        this.lastDragTime = 0;
        this.nodrawDragDuration = 200;
        this.lastMousePosition = null;
        this.refreshRate = 10; //refresh every 10ms
        this.lastRefresh = 0;
    ...
    async paintPlayerBorder(player) {
        const tiles = await player.borderTiles();
        tiles.borderTiles.forEach((tile) => {
        ...
        if (myPlayer) {
            ...
            updates?.[GameUpdateType.AllianceRequestReply]?.forEach((update) => {
                if (update.accepted &&
                    ...
                    const territoryId = update.request.requestorID === myPlayer.smallID()
    ...
    spawnHighlight() {
        ...
        for (const human of humans) {
            ...
            for (const tile of this.game.bfs(centerTile, euclDistFN(centerTile, 9, true))) {
    ...
    drawFocusedPlayerHighlight() {
        ...
        const radius = minRad + (maxRad - minRad) * (0.5 + 0.5 * Math.sin(this.borderAnimTime));
        const baseColor = this.theme.spawnHighlightSelfColor(); //white
        let teamColor = null;
        const team = focusedPlayer.team();
        ...
        this.drawBreathingRing(center.x, center.y, minRad, maxRad, radius, baseColor, // Always draw white static semi-transparent ring
        teamColor);
    ...
    drawTeammateHighlights(minRad, maxRad, radius) {
        const myPlayer = this.game.myPlayer();
        ...
        const teammateRadius = teammateMinRad +
        ...
        for (const teammate of teammates) {
            ...
            this.drawBreathingRing(center.x, center.y, teammateMinRad, teammateMaxRad, teammateRadius, baseColor, breathingColor);
        }
    }
    init() {
    ...
    updateHighlightedTerritory() {
        ...
        const cell = this.transformHandler.screenToWorldCoordinates(this.lastMousePosition.x, this.lastMousePosition.y);
    ...
    redraw() {
        ...
        this.alternativeImageData = this.context.getImageData(0, 0, this.canvas.width, this.canvas.height);
        this.initImageData();
        this.context.putImageData(this.alternativeView ? this.alternativeImageData : this.imageData, 0, 0);
    ...
    renderLayer(context) {
        const now = Date.now();
        if (now > this.lastDragTime + this.nodrawDragDuration &&
            ...
            if (w > 0 && h > 0) {
                ...
                this.context.putImageData(this.alternativeView ? this.alternativeImageData : this.imageData, 0, 0, vx0, vy0, w, h);
                FrameProfiler.end("TerritoryLayer:putImageData", putImageStart);
            }
        }
        const drawCanvasStart = FrameProfiler.start();
        context.drawImage(this.canvas, -this.game.width() / 2, -this.game.height() / 2, this.game.width(), this.game.height());
        FrameProfiler.end("TerritoryLayer:drawCanvas", drawCanvasStart);
        if (this.game.inSpawnPhase()) {
            const highlightDrawStart = FrameProfiler.start();
            context.drawImage(this.highlightCanvas, -this.game.width() / 2, -this.game.height() / 2, this.game.width(), this.game.height());
            FrameProfiler.end("TerritoryLayer:drawHighlightCanvas", highlightDrawStart);
        }
    }
    renderTerritory() {
    ...
    paintTerritory(tile, isBorder = false) {
        ...
        if (!this.game.hasOwner(tile)) {
            if (this.game.hasFallout(tile)) {
                ...
                this.paintTile(this.alternativeImageData, tile, this.theme.falloutColor(), 150);
                return;
            }
            this.clearTile(tile);
            return;
        ...
        const owner = this.game.owner(tile);
        const isSuppressed = this.game.hasSuppression(tile);
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const isHighlighted = this.highlightedTerritory &&
        ...
        if (this.game.isBorder(tile)) {
            ...
            if (myPlayer) {
                const alternativeColor = isSuppressed
                    ? this.suppressedTint(this.alternateViewColor(owner))
                    : this.alternateViewColor(owner);
                this.paintTile(this.alternativeImageData, tile, alternativeColor, 255);
            }
            const isDefended = !isSuppressed &&
                this.game.hasUnitNearby(tile, this.game.config().defensePostRange(), UnitType.DefensePost, owner.id());
            this.paintTile(this.imageData, tile, isSuppressed
                ? this.suppressedTint(owner.borderColor(tile, false))
                : owner.borderColor(tile, isDefended), 255);
        }
        else {
            // Alternative view only shows borders.
            this.clearAlternativeTile(tile);
            this.paintTile(this.imageData, tile, isSuppressed
                ? this.suppressedTint(owner.territoryColor(tile))
                : owner.territoryColor(tile), isSuppressed ? 190 : 150);
        }
    }
    suppressedTint(color) {
        const base = color.toRgb();
        const target = this.suppressionColor.toRgb();
        return new Colord({
            r: Math.round(base.r * 0.55 + target.r * 0.45),
            g: Math.round(base.g * 0.55 + target.g * 0.45),
            b: Math.round(base.b * 0.55 + target.b * 0.45),
            a: base.a,
        });
    }
    alternateViewColor(other) {
        const myPlayer = this.game.myPlayer();
        if (!myPlayer) {
    ...
    async enqueuePlayerBorder(player) {
        const playerBorderTiles = await player.borderTiles();
        playerBorderTiles.borderTiles.forEach((tile) => {
            this.enqueueTile(tile);
        });
    }
    paintHighlightTile(tile, color, alpha) {
        this.clearTile(tile);
    ...
    clearHighlightTile(tile) {
        const x = this.game.x(tile);
        const y = this.game.y(tile);
        this.highlightContext.clearRect(x, y, 1, 1);
    }
    drawBreathingRing(cx, cy, minRad, maxRad, radius, transparentColor, breathingColor) {
...
//# sourceMappingURL=TerritoryLayer.js.map