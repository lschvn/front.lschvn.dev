...
export class UILayer {
    constructor(game, eventBus, transformHandler) {
        ...
        this.allProgressBars = new Map();
        this.allHealthBars = new Map();
        ...
        this.SELECTION_BOX_SIZE = 6; // Size of the selection box (should be larger than the warship)
    ...
    drawSelectionBox(unit) {
        ...
        const opacity = baseOpacity + Math.sin(this.selectionAnimTime * 0.1) * pulseAmount;
    ...
    drawHealthBar(unit) {
        ...
        if (this.allHealthBars.has(unit.id()) &&
            ...
            unit.health() > 0) {
            this.allHealthBars.get(unit.id())?.clear();
            const healthBar = new ProgressBar(COLOR_PROGRESSION, this.context, this.game.x(unit.tile()) - 4, this.game.y(unit.tile()) - 6, HEALTHBAR_WIDTH, PROGRESSBAR_HEIGHT, unit.health() / maxHealth);
    ...
    getProgress(unit) {
        ...
        const underConst = unit.isUnderConstruction();
        if (underConst) {
            const constDuration = this.game.unitInfo(unit.type()).constructionDuration;
            if (constDuration === undefined) {
                throw new Error("unit does not have constructionTime");
            }
            return ((this.game.ticks() - unit.createdAt()) /
    ...
    deletionProgress(game, unit) {
        const deleteAt = unit.markedForDeletion();
        if (deleteAt === false)
            return 1;
        return Math.max(0, (deleteAt - game.ticks()) / game.config().deletionMarkDuration());
    }
    createLoadingBar(unit) {
        if (!this.context) {
            return;
        }
        if (!this.allProgressBars.has(unit.id())) {
            const progressBar = new ProgressBar(COLOR_PROGRESSION, this.context, this.game.x(unit.tile()) - 6, this.game.y(unit.tile()) + 6, LOADINGBAR_WIDTH, PROGRESSBAR_HEIGHT, 0);
