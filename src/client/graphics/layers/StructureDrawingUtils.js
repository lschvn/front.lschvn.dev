...
export class SpriteFactory {
    constructor(theme, game, transformHandler, renderSprites) {
    ...
    loadIcon(unitInfo, unitType) {
        ...
        image.onerror = () => {
            console.error(`Failed to load icon for ${unitType}: ${unitInfo.iconPath}`);
        };
    }
    invalidateTextureCache(unitType) {
    ...
    createGhostContainer(player, ghostStage, pos, structureType) {
        const parentContainer = new PIXI.Container();
        const texture = this.createTexture(structureType, player, false, false, true);
        ...
        const boxHeight = 18;
        const boxY = (sprite.height > 0 ? sprite.height / 2 : 16) + boxHeight / 2 + 4;
        ...
        parentContainer.scale.set(Math.min(1, this.transformHandler.scale / ICON_SCALE_FACTOR_ZOOMED_OUT));
    ...
    // --- internal helpers ---
    createUnitContainer(unit, options) {
        const parentContainer = new PIXI.Container();
        ...
        if (type === "icon" || type === "dot") {
            const texture = this.createTexture(structureType, unit.owner(), isConstruction, isMarkedForDeletion, type === "icon");
            const sprite = new PIXI.Sprite(texture);
            sprite.anchor.set(0.5);
            parentContainer.addChild(sprite);
        }
        ...
        if (type === "icon") {
            const s = scale >= ZOOM_THRESHOLD && !this.renderSprites
    ...
    createTexture(type, owner, isConstruction, isMarkedForDeletion, renderIcon) {
        ...
        const texture = shape
            ? this.createIcon(owner, type, isConstruction, isMarkedForDeletion, shape, renderIcon)
            : PIXI.Texture.EMPTY;
        this.textureCache.set(cacheKey, texture);
        return texture;
    }
    createIcon(owner, structureType, isConstruction, isMarkedForDeletion, shape, renderIcon) {
        ...
        switch (shape) {
            ...
            case "circle":
                context.beginPath();
                context.arc(halfIconSize, halfIconSize, halfIconSize - 1, 0, Math.PI * 2);
                context.fill();
                context.stroke();
                break;
            default:
        ...
        if (structureInfo?.image && renderIcon) {
            ...
            context.drawImage(this.getImageColored(structureInfo.image, owner.structureColors().dark.toRgbString()), offsetX, offsetY);
        }
    ...
    createRange(type, stage, pos, level, targetingAlly = false) {
        if (stage === undefined)
        ...
        switch (type) {
            ...
            case UnitType.Airbase:
                radius = this.game.config().airbaseOperationalRange();
                break;
...
//# sourceMappingURL=StructureDrawingUtils.js.map