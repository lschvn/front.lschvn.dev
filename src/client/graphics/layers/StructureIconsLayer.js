...
import { Cell, Structures, UnitType, } from "../../../core/game/Game";
import { GameUpdateType } from "../../../core/game/GameUpdates";
import { ConfirmGhostStructureEvent, GhostStructureChangedEvent, MouseMoveEvent, MouseUpEvent, ToggleStructureEvent as ToggleStructuresEvent, } from "../../InputHandler";
import { BuildUnitIntentEvent, SendUpgradeStructureIntentEvent, } from "../../Transport";
import { renderNumber } from "../../Utils";
import { DOTS_ZOOM_THRESHOLD, ICON_SCALE_FACTOR_ZOOMED_IN, ICON_SCALE_FACTOR_ZOOMED_OUT, ICON_SIZE, LEVEL_SCALE_FACTOR, OFFSET_ZOOM_Y, SpriteFactory, STRUCTURE_SHAPES, ZOOM_THRESHOLD, } from "./StructureDrawingUtils";
...
export function shouldPreserveGhostAfterBuild(unitType) {
    return unitType === UnitType.AtomBomb || unitType === UnitType.HydrogenBomb;
}
extend([a11yPlugin]);
class StructureRenderInfo {
    constructor(unit, owner, iconContainer, levelContainer, dotContainer, level = 0, underConstruction = true, suppressed = false) {
        this.unit = unit;
        this.owner = owner;
...
export class StructureIconsLayer {
    constructor(game, eventBus, uiState, transformHandler) {
        this.game = game;
        ...
        this.rendersByUnitId = new Map();
        this.seenUnitIds = new Set();
        this.connectedAllySmallIds = new Set();
        this.mousePos = { x: 0, y: 0 };
        this.renderSprites = true;
        this.structures = new Map(Structures.types.map((type) => [type, { visible: true }]));
        this.visibilityStateDirty = true;
        this.pendingConfirm = null;
        this.hasHiddenStructure = false;
        this.theme = game.config().theme();
        this.factory = new SpriteFactory(this.theme, game, transformHandler, this.renderSprites);
    }
    async setupRenderer() {
        try {
        ...
        this.rootStage.addChild(this.dotsStage, this.iconsStage, this.levelsStage, this.ghostStage);
    ...
    async init() {
        this.eventBus.on(ToggleStructuresEvent, (e) => this.toggleStructures(e.structureTypes));
        this.eventBus.on(MouseMoveEvent, (e) => this.moveGhost(e));
        this.eventBus.on(MouseUpEvent, (e) => this.requestConfirmStructure(e));
        this.eventBus.on(ConfirmGhostStructureEvent, () => this.requestConfirmStructure(new MouseUpEvent(this.mousePos.x, this.mousePos.y)));
    ...
    renderLayer(mainContext) {
        ...
        if (this.ghostUnit) {
            ...
            else if (this.uiState.ghostStructure !== this.ghostUnit.buildableUnit.type) {
    ...
    renderGhost() {
        ...
        this.game
            ...
            const unit = buildables.find((u) => u.type === this.ghostUnit.buildableUnit.type);
    ...
    updateGhostPrice(cost, showPrice) {
        if (!this.ghostUnit)
            return;
        ...
        const boxWidth = Math.max(priceBox.minWidth, textWidth + priceBox.paddingX * 2);
        priceBg.clear();
        priceBg
            .roundRect(-boxWidth / 2, priceBox.y - priceBox.height / 2, boxWidth, priceBox.height, 4)
     ...
     */
    requestConfirmStructure(e) {
        if (!this.ghostUnit && !this.uiState.ghostStructure)
            return;
        if (this.isGhostReadyForConfirm()) {
    ...
    createStructure(e) {
        if (!this.ghostUnit)
            return;
        if (this.ghostUnit.buildableUnit.canBuild === false &&
        ...
        if (this.ghostUnit.buildableUnit.canUpgrade !== false) {
            this.eventBus.emit(new SendUpgradeStructureIntentEvent(this.ghostUnit.buildableUnit.canUpgrade, this.ghostUnit.buildableUnit.type));
            this.removeGhostStructure();
        }
        else if (this.ghostUnit.buildableUnit.canBuild) {
            const unitType = this.ghostUnit.buildableUnit.type;
            const rocketDirectionUp = unitType === UnitType.AtomBomb || unitType === UnitType.HydrogenBomb
                ? this.uiState.rocketDirectionUp
                : undefined;
            this.eventBus.emit(new BuildUnitIntentEvent(unitType, this.game.ref(tile.x, tile.y), rocketDirectionUp));
    ...
    createGhostStructure(type) {
        ...
        const localX = this.mousePos.x - rect.left;
        const localY = this.mousePos.y - rect.top;
        const ghost = this.factory.createGhostContainer(player, this.ghostStage, { x: localX, y: localY }, type);
        this.ghostUnit = {
    ...
    resolveGhostRangeLevel(buildableUnit) {
    ...
    updateGhostRange(level, targetingAlly = false) {
        if (!this.ghostUnit) {
            return;
        }
        if (this.ghostUnit.range &&
        ...
        const range = this.factory.createRange(this.ghostUnit.buildableUnit.type, this.ghostStage, { x: position.x, y: position.y }, level, targetingAlly);
        if (range) {
            this.ghostUnit.range = range;
        }
    }
    toggleStructures(toggleStructureType) {
    ...
    findRenderByUnit(unitView) {
        return this.rendersByUnitId.get(unitView.id());
    }
    handleActiveUnit(unitView) {
        if (this.seenUnitIds.has(unitView.id())) {
            ...
            if (render) {
                ...
                this.checkForSuppressionState(render, unitView);
            }
        }
    ...
    modifyVisibility(render) {
        ...
        if (structureInfos) {
            ...
            if (structureInfos.visible && this.hasHiddenStructure) {
                filters.push(new OutlineFilter({ thickness: 2, color: "rgb(255, 255, 255)" }));
            }
            if (render.suppressed) {
                filters.push(new OutlineFilter({ thickness: 2, color: "rgb(245, 158, 11)" }));
            }
            render.iconContainer.filters = filters;
            render.dotContainer.filters = filters;
        }
    }
    checkForDeletionState(render, unit) {
    ...
    checkForConstructionState(render, unit) {
    ...
    checkForOwnershipChange(render, unit) {
    ...
    checkForLevelChange(render, unit) {
    ...
    checkForSuppressionState(render, unit) {
        if (render.suppressed !== unit.isSuppressed()) {
            render.suppressed = unit.isSuppressed();
            this.modifyVisibility(render);
        }
    }
    computeNewLocation(render) {
        ...
        const scale = this.transformHandler.scale;
        screenPos.y = Math.round(scale >= ZOOM_THRESHOLD &&
            this.game.config().userSettings()?.structureSprites()
            ? screenPos.y - scale * OFFSET_ZOOM_Y
            : screenPos.y);
        const type = render.unit.type();
        const margin = type !== undefined && STRUCTURE_SHAPES[type] !== undefined
            ? ICON_SIZE[STRUCTURE_SHAPES[type]]
            : 28;
        const onScreen = screenPos.x + margin > 0 &&
        ...
        if (onScreen) {
            if (scale > ZOOM_THRESHOLD) {
                ...
                target.scale.set(Math.max(1, scale /
                ...
                render.iconContainer.scale.set(Math.min(1, scale / ICON_SCALE_FACTOR_ZOOMED_OUT));
            }
            else {
    ...
    addNewStructure(unitView) {
        this.seenUnitIds.add(unitView.id());
        const render = new StructureRenderInfo(unitView, unitView.owner().id(), this.createIconSprite(unitView), this.createLevelSprite(unitView), this.createDotSprite(unitView), unitView.level(), unitView.isUnderConstruction(), unitView.isSuppressed());
        this.rendersByUnitId.set(unitView.id(), render);
        this.computeNewLocation(render);
        this.modifyVisibility(render);
    }
    createLevelSprite(unit) {
        return this.factory.createUnitContainer(unit, {
            type: "level",
            stage: this.levelsStage,
        });
    }
    createDotSprite(unit) {
        return this.factory.createUnitContainer(unit, {
            type: "dot",
            stage: this.dotsStage,
        });
    }
    createIconSprite(unit) {
        return this.factory.createUnitContainer(unit, {
            type: "icon",
            stage: this.iconsStage,
        });
    }
    deleteStructure(render) {
        render.iconContainer?.destroy();
        render.levelContainer?.destroy();
        render.dotContainer?.destroy();
...
//# sourceMappingURL=StructureIconsLayer.js.map