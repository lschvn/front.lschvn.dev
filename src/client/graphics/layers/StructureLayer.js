...
const suppressedColor = colord("#f59e0b");
...
export class StructureLayer {
    ...
    loadIcon(unitType, config) {
        const image = new Image();
        image.src = config.icon;
        image.onload = () => {
            this.unitIcons.set(unitType, image);
            console.log(`icon loaded: ${unitType}, size: ${image.width}x${image.height}`);
        };
        image.onerror = () => {
    ...
    redraw() {
        ...
        Promise.all(Array.from(this.unitIcons.values()).map((img) => img.decode?.().catch((err) => {
    ...
    isUnitTypeSupported(unitType) {
        return unitType in this.unitConfigs;
    }
    drawBorder(unit, borderColor, config) {
        // Draw border and territory
        for (const tile of this.game.bfs(unit.tile(), isometricDistFN(unit.tile(), config.borderRadius, true))) {
            this.paintCell(new Cell(this.game.x(tile), this.game.y(tile)), borderColor, 255);
        }
        for (const tile of this.game.bfs(unit.tile(), isometricDistFN(unit.tile(), config.territoryRadius, true))) {
            this.paintCell(new Cell(this.game.x(tile), this.game.y(tile)), unit.isUnderConstruction()
                ? underConstructionColor
                : unit.owner().territoryColor(), 130);
        }
    }
    handleUnitRendering(unit) {
        ...
        if (unit.isSuppressed()) {
            borderColor = suppressedColor;
        }
        if (!config || !icon)
            return;
        // Clear previous rendering
        for (const tile of this.game.bfs(unit.tile(), euclDistFN(unit.tile(), config.borderRadius + 1, true))) {
    ...
    renderIcon(image, startX, startY, width, height, unit) {
...
//# sourceMappingURL=StructureLayer.js.map