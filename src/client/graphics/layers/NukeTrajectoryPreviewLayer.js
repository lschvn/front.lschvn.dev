...
import { GhostStructureChangedEvent, MouseMoveEvent, SwapRocketDirectionEvent, } from "../../InputHandler";
/**
 ...
 */
export class NukeTrajectoryPreviewLayer {
    ...
    init() {
        ...
        this.eventBus.on(GhostStructureChangedEvent, (e) => {
            ...
            if (e.ghostStructure !== UnitType.AtomBomb &&
    ...
    updateTrajectoryPreview() {
        ...
        const isNukeType = ghostStructure === UnitType.AtomBomb ||
        ...
        const worldCoords = this.transformHandler.screenToWorldCoordinates(localX, localY);
        ...
        player
            ...
            const buildableUnit = buildables.find((bu) => bu.type === ghostStructure);
    ...
    updateTrajectoryPath() {
        ...
        for (let i = 0; i < this.trajectoryPoints.length; i++) {
            ...
            if (this.untargetableSegmentBounds[0] === -1) {
                if (this.game.euclideanDistSquared(tile, this.cachedSpawnTile) >
                    targetRangeSquared) {
        ...
        const playersToBreakAllianceWith = listNukeBreakAlliance({
            ...
            allySmallIds: new Set(this.game
                .myPlayer()
                ?.allies()
                .map((a) => a.smallID())),
        ...
        for (let i = 0; i < this.trajectoryPoints.length; i++) {
            const tile = this.trajectoryPoints[i];
            for (const sam of this.game.nearbyUnits(tile, this.game.config().maxSamRange(), UnitType.SAMLauncher)) {
                if (sam.unit.owner().isMe() ||
            ...
            if (this.targetedIndex !== this.trajectoryPoints.length)
                break;
    ...
    drawTrajectoryPreview(context) {
        ...
        const drawUntargetableCircle = (x, y) => {
            context.beginPath();
...
//# sourceMappingURL=NukeTrajectoryPreviewLayer.js.map