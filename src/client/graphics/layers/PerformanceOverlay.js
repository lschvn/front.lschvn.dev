...
var PerformanceOverlay_1;
import { LitElement, css, html } from "lit";
import { customElement, property, state } from "lit/decorators.js";
import { TickMetricsEvent, TogglePerformanceOverlayEvent, } from "../../InputHandler";
import { translateText } from "../../Utils";
import { FrameProfiler } from "../FrameProfiler";
let PerformanceOverlay = PerformanceOverlay_1 = class PerformanceOverlay extends LitElement {
    ensureUiText() {
        const selector = this.langSelector && this.langSelector.isConnected
            ? this.langSelector
        ...
        this.uiText = {
            ...
            renderLayersTableHeader: translateText("performance_overlay.render_layers_table_header"),
            tickLayersTableHeader: translateText("performance_overlay.tick_layers_table_header"),
        };
    }
    static computeLayerBreakdown(stats) {
    ...
    constructor() {
        ...
        this.position = { x: 8, y: 8 }; // px values
        ...
        this.renderPerTickLayerStats = new Map();
        this.langSelector = null;
        this.uiTextLang = null;
        this.uiTextTranslationsRef = undefined;
        this.uiTextDefaultTranslationsRef = undefined;
        this.uiText = {
        ...
        this.onTogglePerformanceOverlay = (_event) => {
            const nextVisible = !this.isVisible;
            this.setVisible(nextVisible);
            this.userSettings.set("settings.performanceOverlay", nextVisible);
        };
        this.onTickMetricsEvent = (event) => {
            this.updateTickMetrics(event.tickExecutionDuration, event.tickDelay);
        };
        this.onUserSettingsChanged = (event) => {
            const customEvent = event;
            if (customEvent.detail?.key !== "settings.performanceOverlay")
                return;
        ...
        this.onDragPointerMove = (e) => {
            ...
            const overlayWidth = Math.min(this.overlayWidthPx ?? defaultWidth, viewportWidth - margin * 2);
            this.position = {
                x: Math.max(margin, Math.min(viewportWidth - overlayWidth - margin, newX)),
        ...
        this.onResizePointerMove = (e) => {
            if (!this.resizeState || e.pointerId !== this.resizeState.pointerId)
                return;
            const margin = 8;
            const viewportWidth = window.innerWidth;
            ...
            const overlay = this.renderRoot.querySelector(".performance-overlay");
        ...
        this.handleResizePointerDown = (e) => {
            e.preventDefault();
            e.stopPropagation();
            const overlay = this.renderRoot.querySelector(".performance-overlay");
        ...
        this.toggleRenderLayersExpanded = (e) => {
            e.preventDefault();
            e.stopPropagation();
            this.renderLayersExpanded = !this.renderLayersExpanded;
        };
        this.toggleTickLayersExpanded = (e) => {
            e.preventDefault();
            e.stopPropagation();
    ...
    init() {
        ...
        if (this.subscribedEventBus && this.subscribedEventBus !== this.eventBus) {
            this.subscribedEventBus.off(TogglePerformanceOverlayEvent, this.onTogglePerformanceOverlay);
            this.subscribedEventBus.off(TickMetricsEvent, this.onTickMetricsEvent);
            this.subscribedEventBus = null;
        }
        if (this.subscribedEventBus !== this.eventBus) {
            this.eventBus.on(TogglePerformanceOverlayEvent, this.onTogglePerformanceOverlay);
        ...
        if (!this.isUserSettingsListenerAttached) {
            globalThis.addEventListener("user-settings-changed", this.onUserSettingsChanged);
    ...
    disconnectedCallback() {
        ...
        if (this.isUserSettingsListenerAttached) {
            globalThis.removeEventListener("user-settings-changed", this.onUserSettingsChanged);
            this.isUserSettingsListenerAttached = false;
        }
    ...
    updateFrameMetrics(frameDuration, layerDurations) {
        if (!this.isVisible)
            return;
        ...
        if (now - this.lastSecondTime >= 1000) {
            ...
            if (this.fpsHistory.length > 60) {
                ...
                if (removed !== undefined)
                    this.fpsHistorySum -= removed;
            }
            // Calculate 60-second average
            if (this.fpsHistory.length > 0) {
                this.averageFPS = Math.round(this.fpsHistorySum / this.fpsHistory.length);
            }
            this.framesThisSecond = 0;
            this.lastSecondTime = now;
        }
    ...
    updateLayerStats(layerDurations) {
    ...
    updateRenderPerTickMetrics(frameCount, layerDurations) {
        if (!this.isVisible)
            return;
        const alpha = 0.2; // smoothing factor for EMA
        this.renderLastTickFrameCount = frameCount;
        this.renderLastTickLayerDurations = layerDurations;
        this.renderLastTickLayerTotalMs = Object.values(layerDurations).reduce((acc, ms) => acc + ms, 0);
    ...
    updateTickLayerMetrics(tickLayerDurations) {
        if (!this.isVisible)
            return;
        const alpha = 0.2; // smoothing factor for EMA
    ...
    updateTickMetrics(tickExecutionDuration, tickDelay) {
        if (!this.isVisible)
            return;
        const now = performance.now();
        this.tickTimestamps.push(now);
        while (this.tickHead1s < this.tickTimestamps.length &&
            now - this.tickTimestamps[this.tickHead1s] > 1000) {
            this.tickHead1s++;
        }
        while (this.tickHead60s < this.tickTimestamps.length &&
        ...
        const oldest60 = ticksLast60s > 0 ? this.tickTimestamps[this.tickHead60s] : now;
        ...
        if (tickExecutionDuration !== undefined) {
            ...
            if (this.tickExecutionTimes.length > 60) {
                ...
                if (removed !== undefined)
                    this.tickExecutionTimesSum -= removed;
            }
            if (this.tickExecutionTimes.length > 0) {
                ...
                let max = 0;
                for (const v of this.tickExecutionTimes)
                    max = Math.max(max, v);
        ...
        if (tickDelay !== undefined) {
            ...
            if (this.tickDelayTimes.length > 60) {
                ...
                if (removed !== undefined)
                    this.tickDelayTimesSum -= removed;
            }
            if (this.tickDelayTimes.length > 0) {
                ...
                let max = 0;
                for (const v of this.tickDelayTimes)
                    max = Math.max(max, v);
    ...
    buildPerformanceSnapshot() {
        return {
            ...
            layers: PerformanceOverlay_1.computeLayerBreakdown(this.layerStats).map((layer) => ({ ...layer })),
            tickLayers: PerformanceOverlay_1.computeLayerBreakdown(this.tickLayerStats).map((layer) => ({ ...layer })),
        };
    }
    ...
    render() {
        ...
        const clampedY = Math.max(margin, Math.min(this.position.y, viewportHeight - 100));
        const copyLabel = this.copyStatus === "success"
            ? this.uiText.copied
            : this.copyStatus === "error"
                ? this.uiText.failedCopy
                : this.uiText.copyClipboard;
        const renderLayerBreakdown = this.renderLayersExpanded
            ? PerformanceOverlay_1.computeLayerBreakdown(this.layerStats)
            : [];
        const tickLayerBreakdown = this.tickLayersExpanded
            ? PerformanceOverlay_1.computeLayerBreakdown(this.tickLayerStats)
            : [];
        const renderLayersToShow = renderLayerBreakdown.slice(0, 10);
        const tickLayersToShow = tickLayerBreakdown.slice(0, 10);
        const maxLayerAvg = renderLayersToShow.length > 0
            ? Math.max(...renderLayersToShow.map((l) => l.avg))
            : 1;
        const maxTickLayerAvg = tickLayersToShow.length > 0
            ? Math.max(...tickLayersToShow.map((l) => l.avg))
            : 1;
        const overlayWidthStyle = this.overlayWidthPx === null
            ? ""
          ...
          ${this.layerStats.size
            ? html `<div class="layers-section">
                ...
                ? html `<div class="layer-row table-header" style="--pct: 0%;">
                    ...
                    const width = Math.min(100, (layer.avg / maxLayerAvg) * 100 || 0);
                    const perTickRenderMs = this.renderLastTickLayerDurations[layer.name] ?? 0;
                    const perTickRenderAvgMs = this.renderPerTickLayerStats.get(layer.name)?.avg ??
                        0;
                    const isInactive = perTickRenderMs <= 0.01;
                    const title = `${layer.name} | last tick render: ${perTickRenderMs.toFixed(2)}ms`;
                    return html `<div
          ...
          ${this.tickLayerStats.size
            ? html `<div class="layers-section">
                ...
                ? html `<div class="layer-row table-header" style="--pct: 0%;">
                    ...
                    const width = Math.min(100, (layer.avg / maxTickLayerAvg) * 100 || 0);
...
], PerformanceOverlay.prototype, "eventBus", void 0);
__decorate([
    property({ type: Object })
], PerformanceOverlay.prototype, "userSettings", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "currentFPS", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "averageFPS", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "frameTime", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "tickExecutionAvg", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "tickExecutionMax", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "tickDelayAvg", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "tickDelayMax", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "isVisible", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "currentTPS", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "averageTPS", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "isDragging", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "position", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "copyStatus", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "renderLayersExpanded", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "tickLayersExpanded", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "overlayWidthPx", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "tickLayerLastCount", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "tickLayerLastTotalMs", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "tickLayerLastDurations", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "renderLastTickFrameCount", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "renderLastTickLayerTotalMs", void 0);
__decorate([
    state()
], PerformanceOverlay.prototype, "renderLastTickLayerDurations", void 0);
PerformanceOverlay = PerformanceOverlay_1 = __decorate([
    customElement("performance-overlay")
], PerformanceOverlay);
export { PerformanceOverlay };
//# sourceMappingURL=PerformanceOverlay.js.map