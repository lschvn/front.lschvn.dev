...
import { GoToPlayerEvent, GoToPositionEvent, GoToUnitEvent, } from "./layers/Leaderboard";
export const GOTO_INTERVAL_MS = 16;
export const CAMERA_MAX_SPEED = 15;
export const CAMERA_SMOOTHING = 0.03;
export class TransformHandler {
    constructor(game, eventBus, canvas) {
        this.game = game;
    ...
    handleTransform(context) {
        ...
        // Apply zoom and pan
        context.setTransform(this.scale, 0, 0, this.scale, this.game.width() / 2 - this.offsetX * this.scale, this.game.height() / 2 - this.offsetY * this.scale);
    }
    worldToScreenCoordinates(cell) {
    ...
    screenBoundingRect() {
        ...
        const rightX = (canvasWidth - this.game.width() / 2) / this.scale + this.offsetX;
        const bottomY = (canvasHeight - this.game.height() / 2) / this.scale + this.offsetY;
    ...
    onGoToUnit(event) {
        this.clearTarget();
        this.target = new Cell(this.game.x(event.unit.lastTile()), this.game.y(event.unit.lastTile()));
        this.intervalID = setInterval(() => this.goTo(), GOTO_INTERVAL_MS);
    }
    centerCamera() {
        this.clearTarget();
        const player = this.game.myPlayer();
        if (!player || !player.nameLocation())
            return;
    ...
    goTo() {
        ...
        if (Math.abs(this.target.x - screenX) + Math.abs(this.target.y - screenY) <
            2) {
            this.clearTarget();
            return;
        }
        ...
        this.offsetX += Math.max(Math.min((this.target.x - screenX) * r, CAMERA_MAX_SPEED), -CAMERA_MAX_SPEED);
        this.offsetY += Math.max(Math.min((this.target.y - screenY) * r, CAMERA_MAX_SPEED), -CAMERA_MAX_SPEED);
        this.changed = true;
    }
    onZoom(event) {
        this.clearTarget();
