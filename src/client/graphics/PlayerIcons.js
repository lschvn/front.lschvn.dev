...
export function getFirstPlacePlayer(game) {
    const sorted = game
        .playerViews()
        .sort((a, b) => b.numTilesOwned() - a.numTilesOwned());
    return sorted.length > 0 ? sorted[0] : null;
}
export function getPlayerIcons(params) {
    ...
    if (emojisEnabled) {
        const emojis = player
            .outgoingEmojis()
            .filter((emoji) => emoji.recipientID === AllPlayers ||
...
export function createAllianceProgressIcon(size, fraction, hasExtensionRequest, darkMode) {
    // Wrapper
