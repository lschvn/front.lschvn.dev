...
import bomberSquadronSprite from "/sprites/bomberSquadron.svg?url";
import fighterSquadronSprite from "/sprites/fighterSquadron.svg?url";
...
const SPRITE_CONFIG = {
    ...
    [UnitType.FighterSquadron]: fighterSquadronSprite,
    [UnitType.BomberSquadron]: bomberSquadronSprite,
...
export const loadAllSprites = async () => {
    ...
    let loadedCount = 0;
    await Promise.all(entries.map(async ([unitType, url]) => {
 ...
 */
function trainTypeToSpriteType(unit) {
...
export const isSpriteReady = (unit) => {
    const unitType = unit.type();
    if (unitType === UnitType.Train) {
        const trainType = trainTypeToSpriteType(unit);
 ...
 */
export const colorizeCanvas = (source, colorA, colorB, colorC) => {
...
function computeSpriteKey(unit, territoryColor, borderColor) {
...
export const getColoredSprite = (unit, theme, customTerritoryColor, customBorderColor) => {
    const territoryColor = customTerritoryColor ?? unit.owner().territoryColor();
    const borderColor = customBorderColor ?? unit.owner().borderColor();
    ...
    const coloredCanvas = colorizeCanvas(sprite, territoryColor, borderColor, spawnHighlightColor);
