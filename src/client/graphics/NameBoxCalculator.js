...
export function placeName(game, player) {
    ...
    let center = new Cell(Math.floor(largestRectangle.x + largestRectangle.width / 2 + boundingBox.min.x), Math.floor(largestRectangle.y + largestRectangle.height / 2 + boundingBox.min.y));
...
export function createGrid(game, player, boundingBox, scalingFactor) {
...
export function findLargestInscribedRectangle(grid) {
...
export function largestRectangleInHistogram(widths) {
    const stack = [];
    let maxArea = 0;
    let largestRect = { x: 0, y: 0, width: 0, height: 0 };
...
export function calculateFontSize(rectangle, name) {
...
//# sourceMappingURL=NameBoxCalculator.js.map