...
import { Duos, GameMode, HumansVsNations, Quads, Trios, } from "../core/game/Game";
import { PublicLobbySocket } from "./LobbySocket";
import { terrainMapFileLoader } from "./TerrainMapFileLoader";
import { getMapName, getModifierLabels, renderDuration, translateText, } from "./Utils";
const CARD_BG = "bg-[color-mix(in_oklab,var(--frenchBlue)_70%,black)]";
let GameModeSelector = class GameModeSelector extends LitElement {
    constructor() {
        super(...arguments);
        this.lobbies = null;
        this.mapAspectRatios = new Map();
        this.serverTimeOffset = 0;
        this.defaultLobbyTime = 0;
        this.lobbySocket = new PublicLobbySocket((lobbies) => this.handleLobbiesUpdate(lobbies));
        this.openRankedMenu = () => {
        ...
        this.openSinglePlayerModal = () => {
            if (!this.validateUsername())
                return;
            document.querySelector("single-player-modal")?.open();
        };
        this.openHostLobby = () => {
            if (!this.validateUsername())
                return;
            document.querySelector("host-lobby-modal")?.open();
        };
        this.openJoinLobby = () => {
            if (!this.validateUsername())
                return;
            document.querySelector("join-lobby-modal")?.open();
        };
    }
    createRenderRoot() {
        return this;
    ...
    handleLobbiesUpdate(lobbies) {
        ...
        document.dispatchEvent(new CustomEvent("public-lobbies-update", {
            detail: { payload: lobbies },
        }));
        this.requestUpdate();
        const allGames = Object.values(lobbies.games ?? {}).flat();
        for (const game of allGames) {
            const mapType = game.gameConfig?.gameMap;
            if (mapType && !this.mapAspectRatios.has(mapType)) {
                ...
                terrainMapFileLoader
                    ...
                    if (m?.map?.width && m?.map?.height) {
                        this.mapAspectRatios = new Map(this.mapAspectRatios).set(mapType, m.map.width / m.map.height);
                    }
                })
    ...
    render() {
      ...
      <div class="flex flex-col gap-4 w-full px-4 sm:px-0 mx-auto pb-4 sm:pb-0">
        ...
        <div class="sm:hidden grid grid-cols-3 gap-4 h-14">
          ${this.renderSmallActionCard(translateText("main.create"), this.openHostLobby, "bg-[color-mix(in_oklab,var(--frenchBlue)_75%,black)]")}
          ${this.renderSmallActionCard(translateText("mode_selector.ranked_title"), this.openRankedMenu, "bg-[color-mix(in_oklab,var(--frenchBlue)_75%,black)]")}
          ${this.renderSmallActionCard(translateText("main.join"), this.openJoinLobby, "bg-[color-mix(in_oklab,var(--frenchBlue)_75%,black)]")}
        </div>
        <!-- Game cards grid -->
        <div
    ...
    renderSpecialLobbyCard(lobby) {
        return this.renderLobbyCard(lobby, this.getLobbyTitle(lobby));
    }
    renderSmallActionCard(title, onClick, bgClass = CARD_BG) {
        return html `
      <button
        @click=${onClick}
    ...
    renderLobbyCard(lobby, titleContent) {
        const mapType = lobby.gameConfig.gameMap;
        ...
        const useContain = aspectRatio !== undefined && (aspectRatio > 4 || aspectRatio < 0.25);
        const timeRemaining = lobby.startsAt
            ? Math.max(0, Math.floor((lobby.startsAt - this.serverTimeOffset - Date.now()) / 1000))
            : undefined;
        let timeDisplay = "";
        ...
        const modifierLabels = getModifierLabels(lobby.gameConfig?.publicGameModifiers);
        ...
        >
          ${modifierLabels.length > 0
            ? html `<div class="flex flex-col items-start gap-1">
                ${modifierLabels.map((label) => html `<span
    ...
    getLobbyTitle(lobby) {
        ...
        if (config?.gameMode === GameMode.Team) {
            ...
            const formatTeamsOf = (teamCount, playersPerTeam, label) => {
                if (!teamCount)
            ...
            switch (config.playerTeams) {
                ...
                default:
                    if (typeof config.playerTeams === "number") {
                        ...
                        const playersPerTeam = totalPlayers && teamCount > 0
...
__decorate([
    state()
], GameModeSelector.prototype, "lobbies", void 0);
__decorate([
    state()
], GameModeSelector.prototype, "mapAspectRatios", void 0);
GameModeSelector = __decorate([
    customElement("game-mode-selector")
], GameModeSelector);
export { GameModeSelector };
//# sourceMappingURL=GameModeSelector.js.map