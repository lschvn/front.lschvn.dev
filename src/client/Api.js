import { z } from "zod";
import { ClanLeaderboardResponseSchema, PlayerProfileSchema, RankedLeaderboardResponseSchema, UserMeResponseSchema, } from "../core/ApiSchemas";
import { AnalyticsRecordSchema } from "../core/Schemas";
...
export async function createCheckoutSession(priceId, colorPaletteName) {
    try {
        ...
        if (!response.ok) {
            console.error("createCheckoutSession: request failed", response.status, response.statusText);
            return false;
        }
...
export function hasLinkedAccount(userMeResponse) {
...
export async function fetchGameById(gameId) {
    try {
        ...
        if (res.status !== 200) {
            console.warn("fetchGameById: unexpected status", res.status, res.statusText);
            return false;
        }
        const json = await res.json();
...
export async function fetchClanLeaderboard() {
    try {
        ...
        if (!res.ok) {
            console.warn("fetchClanLeaderboard: unexpected status", res.status, res.statusText);
            return false;
        }
        const json = await res.json();
        const parsed = ClanLeaderboardResponseSchema.safeParse(json);
        if (!parsed.success) {
            console.warn("fetchClanLeaderboard: Zod validation failed", parsed.error.toString());
            return false;
        }
        return parsed.data;
    }
...
export async function fetchPlayerLeaderboard(page) {
    try {
        ...
        if (!res.ok) {
            console.warn("fetchPlayerLeaderboard: unexpected status", res.status, res.statusText);
            return false;
        }
        const json = await res.json();
