...
let UserSettingModal = class UserSettingModal extends BaseModal {
    constructor() {
        super(...arguments);
        this.userSettings = new UserSettings();
        this.activeTab = "basic";
        this.keySequence = [];
        this.showEasterEggSettings = false;
        this.keybinds = {};
        this.handleEasterEggKey = (e) => {
            if (!this.isModalOpen || this.showEasterEggSettings)
                return;
        ...
        try {
            ...
            if (typeof parsed === "object" &&
                ...
                if (isValid) {
                    ...
                    console.warn("Invalid keybinds structure: entries must be objects with 'key' (string) and 'value' (string or string[]) properties. Ignoring saved data.");
                }
            }
            else {
                console.warn("Invalid keybinds data: expected non-array object. Ignoring saved data.");
            }
        }
        catch (e) {
    ...
    handleKeybindChange(e) {
        ...
        const activeKeybinds = { ...DefaultKeybinds };
        ...
        if (values.includes(value) && value !== "Null") {
            ...
            window.dispatchEvent(new CustomEvent("show-message", {
              ...
              <span class="font-medium">
                ${(() => {
                        const message = translateText("user_setting.keybind_conflict_error", { key: displayKey });
            ...
            const element = this.renderRoot.querySelector(`setting-keybind[action="${action}"]`);
            if (element) {
    ...
    changeAttackRatioIncrement(e) {
        const rawValue = e.detail?.value;
        const value = typeof rawValue === "number" ? rawValue : parseInt(String(rawValue), 10);
        if (!Number.isFinite(value)) {
            console.warn("Select event missing detail.value", e);
            return;
        }
        this.userSettings.setFloat("settings.attackRatioIncrement", Math.round(value));
        this.requestUpdate();
    }
    ...
    toggleSupplyOverlay(e) {
        const enabled = e.detail?.checked;
        if (typeof enabled !== "boolean")
            return;
        this.userSettings.set("settings.supplyOverlay", enabled);
    }
    ...
    render() {
        const activeContent = this.activeTab === "basic"
    ...
    renderKeybindSettings() {
      ...
      <setting-keybind
        action="attackModeCycle"
        label="Cycle attack mode"
        description="Switch between Blitz, Siege, and Raid."
        defaultKey="KeyV"
        .value=${this.getKeyValue("attackModeCycle")}
        .display=${this.getKeyChar("attackModeCycle")}
        @change=${this.handleKeybindChange}
      ></setting-keybind>
    ...
    renderBasicSettings() {
      ...
      <setting-toggle
        label="${translateText("user_setting.supply_overlay_label")}"
        description="${translateText("user_setting.supply_overlay_desc")}"
        id="supply-overlay-toggle"
        .checked=${this.userSettings.supplyOverlay()}
        @change=${this.toggleSupplyOverlay}
      ></setting-toggle>
      ...
      ${this.showEasterEggSettings
            ...
            <setting-number
              ...
              description="${translateText("user_setting.easter_bug_count_desc")}"
              value="100"
              min="0"
              max="1000"
              easter="true"
...
__decorate([
    state()
], UserSettingModal.prototype, "activeTab", void 0);
__decorate([
    state()
], UserSettingModal.prototype, "keySequence", void 0);
__decorate([
    state()
], UserSettingModal.prototype, "showEasterEggSettings", void 0);
__decorate([
    state()
], UserSettingModal.prototype, "keybinds", void 0);
UserSettingModal = __decorate([
    customElement("user-setting")
], UserSettingModal);
export { UserSettingModal };
//# sourceMappingURL=UserSettingModal.js.map