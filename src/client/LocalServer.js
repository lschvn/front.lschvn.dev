import { z } from "zod";
import { PartialGameRecordSchema, } from "../core/Schemas";
import { createPartialGameRecord, decompressGameRecord, getClanTag, replacer, } from "../core/Util";
...
export class LocalServer {
    constructor(lobbyConfig, isReplay, eventBus) {
    ...
    updateCallback(clientConnect, clientMessage) {
    ...
    start() {
        ...
        this.turnCheckInterval = setInterval(() => {
            ...
            const allowReplayBacklog = this.replaySpeedMultiplier === ReplaySpeedMultiplier.fastest &&
                this.lobbyConfig.gameRecord !== undefined;
            const maxBacklog = allowReplayBacklog ? MAX_REPLAY_BACKLOG_TURNS : 0;
            const canQueueNextTurn = backlog === 0 || (maxBacklog > 0 && backlog < maxBacklog);
            if (canQueueNextTurn &&
        ...
        if (this.lobbyConfig.gameRecord) {
            this.replayTurns = decompressGameRecord(this.lobbyConfig.gameRecord).turns;
        }
    ...
    onMessage(clientMsg) {
        ...
        if (clientMsg.type === "hash") {
            ...
            if (!archivedHash) {
                console.warn(`no archived hash found for turn ${clientMsg.turnNumber}, client hash: ${clientMsg.hash}`);
                return;
            }
            if (archivedHash !== clientMsg.hash) {
                console.error(`desync detected on turn ${clientMsg.turnNumber}, client hash: ${clientMsg.hash}, server hash: ${archivedHash}`);
                this.clientMessage({
                    type: "desync",
                ...
                console.log(`hash verified on turn ${clientMsg.turnNumber}, client hash: ${clientMsg.hash}, server hash: ${archivedHash}`);
            }
        }
    ...
    endGame() {
        ...
        const record = createPartialGameRecord(this.lobbyConfig.gameStartInfo.gameID, this.lobbyConfig.gameStartInfo.config, players, this.turns, this.startedAt, Date.now(), this.winner?.winner);
        ...
        const workerPath = this.lobbyConfig.serverConfig.workerPath(this.lobbyConfig.gameStartInfo.gameID);
