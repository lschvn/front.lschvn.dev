...
import { fetchCosmetics, getPlayerCosmetics, handlePurchase, patternRelationship, } from "./Cosmetics";
import { translateText } from "./Utils";
let TerritoryPatternsModal = class TerritoryPatternsModal extends BaseModal {
    constructor() {
        super();
    ...
    renderPatternGrid() {
        ...
        for (const pattern of patterns) {
            ...
            for (const colorPalette of colorPalettes) {
                let rel = "owned";
                if (pattern) {
                    rel = patternRelationship(pattern, colorPalette, this.userMeResponse, this.affiliateCode);
                }
                if (rel === "blocked") {
                    continue;
                }
                ...
                const isSelected = (isDefaultPattern && this.selectedPattern === null) ||
                    (!isDefaultPattern &&
          ...
          <pattern-button
            .pattern=${pattern}
            .colorPalette=${this.cosmetics?.colorPalettes?.[colorPalette?.name ?? ""] ?? null}
            .requiresPurchase=${rel === "purchasable"}
            .selected=${isSelected}
            .onSelect=${(p) => this.selectPattern(p)}
            .onPurchase=${(p, colorPalette) => handlePurchase(p, colorPalette)}
          ></pattern-button>
        `);
            }
        }
    ...
    renderColorSwatchGrid() {
        const hexCodes = (this.userMeResponse === false
            ? []
    ...
    selectPattern(pattern) {
        ...
        if (pattern === null) {
            ...
            const name = pattern.colorPalette?.name === undefined
                ? pattern.name
    ...
    showSkinSelectedPopup() {
        ...
        if (this.selectedPattern && this.selectedPattern.name) {
            skinName = this.selectedPattern.name
                ...
                .join(" ");
            if (this.selectedPattern.colorPalette &&
    ...
    renderColorPreview(hexCode, width, height) {
        return html `
      <div
...
__decorate([
    state()
], TerritoryPatternsModal.prototype, "selectedPattern", void 0);
__decorate([
    state()
], TerritoryPatternsModal.prototype, "selectedColor", void 0);
__decorate([
    state()
], TerritoryPatternsModal.prototype, "activeTab", void 0);
__decorate([
    state()
], TerritoryPatternsModal.prototype, "showOnlyOwned", void 0);
TerritoryPatternsModal = __decorate([
    customElement("territory-patterns-modal")
], TerritoryPatternsModal);
export { TerritoryPatternsModal };
//# sourceMappingURL=TerritoryPatternsModal.js.map