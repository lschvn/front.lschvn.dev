...
let LangSelector = class LangSelector extends LitElement {
    constructor() {
        super(...arguments);
        this.currentLang = "en";
        this.languageList = [];
    ...
    connectedCallback() {
        ...
        window.addEventListener("language-selected", this.handleLanguageSelected);
    }
    disconnectedCallback() {
        super.disconnectedCallback();
        window.removeEventListener("language-selected", this.handleLanguageSelected);
    }
    setupDebugKey() {
    ...
    getClosestSupportedLang(lang) {
        ...
        const candidates = Array.from(supported).filter((key) => key.startsWith(base));
    ...
    async loadLanguageList() {
        try {
            ...
            for (const langData of this.languageMetadata) {
                if (langData.code === "debug" && !debugLang)
                    continue;
                list.push({
                    code: langData.code,
    ...
    applyTranslation() {
        ...
        const applyAttributeTranslation = (dataAttr, targetAttr) => {
    ...
    async openModal() {
        ...
        await this.loadLanguageList();
        const languageModal = document.getElementById("page-language");
        if (languageModal) {
...
__decorate([
    state()
], LangSelector.prototype, "translations", void 0);
__decorate([
    state()
], LangSelector.prototype, "defaultTranslations", void 0);
__decorate([
    state()
], LangSelector.prototype, "currentLang", void 0);
__decorate([
    state()
], LangSelector.prototype, "languageList", void 0);
__decorate([
    state()
], LangSelector.prototype, "debugMode", void 0);
__decorate([
    state()
], LangSelector.prototype, "isVisible", void 0);
