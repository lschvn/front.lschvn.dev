...
import { ColoredTeams, Duos, GameMode, HumansVsNations, PlayerInfo, PlayerType, Quads, Trios, } from "../../core/game/Game";
import { assignTeamsLobbyPreview } from "../../core/game/TeamAssignment";
import { UserSettings } from "../../core/game/UserSettings";
import { createRandomName } from "../../core/Util";
import { getTranslatedPlayerTeamLabel, translateText } from "../Utils";
let LobbyTeamView = class LobbyTeamView extends LitElement {
    constructor() {
        super(...arguments);
        ...
        this.theme = new PastelTheme();
        this.showTeamColors = false;
    ...
    willUpdate(changedProperties) {
        ...
        if (changedProperties.has("gameMode") ||
    ...
    renderTeamMode() {
        const active = this.teamPreview.filter((t) => t.players.length > 0 || t.team === ColoredTeams.Nations);
        const empty = this.teamPreview.filter((t) => t.players.length === 0 && t.team !== ColoredTeams.Nations);
        return html ` <div
    ...
    >
      ...
      >
        ...
        ${repeat(this.clients, (c) => c.clientID ?? c.username, (client) => {
    ...
    renderFreeForAll() {
        return html `${repeat(this.clients, (c) => c.clientID ?? c.username, (client) => {
            const displayName = this.displayUsername(client);
    ...
    renderTeamCard(preview, isEmpty = false) {
        const displayCount = preview.team === ColoredTeams.Nations
            ? this.effectiveNationCount
            : preview.players.length;
        const maxTeamSize = preview.team === ColoredTeams.Nations
            ? this.effectiveNationCount
            : this.teamMaxSize;
      ...
      <div class="bg-gray-800 border border-gray-700 rounded-xl flex flex-col">
        ...
        <div class="p-2 ${isEmpty ? "" : "flex flex-col gap-1.5"}">
          ${isEmpty
            ...
            : repeat(preview.players, (p) => p.clientID ?? p.username, (p) => {
    ...
    computeTeamPreview(teams = []) {
        ...
        const players = this.clients.map((c) => new PlayerInfo(c.username, PlayerType.Human, c.clientID, c.clientID));
        const assignment = assignTeamsLobbyPreview(players, teams, this.effectiveNationCount);
        ...
        if (this.teamCount === Duos) {
            ...
            this.teamMaxSize = Math.max(1, Math.ceil((this.clients.length + this.effectiveNationCount) / teams.length));
        }
    ...
    displayUsername(client) {
        ...
        return (createRandomName(client.username, PlayerType.Human) ?? client.username);
    }
};
__decorate([
    property({ type: String })
], LobbyTeamView.prototype, "gameMode", void 0);
__decorate([
    property({ type: Array })
], LobbyTeamView.prototype, "clients", void 0);
__decorate([
    state()
], LobbyTeamView.prototype, "teamPreview", void 0);
__decorate([
    state()
], LobbyTeamView.prototype, "teamMaxSize", void 0);
__decorate([
    property({ type: String })
], LobbyTeamView.prototype, "lobbyCreatorClientID", void 0);
__decorate([
    property({ type: String })
], LobbyTeamView.prototype, "currentClientID", void 0);
__decorate([
    property({ attribute: "team-count" })
], LobbyTeamView.prototype, "teamCount", void 0);
__decorate([
    property({ type: Function })
], LobbyTeamView.prototype, "onKickPlayer", void 0);
__decorate([
    property({ type: Number })
], LobbyTeamView.prototype, "nationCount", void 0);
__decorate([
    property({ type: Boolean })
], LobbyTeamView.prototype, "isPublicGame", void 0);
__decorate([
    state()
], LobbyTeamView.prototype, "showTeamColors", void 0);
LobbyTeamView = __decorate([
    customElement("lobby-player-view")
], LobbyTeamView);
export { LobbyTeamView };
//# sourceMappingURL=LobbyPlayerView.js.map