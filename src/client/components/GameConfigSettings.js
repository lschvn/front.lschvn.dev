...
import { Difficulty, Duos, GameMode, HumansVsNations, Quads, Trios, UnitType, } from "../../core/game/Game";
...
const DISABLED_CARD = "w-full rounded-xl border transition-all duration-200 opacity-30 grayscale cursor-not-allowed bg-white/5 border-white/5";
...
const DIFFICULTY_OPTIONS = Object.entries(Difficulty).filter(([key]) => isNaN(Number(key)));
...
function stateTextClass(active) {
    return active ? "text-white" : "text-white/60";
}
function renderTextCardButton(label, active, onClick, cardExtraClass) {
    return html `
...
function renderSection(iconSvg, colorClass, bgClass, titleKey, content, sectionClass = "space-y-6") {
    return html `
...
const ENABLES_ICON = svg `<path
  ...
  clip-rule="evenodd"
/>`;
function renderSectionHeader(iconSvg, colorClass, bgClass, titleKey) {
    return html `
...
let GameConfigSettings = class GameConfigSettings extends LitElement {
    constructor() {
        super(...arguments);
        this.sectionGapClass = "space-y-6";
        this.handleSelectMap = (map) => {
            this.emit("map-selected", { map });
        };
        this.handleSelectRandom = () => {
        ...
        this.handleTeamCountSelect = (count) => {
        ...
        this.handleBotsChanged = (event) => {
            const customEvent = event;
            this.emit("bots-changed", customEvent.detail);
        };
        this.handleNationsChanged = (event) => {
            const customEvent = event;
            this.emit("nations-changed", customEvent.detail);
        };
        this.handleUnitToggle = (unit, checked) => {
    ...
    renderOptionToggle(toggle) {
        if (toggle.hidden)
            return html ``;
        return renderTextCardButton(translateText(toggle.labelKey), toggle.checked, () => this.handleOptionToggle(toggle), "p-4 text-center");
    }
    renderUnitTypeOptions(disabledUnits) {
    ...
    render() {
      ...
      <div class=${this.sectionGapClass}>
        ${renderSection(MAP_ICON, "text-blue-400", "bg-blue-500/20", "map.map", html `<map-picker
          ...
          ></map-picker>`)}
        ${renderSection(DIFFICULTY_ICON, "text-green-400", "bg-green-500/20", "difficulty.difficulty", html `
            ...
            return html `
                  <button
                    ?disabled=${isDisabled}
                    @click=${() => !isDisabled &&
                this.handleDifficultySelect(value)}
                    class="${isDisabled
                ? `${DISABLED_CARD} flex flex-col items-center p-4 gap-3`
                : cardClass(isSelected, "flex flex-col items-center p-4 gap-3")}"
                  >
                    <difficulty-display
                      .difficultyKey=${key}
        ...
        ${renderSection(OPTIONS_ICON, "text-orange-400", "bg-orange-500/20", settings.options.titleKey, html `
            ...
            : nothing}
              ${settings.options.toggles.map((toggle) => this.renderOptionToggle(toggle))}
...
], GameConfigSettings.prototype, "settings", void 0);
__decorate([
    property({ attribute: false })
], GameConfigSettings.prototype, "sectionGapClass", void 0);
GameConfigSettings = __decorate([
    customElement("game-config-settings")
], GameConfigSettings);
export { GameConfigSettings };
//# sourceMappingURL=GameConfigSettings.js.map