...
const unitOptions: { type: UnitType; translationKey: string }[] = [
  ...
  { type: UnitType.RadarStation, translationKey: "unit_type.radar_station" },
  { type: UnitType.Port, translationKey: "unit_type.port" },
  { type: UnitType.AABattery, translationKey: "unit_type.aa_battery" },
  { type: UnitType.Warship, translationKey: "unit_type.warship" },
  { type: UnitType.TransportShip, translationKey: "unit_type.boat" },
  { type: UnitType.FighterSquadron, translationKey: "unit_type.fighter_squadron" },
  { type: UnitType.BomberSquadron, translationKey: "unit_type.bomber_squadron" },
...
export interface GameConfigSettingsData {
  ...
  procedural?: {
    visible: boolean;
    seed: string;
    worldSizePreset: ProceduralWorldSizePreset;
    landmassStyle: ProceduralLandmassStyle;
    performancePreset: ProceduralPerformancePreset;
    warningKey?: string;
  };
  unitTypes: {
    titleKey: string;
...
export class GameConfigSettings extends LitElement {
  ...
  private handleProceduralSeedInput = (event: Event) => {
    const input = event.target as HTMLInputElement;
    this.emit("procedural-seed-changed", { seed: input.value.trim() });
  };

  private handleProceduralRandomSeed = () => {
    this.emit("procedural-random-seed", {});
  };

  private handleWorldSizePresetChange = (event: Event) => {
    const select = event.target as HTMLSelectElement;
    this.emit("procedural-world-size-changed", {
      worldSizePreset: select.value as ProceduralWorldSizePreset,
    });
  };

  private handleLandmassStyleChange = (event: Event) => {
    const select = event.target as HTMLSelectElement;
    this.emit("procedural-landmass-style-changed", {
      landmassStyle: select.value as ProceduralLandmassStyle,
    });
  };

  private handlePerformancePresetChange = (event: Event) => {
    const select = event.target as HTMLSelectElement;
    this.emit("procedural-performance-preset-changed", {
      performancePreset: select.value as ProceduralPerformancePreset,
    });
  };
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <label class="col-span-1 md:col-span-2 flex flex-col gap-2">
                    <span class="text-xs font-bold uppercase tracking-wider text-white/70">
                      ${translateText("map.seed")}
                    </span>
                    <div class="flex gap-2">
                      <input
                        class="flex-1 rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-white"
                        .value=${settings.procedural.seed}
                        @change=${this.handleProceduralSeedInput}
                        @blur=${this.handleProceduralSeedInput}
                      />
                      <button
                        class="rounded-lg border border-blue-400/40 bg-blue-500/20 px-3 py-2 text-xs font-bold uppercase tracking-wider text-blue-100"
                        @click=${this.handleProceduralRandomSeed}
                      >
                        ${translateText("map.random_seed")}
                      </button>
                    </div>
                  </label>
                  <label class="flex flex-col gap-2">
                    <span class="text-xs font-bold uppercase tracking-wider text-white/70">
                      ${translateText("map.world_size_preset")}
                    </span>
                    <select
                      class="rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-white"
                      @change=${this.handleWorldSizePresetChange}
                    >
                      ${Object.values(ProceduralWorldSizePreset).map(
                        (preset) => html`
                          <option
                            value=${preset}
                            ?selected=${settings.procedural?.worldSizePreset ===
                            preset}
                          >
                            ${preset}
                          </option>
                  ...
                  <label class="flex flex-col gap-2">
                    <span class="text-xs font-bold uppercase tracking-wider text-white/70">
                      ${translateText("map.landmass_style")}
                    </span>
                    <select
                      class="rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-white"
                      @change=${this.handleLandmassStyleChange}
                    >
                      ${Object.values(ProceduralLandmassStyle).map(
                        (preset) => html`
                          <option
                            value=${preset}
                            ?selected=${settings.procedural?.landmassStyle ===
                            preset}
                          >
                            ${preset}
                          </option>
                  ...
                  <label class="flex flex-col gap-2">
                    <span class="text-xs font-bold uppercase tracking-wider text-white/70">
                      ${translateText("map.performance_preset")}
                    </span>
                    <select
                      class="rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-white"
                      @change=${this.handlePerformancePresetChange}
                    >
                      ${Object.values(ProceduralPerformancePreset).map(
                        (preset) => html`
                          <option
                            value=${preset}
                            ?selected=${settings.procedural?.performancePreset ===
                            preset}
                          >
                            ${preset}
                          </option>
                  ...
                  ${settings.procedural.warningKey
                    ? html`<div
                        class="col-span-1 md:col-span-2 rounded-xl border border-yellow-500/40 bg-yellow-500/10 px-3 py-2 text-xs font-bold uppercase tracking-wider text-yellow-300"
                      >
                        ${translateText(settings.procedural.warningKey)}
                      </div>`
                    : nothing}
                </div>
              `,
          ...
          html`
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              >
                <fluent-slider
                  min="0"
                  max="${settings.options.bots.maxValue ?? 400}"
                  step="1"
              ...
              ${settings.options.nations && !settings.options.nations.hidden
                ? html`<div
                  ...
                  >
                    <fluent-slider
                      min="0"
                      max="${settings.options.nations.maxValue ?? 400}"
                      step="1"
