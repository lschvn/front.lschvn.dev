...
let LeaderboardPlayerList = class LeaderboardPlayerList extends LitElement {
    constructor() {
        super(...arguments);
        this.playerData = [];
        ...
        this.hasLoadedPlayers = false;
        this.playerPageSize = 50;
        this.currentPage = 1;
        this.currentUserId = null;
        this.currentUserIdLoaded = false;
    }
    createRenderRoot() {
        return this;
    }
    async ensureLoaded() {
        if (this.hasLoadedPlayers || this.isLoading)
            return;
        await this.loadPlayerLeaderboard(true);
    }
    async loadPlayerLeaderboard(reset = false) {
        if (reset) {
            this.currentPage = 1;
            this.playerHasMore = true;
            this.loadMoreError = null;
        ...
        try {
            ...
            const nextPlayers = result[RankedType.OneVOne].map((entry) => ({
                rank: entry.rank,
            ...
            if (reset) {
                ...
                const existingIds = new Set(this.playerData.map((player) => player.playerId));
                const deduped = nextPlayers.filter((player) => !existingIds.has(player.playerId));
            ...
            if (this.currentUserId && !this.currentUserEntry) {
                this.currentUserEntry =
                    nextPlayers.find((player) => player.playerId === this.currentUserId) ?? null;
            }
            this.hasLoadedPlayers = true;
    ...
    updateStickyVisibility() {
        ...
        const currentRow = this.scrollContainer.querySelector('[data-current-user="true"]');
        if (!currentRow) {
        ...
        <div class="h-full border border-white/5 bg-black/20 relative">
          ...
          ${this.currentUserEntry
            ? html `
                ...
                ? this.currentUserEntry.username.replace(/^\[.*?\]\s*/, "")
...
__decorate([
    state()
], LeaderboardPlayerList.prototype, "playerData", void 0);
__decorate([
    state()
], LeaderboardPlayerList.prototype, "currentUserEntry", void 0);
__decorate([
    state()
], LeaderboardPlayerList.prototype, "showStickyUser", void 0);
__decorate([
    state()
], LeaderboardPlayerList.prototype, "isLoading", void 0);
__decorate([
    state()
], LeaderboardPlayerList.prototype, "error", void 0);
__decorate([
    state()
], LeaderboardPlayerList.prototype, "isLoadingMore", void 0);
__decorate([
    state()
], LeaderboardPlayerList.prototype, "loadMoreError", void 0);
__decorate([
    state()
], LeaderboardPlayerList.prototype, "playerHasMore", void 0);
__decorate([
    query(".scroll-container")
], LeaderboardPlayerList.prototype, "scrollContainer", void 0);
LeaderboardPlayerList = __decorate([
    customElement("leaderboard-player-list")
], LeaderboardPlayerList);
export { LeaderboardPlayerList };
//# sourceMappingURL=LeaderboardPlayerList.js.map